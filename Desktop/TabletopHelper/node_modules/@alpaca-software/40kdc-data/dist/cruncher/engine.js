...
export function profileCanTarget(profile, target) {
    ...
    if (required.length > 0 && !required.some((keyword) => keywords.has(keyword.toLowerCase())))
        return false;
...
export function crunch(input, dataset) {
    ...
    if (!profileCanTarget(weaponProfile, input.target.unit)) {
        throw new RangeError(`crunch: weapon ${input.attacker.weapon.id} profile ${input.attacker.profileIndex} cannot target unit ${input.target.unit.id}`);
    }
    ...
    const covered = resolved.cover.active && !ignoresCover && !isMelee;
...
function profileBuffsFor(attacker, dataset, ctx) {
    ...
    for (const ref of profile.keywords ?? []) {
        ...
        if (!view)
            continue;
        out.push(...view.getBuffs(parameters, attacker.weapon.id, ctx));
    }
    return out;
}
