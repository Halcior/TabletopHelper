...
const MODEL_SCOPED_REASON = "model-scoped effect from an attached model: applies to that model only (core rule 19.04)";
/**
 ...
 */
function isModelScopedFromAttachedMember(node, source) {
    if (source.kind !== "ability" || source.abilityKind !== "attached")
        return false;
...
function walk(node, source, opts, out) {
    if (!isObject(node))
        return;
    const currentNode = opts.defaultTarget !== undefined && node.target === undefined
...
function translateNamedRegionState(node, source, opts, out) {
    if (opts.perspective !== "attacker")
        return;
    ...
    const keywords = gate && Array.isArray(gate.keywords)
        ? gate.keywords.filter((keyword) => typeof keyword === "string")
        : [];
    ...
    if (qualifiedBranch) {
        out.unsupported.push({
            reason: "named-region-state qualified branch: region membership is unavailable in EngineContext; qualified replacement is unsupported",
...
function enumerateNamedOptions(node, source, opts, out, groupId, maxActivations) {
    const options = Array.isArray(node.options) ? node.options : [];
    for (const opt of options) {
        if (!isObject(opt))
            continue;
 ...
 */
function enumerateMenuActions(node, source, opts, out) {
    ...
    for (const action of actions) {
        ...
        const applicability = requiresKeyword?.length === 1 && typeof requiresKeyword[0] === "string"
...
function evaluateCondition(condition, ctx) {
    ...
    switch (condition.type) {
        ...
        case "attack-is-type": {
            const attackType = condition.parameters?.attack_type;
            if (attackType === "melee")
