 ...
 */
export function loadFactions(only) {
    const out = [];
...
function loadUnitNames(faction) {
    try {
        const units = JSON.parse(readFileSync(resolve(CORE_ROOT, faction, "units.json"), "utf-8"));
 ...
 */
function loadRawText(faction, storeRoot) {
    try {
        const recs = JSON.parse(readFileSync(resolve(storeRoot, `${faction}.json`), "utf-8"));
        return new Map(recs.filter((r) => r.raw_text && r.raw_text.trim()).map((r) => [r.ability_id, r.raw_text]));
    }
    catch {
        return new Map();
    }
 ...
 */
function reviewMarkdown(report, storeRoot) {
    const L = [
    ...
    for (const { faction } of report.byFaction) {
        ...
        for (const r of rows) {
            const units = r.unit_ids.length === 0
            ...
            if (r.flags.length > 0)
                L.push(`- **Phrasing flags:** ${r.flags.join(", ")}`);
            L.push("");
        }
    }
    return L.join("\n");
...
export function auditPhrasingCommand(opts = {}) {
    ...
    if (opts.review) {
        ...
        console.log(`author:report — ${report.total} abilities${opts.faction ? ` (${opts.faction})` : ""} → _private/reports/${name}`);
        return;
    }
    if (opts.write) {
