import { attachmentToken, charSlotAssignment, coarsenedLoadoutGroups, displayedUnitPoints, groupWeaponsText, titleCaseId, totalArmyPoints, } from "./helpers.js";
export const FENCE = "+++++++++++++++++++++++++++++++++++++++++++++++";
function keywordTokens(unit) {
    return (unit.keyword_overrides ?? []).map((keyword) => keyword === "Character"
...
function header(roster, units, charSlots) {
    ...
    const detachmentLines = roster.detachments.length > 0
 ...
 */
export function wtcCompactBodyLines(units, slots) {
    const lines = [""];
    for (let i = 0; i < units.length; i += 1) {
        ...
        lines.push(`${prefix}${u.model_count}x ${u.ref.raw_name} (${ptsText}): ${exactGroups ? "" : wargearListText(u, true)}`);
        if (exactGroups)
            lines.push(...exactGroups);
 ...
 */
export function wtcModelLines(u) {
    const exactGroups = exactGroupLines(u);
    if (exactGroups)
        return exactGroups;
    if (u.model_count > 1) {
        ...
        if (coarse && coarse.length > 1) {
            return coarse.map((c, i) => {
                ...
                return `${c.count} with ${groupWeaponsText(c.wargear)}${tags.length > 0 ? `, ${tags.join(", ")}` : ""}`;
            });
        }
 ...
 * newline.
 */
export function wtcFullBodyLines(units, slots, factionId) {
 ...
 */
export function fullBodyLines(units, slots, factionId, modelLines) {
    const battlelineIdxs = [];
    const alliedIdxs = [];
    for (let i = 0; i < units.length; i += 1) {
        if (isAlliedUnit(units[i], factionId))
            alliedIdxs.push(i);
        else
