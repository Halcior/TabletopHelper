...
function keywordLabel(name, parameters) {
    if (parameters) {
    ...
    return weapon.raw.profiles.map((p, i) => {
        ...
        return el("profile", [["name", p.name], ["typeName", typeName]], [
            el("characteristics", [], [
                ...
                leaf("characteristic", [["name", skillName]], typeof skill === "number" ? fmtTarget(skill) : "N/A"),
...
function resolveWeapon(w, dataset, factionId) {
    const id = w.ref.id;
    if (id === null)
        return undefined;
...
function upgradeSelection(id, weapon, totalCount) {
    return el("selection", [["id", id], ["name", weapon.name], ["type", "upgrade"], ["number", String(totalCount)]], [el("profiles", [], weaponProfiles(weapon))]);
}
/** One `<selection type="model">` for a loadout group, with its per-model
 * weapons nested as upgrade selections. */
function modelSelection(idBase, modelName, modelCount, wargear, dataset, factionId) {
    const upgrades = [];
    wargear.forEach((w, wi) => {
        const weapon = resolveWeapon(w, dataset, factionId);
        if (!weapon)
            return; // unresolved weapon — skip (already flagged in diagnostics)
        upgrades.push(upgradeSelection(`${idBase}-w${wi}`, weapon, w.count * modelCount));
    });
    const children = [];
    if (upgrades.length > 0)
        children.push(el("selections", [], upgrades));
    return el("selection", [["id", idBase], ["name", modelName], ["type", "model"], ["number", String(modelCount)]], children);
}
 ...
 * expects). */
function modelSelections(unit, unitId, view, dataset, factionId) {
    const groups = unit.loadout_groups && unit.loadout_groups.length > 0
        ? unit.loadout_groups
        : [{ model_name: null, count: unit.model_count, wargear: unit.wargear }];
    return groups.map((g, gi) => modelSelection(`${unitId}-m${gi}`, g.model_name ?? view.name, g.count, g.wargear, dataset, factionId));
}
...
function unitSelection(unit, index, dataset, factionId) {
    ...
    return el("selection", [["id", unitId], ["name", unit.ref.raw_name], ["type", "unit"], ["number", "1"]], children);
}
export const yellowscribeSerializer = {
    id: "yellowscribe",
    serialize(roster, dataset) {
...
//# sourceMappingURL=yellowscribe.js.map