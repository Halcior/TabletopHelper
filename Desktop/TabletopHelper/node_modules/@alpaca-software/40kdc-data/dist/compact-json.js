...
const arrayBreaks = (a) => a.some(isContainer);
function renderArray(arr, indent, ctx) {
    if (arr.length === 0)
        return "[]";
    if (!arrayBreaks(arr))
        return "[" + arr.map((e) => JSON.stringify(e)).join(", ") + "]";
    const inner = indent + "  ";
    const parts = arr.map((e) => inner +
        (Array.isArray(e)
            ? renderArray(e, inner, ctx)
            : isContainer(e)
                ? renderObject(e, inner, ctx)
                : JSON.stringify(e)));
    return "[\n" + parts.join(",\n") + "\n" + indent + "]";
}
function renderObject(obj, indent, ctx) {
    ...
    const renderVal = (v) => Array.isArray(v)
        ...
        : isContainer(v)
            ? renderObject(v, childIndent, "inline")
            : JSON.stringify(v);
    if (ctx === "block") {
        const inner = indent + "  ";
        return ("{\n" +
...
export function formatCompact(data) {
    const body = Array.isArray(data)
        ...
        : isContainer(data)
            ? renderObject(data, "", "block")
            : JSON.stringify(data);
    return body + "\n";
}
//# sourceMappingURL=compact-json.js.map