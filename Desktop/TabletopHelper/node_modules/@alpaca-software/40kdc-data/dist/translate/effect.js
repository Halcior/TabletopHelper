 ...
 */
import { describeCondition, describeSelectionEligibility, describeTiming, negatedTiming, eventClause, dekebab } from "./condition.js";
...
function selectUnitsSubject(sel = {}) {
    const kw = (sel.keywords ?? []).map((k) => titleCase(jstr(k))).join(" ");
    const exactCount = sel.count ?? (sel.min_count != null && Number(sel.min_count) === Number(sel.max_count)
        ? sel.max_count
        : undefined);
    ...
    const eligibility = typeof sel.eligibility === "object" && sel.eligibility != null
        ? ` ${describeSelectionEligibility(sel.eligibility)}`
        : "";
    return `${quantity} ${jstr(sel.owner)}${kw ? ` ${kw}` : ""} ${noun}${inclusive}${within}${visible}${eligibility}`;
}
function selectUnitsEngagement(sel = {}) {
...
function selectUnitsPlural(sel = {}) {
    return Number(sel.count ?? sel.max_count) > 1;
}
function selectedRecipient(text, sel = {}) {
...
function leaderModelAbilityGrantClause(e, ctx) {
    ...
    const role = e.beneficiary === "attached-character-leader"
    ...
    const rendered = describeEffectInline({ ...nested, target: "self" }, ctx).replace(/^this model\b/, "that leader model");
    return `while ${leader} leads ${source}, ${rendered}`;
}
/** "enemy unit within 6\"" — the `for-each-unit` selector phrase. */
function forEachUnitSubject(sel = {}) {
...
function designationTargetSubject(sel) {
...
function transportCapacityConversion(m) {
    ...
    const eligibility = m.transport_eligibility;
    const qualification = eligibility?.requires_capacity_keyword != null
    ...
    if (m.occupancy_kind === "equivalent-model") {
        const equivalent = m.equivalent_model_keyword != null
...
function persistentDesignationLead(e) {
    const select = typeof e.select === "object" && e.select ? e.select : {};
...
function persistentDesignationWhen(e) {
    const select = typeof e.select === "object" && e.select ? e.select : {};
    const consumer = e.consumer ?? {};
...
function bracketKeyword(k) {
    ...
    if (anti) {
        ...
        if (m)
            return `[ANTI-${dekebab(m[1]).trim().toUpperCase()} ${m[2]}+]`;
 ...
 * "for each".
 */
function poolThreshold(comp, threshold) {
...
function subject(target, ctx) {
    const within = ctx.rangeInches != null
 ...
 */
function ofOrPossessive(subj, rest) {
...
function endOfPhaseDisembarkBattleShockCondition(c) {
    if (c?.operator !== "and" || c.operands?.length !== 2)
        return false;
...
function describeTrigger(t) {
    ...
    if (t.event === "falls-back" && t.subject === "enemy-unit")
    ...
    if (t.proximity?.range != null) {
        const of = t.proximity.of === "attached-unit"
    ...
    if (t.event === "end-of-phase" && endOfPhaseDisembarkBattleShockCondition(t.condition))
        ...
        s += `, if ${describeCondition(t.condition)}`;
    return s;
}
...
function menuActionSubject(elig) {
    const requires = elig?.requires_keyword ?? [];
    const excludes = elig?.excludes_keyword ?? [];
    if (excludes.length)
        return `one friendly non-${excludes.map(jstr).join("/")} unit`;
    if (requires.length)
        return `a friendly ${requires.map(jstr).join(" ")} unit`;
    return "the unit";
}
...
function menuActionEligibilityClause(elig) {
    if (!elig)
        return "";
    ...
    if (!hasKeywordGate && requirementPhrases.length === 0)
        return "";
    const parts = [];
    if (hasKeywordGate)
        parts.push(`only usable by ${menuActionSubject(elig)}`);
    if (requirementPhrases.length)
        parts.push(requirementPhrases.join(" and "));
...
function menuActionDurationClause(duration) {
    switch (duration) {
...
function describeMenuAction(a, ctx) {
...
function sharedUsageClause(su) {
    if (!su)
        return "";
    const parts = [];
    if (su.unit_max_manoeuvres_per_phase != null) {
        parts.push(su.unit_max_manoeuvres_per_phase === 1
    ...
    if (su.default_manoeuvre_max_per_phase != null) {
        parts.push(su.default_manoeuvre_max_per_phase === 1
...
function negatedTargetKeywords(keywords) {
 ...
 */
function notWrappedTargetKeyword(op) {
    if (op.operator !== "not" || !op.operands || op.operands.length !== 1)
        return null;
    const inner = op.operands[0];
    if (inner.type !== "target-has-keyword" || inner.negated)
        return null;
...
function excludedTargetKeywords(keywords) {
 ...
 */
function joinAndLeadIns(operands) {
    const parts = [];
    ...
    return parts.reduce((acc, part) => acc === ""
        ? part
...
function conditionLeadIn(c) {
    ...
    if (c.operator === "and" && c.operands)
        return joinAndLeadIns(c.operands);
    ...
    if (c.negated && c.type === "timing-is")
        return negatedTiming((c.parameters ?? {}).timing);
    if (c.negated)
    ...
    switch (c.type) {
        ...
        case "player-turn-is": {
            const t = jstr(p.turn);
            const phrase = t === "your-turn" || t === "your" || t === "own"
                ? "your"
        ...
        case "battle-round": {
            ...
            const bOrd = (n) => ["zeroth", "first", "second", "third", "fourth", "fifth"][n] ?? `${n}th`;
            if (bMin != null && bMax != null)
                return bMin === bMax ? `during the ${bOrd(bMin)} battle round` : `during battle rounds ${bMin}-${bMax}`;
            if (bMin != null)
                return `from the ${bOrd(bMin)} battle round onward`;
            if (bMax != null)
                return `during the first ${bMax} battle rounds`;
...
function scalingClause(s) {
    const ofText = SCALE_OF[jstr(s.of)] ?? dekebab(jstr(s.of));
    let c = `for every ${jstr(s.per)} ${ofText}`;
    if (s.within_inches != null)
        c += ` within ${jstr(s.within_inches)}"`;
    if (s.round === "up")
        c += " (rounding up)";
    if (s.max_value != null)
        c += ` (to a maximum of ${jstr(s.max_value)})`;
    return c;
}
/** Movement-modifier passthrough enum → human phrase. */
const PASSTHROUGH_PHRASE = {
...
function movementClause(m, subj) {
    const kind = m.move_type;
    ...
    const moveKinds = Array.isArray(m.applies_to_moves)
        ? andList(m.applies_to_moves.map((x) => MOVE_NOUN[x] ?? dekebab(x)))
        : null;
    ...
    if (kind == null) {
        ...
        if (Array.isArray(m.passthrough) && m.passthrough.length) {
            parts.push(m.passthrough.map((p) => PASSTHROUGH_PHRASE[p] ?? dekebab(p)).join(" and "));
        }
        let clause;
        if (parts.length) {
        ...
        if (m.excludes_keyword != null)
            clause += ` (excluding ${titleCase(jstr(m.excludes_keyword))} models)`;
        if (moveKinds)
...
function auraEligibleSubject(who, eligible) {
...
function keywordFilterClause(value, noun) {
...
function auraClause(e, m, ctx) {
    ...
    const rangeText = Array.isArray(range)
        ? `${range.map((r) => `${r}"`).join("/")} (by battle round)`
        : range != null
            ? `${jstr(range)}"`
            : null;
    ...
    const effectText = m.effect != null
        ? filtered
            ? `, and each such unit ${describeEffectInline(m.effect, { ...ctx, selectedUnit: true }).replace(/^the unit\b\s*/, "")}`
            : ` ${describeEffectInline(m.effect, m.eligible != null ? { ...ctx, selectedUnit: true } : { ...ctx })}`
        : filtered
...
function resurrectionPlacement(placement) {
    if (placement == null)
        return "";
...
function resurrectionTiming(timing) {
    if (timing == null)
        return "";
    switch (jstr(timing)) {
...
function describeRequirement(req) {
    const one = (r) => `${jstr(r?.type)} of ${jstr(r?.min_value)}+`;
    const anyOf = req?.any_of;
    if (Array.isArray(anyOf))
        return anyOf.map(one).join(" or ");
    return one(req);
}
function diceTableResultLabel(results) {
    if (!Array.isArray(results))
        return "";
    const faces = results.filter((value) => typeof value === "number").sort((a, b) => a - b);
    if (faces.length > 1 && faces.every((face, index) => index === 0 || face === faces[index - 1] + 1)) {
        return `${faces[0]}-${faces[faces.length - 1]}`;
    }
    return faces.join(", ");
}
function diceTableInline(e, ctx) {
    const outcomes = (e.outcomes ?? []).map((outcome) => `on ${diceTableResultLabel(outcome.results)}, ${describeEffectInline(outcome.effect ?? {}, ctx)}`);
    return `roll one ${diceCase(e.dice)}: ${outcomes.join("; ")}`;
}
function describeEffectInlineBase(e, ctx = {}) {
    const m = e.modifier ?? {};
    ...
    switch (e.type) {
        ...
        case "roll-modifier": {
            ...
            if (m.value == null)
                return `${dekebab(jstr(m.operation))} ${ofOrPossessive(subj, `${rollName(roll)} rolls`)}${ctxNote}`;
        ...
        case "mortal-wounds": {
            ...
            if (m.mortal_per_success != null) {
                ...
                if (m.per_model != null) {
                    const where = m.model_relation === "engaged-with-target"
            ...
            const table = (m.amount_table ?? m.table);
        ...
        case "keyword-grant": {
            ...
            if (m.weapon_name != null)
                return `${ofOrPossessive(subj, jstr(m.weapon_name))} gains ${kw}`;
            if (m.weapon_type != null)
                return `${ofOrPossessive(subj, `${jstr(m.weapon_type)} weapons`)} gain ${kw}`;
        ...
        case "ability-grant": {
            ...
            if (grant != null && m.enabled === false)
                return `${subj} cannot use the ${grantLabel(jstr(grant))} ability`;
            return grant != null
        ...
        case "modifier-immunity": {
            ...
            const exc = Array.isArray(m.exclude) && m.exclude.length
                ? ` (except ${m.exclude.map((s) => statName(jstr(s))).join(" and ")})`
                : "";
        ...
        case "replace-roll-from-pool": {
            const rolls = Array.isArray(m.rolls) ? m.rolls.map((r) => dekebab(jstr(r))) : [];
        ...
        case "auto-result": {
            ...
            if (m.test != null) {
                if (r === "pass")
                    return `${subj} automatically ${v(subj, "passes")} ${testName(m.test)} tests`;
                if (r === "fail")
                    return `${subj} automatically ${v(subj, "fails")} ${testName(m.test)} tests`;
        ...
        case "disembark-after-move": {
            ...
            const who = m.requires_keyword != null
                ? `units with the ${titleCase(jstr(m.requires_keyword))} ability`
                : "units";
            const when = m.after === "advance"
            ...
            const away = m.min_enemy_distance != null
        ...
        case "designate-target": {
            const sel = (typeof e.select === "object" && e.select ? e.select : {});
            ...
            const when = e.applies?.to === "target"
...
function namedRegionRecord(value) {
    return value != null && typeof value === "object" && !Array.isArray(value)
        ? value
        : {};
}
function namedRegionTitle(value) {
    return titleCase(jstr(value));
}
function namedRegionRelation(value) {
    return jstr(value) === "wholly-within" ? "wholly within" : dekebab(jstr(value));
}
function namedRegionKeywords(value) {
    return Array.isArray(value) ? value.map(jstr).join(" or ") : "?";
}
function namedRegionPrefix(m) {
    ...
    for (const entry of phaseExtensions) {
        ...
        if (zone === "no-mans-land") {
            sentences.push(`At the start of each phase, No Man's Land is within ${region} until the end of that phase if you control at least half of its objective markers.`);
            hasPhaseExtension = true;
        }
        else if (zone === "opponent-deployment-zone") {
            sentences.push(hasPhaseExtension
            ...
            sentences.push(`At the start of each phase, ${label} is within ${region} until the end of that phase if you control at least half of its objective markers.`);
            hasPhaseExtension = true;
        }
    }
...
function namedRegionEffect(branch, qualified, ctx = {}) {
...
function namedRegionBranchText(m, wholeUnit, qualified, conditional = false, ctx = {}) {
    const consumer = namedRegionRecord(m.consumer);
...
function describeNamedRegionState(m, ctx = {}) {
    const consumer = namedRegionRecord(m.consumer);
    const membership = namedRegionRecord(consumer.membership);
...
function describeNamedRegionConditional(m, condition, ctx = {}) {
    const consumer = namedRegionRecord(m.consumer);
    const membership = namedRegionRecord(consumer.membership);
    const wholeUnit = membership.unit_scope === "whole-unit";
    const positive = { ...condition, negated: false };
 ...
 */
function describeRuleState(m, subj) {
    ...
    if (kind === "faction-rule" && !granted) {
        ...
        let cost = "";
        const c = m.cost;
        if (c != null && typeof c === "object" && c.dice != null) {
            const cc = c;
            const from = cc.from == null
                ? ""
                : jstr(cc.from) === rule
                    ? " from that roll"
    ...
    if (kind === "faction-rule")
        return `${subj} ${v(subj, "gains")} ${titleCase(rule)}`;
...
export function describeEffect(e, depth = 0, ctx = {}) {
    ...
    switch (e.type) {
        ...
        case "dice-pool-allocation": {
            const pool = e.pool ? `${jstr(e.pool.count)}${jstr(e.pool.die)}` : "your dice pool";
            const upTo = e.max_activations != null
                ? ` to activate up to ${jstr(e.max_activations)} of the following`
                : " to activate the following";
            const lines = [`${indent}${arrow}Roll ${pool}; allocate dice${upTo}:`];
            for (const opt of e.options ?? []) {
                lines.push(`${indent}  - ${jstr(opt.name)} (requires ${describeRequirement(opt.requirement)}): ${describeEffectInline(opt.effect ?? {}, ctx)}.`);
            }
            return lines.join("\n");
        }
        case "select-units": {
...
export function describeScope(s) {
    ...
    const duration = s.duration === "until-next-battle-round"
 ...
 */
function auraRadius(scope) {
    if (scope?.range_inches != null)
...
function conditionWithinRange(c) {
...
function renderTopLevel(e, scope, usage, trigger) {
    const ctx = {
    ...
    const trig = triggers
        .map((t) => describeTrigger(condRange != null && t.proximity?.range === condRange ? { ...t, proximity: undefined } : t))
        .filter((s) => s.length > 0)
        .join(" or ");
    ...
    if (CONTAINER_TYPES.has(e.type ?? "")) {
        ...
        const ownDuration = (e.type === "designate-target" || e.type === "persistent-designation") && e.duration != null;
