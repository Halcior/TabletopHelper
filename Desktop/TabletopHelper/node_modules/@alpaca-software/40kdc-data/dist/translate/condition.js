...
const TIMING_ONLY_PHRASES = {
    ...
    "friendly-unit-empowered-within-9": 'each time you spend 1 Pain token to Empower a friendly unit within 9" of this unit',
...
export function describeTiming(timing) {
    const t = str(timing);
    if (TIMING_ONLY_PHRASES[t])
        return TIMING_ONLY_PHRASES[t];
    const canon = TIMING_ALIASES[t] ?? t;
    if (EVENT_PHRASES[canon])
        return EVENT_PHRASES[canon];
...
export function negatedTiming(timing) {
...
export function eventClause(event) {
    const e = str(event);
...
function regionMembershipPhrase(p, negated = false) {
    const region = titleWords(p.region_id ?? p.state_ref?.region_id);
    const relation = str(p.relation ?? "within");
    const relationPhrase = relation === "wholly-within" ? "wholly within" : dekebab(relation);
    const subject = p.unit_scope === "whole-unit"
 ...
 */
export function describeSelectionEligibility(c) {
    ...
    if (phrase.startsWith("the unit is "))
        return `that is ${phrase.slice("the unit is ".length)}`;
    if (phrase.startsWith("not the unit is "))
        return `that is not ${phrase.slice("not the unit is ".length)}`;
    if (phrase.startsWith("the unit has "))
        return `with ${phrase.slice("the unit has ".length)}`;
    return `if ${phrase}`;
}
export function describeCondition(c) {
    ...
    switch (c.type) {
        ...
        case "was-hit-by-attack": {
            const subject = p.subject === "target"
                ? "the target"
            ...
            const boundSource = p.source && typeof p.source === "object" && "event_var" in p.source
                ? " from that enemy unit"
                : p.source != null
                    ? ` from ${str(p.source)}`
                    : "";
            const window = p.window === "just-finished-shooting-sequence"
                ? " during its just-finished shooting sequence"
                : " this phase";
            const n = Number(p.count_min ?? 1);
            if (n > 1)
                return `${negate}${subject} was hit by ${n}+ ${atk}attacks${weapon}${boundSource}${window}`;
        ...
        case "battle-round": {
            ...
            const ord = (n) => ["zeroth", "first", "second", "third", "fourth", "fifth"][n] ?? `${n}th`;
            let where;
            if (min != null && max != null)
                where = min === max ? `the ${ord(min)} battle round` : `battle rounds ${min}-${max}`;
            else if (min != null)
                where = `the ${ord(min)} battle round onward`;
            else if (max != null)
                where = `the first ${max} battle rounds`;
            else
