...
function parseWtcHeader(text) {
    ...
    for (const line of lines) {
        ...
        if (detMatch) {
            ...
            if (name !== "—")
                detachment_raw_names.push(name);
            continue;
        }
...
const ATTACHMENT_LINE = /^Attachment:\s*(leader|support)\s*->\s*(.+?)(\s+\[provisional\])?\s*$/i;
...
function applyWithGroup(unit, listText, defaultMultiplier = 1) {
    ...
    for (const keyword of cls.keyword_overrides)
        unit.keyword_overrides.add(keyword);
    ...
    addWargear(unit, cls.wargear.map((w) => ({ raw_name: w.raw_name, count: w.count * multiplier })));
    return cls.wargear;
}
 ...
 */
function applyPendingAttachedTo(units, pendingAttachedTo) {
...
function parseCompactBody(body) {
    ...
    for (const raw of lines) {
        ...
        if (enhMatch && current) {
            attachEnhancement(current, enhMatch[1], enhMatch[2] ? Number.parseInt(enhMatch[2], 10) : null);
        ...
        if (attachmentMatch && current) {
            current.leader_attachment = {
                role: attachmentMatch[1].toLowerCase(),
        ...
        if (breakdown && current) {
            ...
            const groupWargear = breakdown[3] === undefined ? [] : applyWithGroup(current, breakdown[3], count);
...
function parseFullBody(body) {
    ...
    const flushPendingBreakdown = () => {
        if (!current || !pendingBreakdown)
            return;
...
function hasCompactUnit(text) {
    return text.split(/\r?\n/).some((line) => UNIT_HEADER_COMPACT.test(line.trim()));
}
function isSerializedFullFormat(text) {
