...
const DETACHMENT_SOURCE_ALIASES = {
...
function lookupNameKeys(rawName) {
    const keys = new Set();
...
function singularNameKey(rawName) {
...
function mapBattleSize(raw) {
    ...
    if (key.includes("strike force") || key.includes("strikeforce"))
        return "strike-force";
...
export function resolve(parsed, ds, format = "listforge") {
    ...
    if (parsed.faction_raw_name) {
        ...
        if (!directHit) {
            ...
            for (let index = detachmentRawNames.length - 1; index >= 0; index -= 1) {
                if (factionNameCandidates(detachmentRawNames[index]).some((candidate) => ds.factions.find(candidate))) {
        ...
        if (directHit) {
            ...
            if (prefixHit) {
                ...
                const secondaryHit = secondaryFaction &&
                    factionNameCandidates(secondaryFaction[1])
                        .map((candidate) => ds.factions.find(candidate))
                        .find((candidate) => candidate !== undefined);
                if (remainder && !secondaryHit)
                    detachmentRawNames.push(remainder);
            }
            else {
                ...
                if (titleHit) {
                    ...
                    const recoveredDetachment = parsed.faction_raw_name.replace(/\s*\(\d+\s+(?:Detachment Points?|PD)\)\s*$/i, "");
                    if (recoveredDetachment)
                        detachmentRawNames.unshift(recoveredDetachment);
                }
            }
        }
    }
    if (faction_id === null) {
        ...
        for (const unit of parsed.units) {
            const exactFactions = new Set(unitLookupCandidates(unit.raw_name, null, ds)
                .flatMap((candidate) => ds.units.findAll(candidate))
                .filter((candidate) => {
                const key = normalizeName(unit.raw_name);
                return (normalizeName(candidate.name) === key ||
    ...
    const resolveDetachment = (raw_name) => {
    ...
    const coalescedDetachmentNames = [];
    ...
    const detachments = coalescedDetachmentNames.flatMap((raw_name) => {
        ...
        if (parts.length > 1) {
            ...
            if (split.every((d) => d !== null))
                return split;
        }
        if (factionWasInferred)
            return [];
    ...
    if (!force_disposition && parsed.force_disposition_raw_name) {
        const dispositionName = normalizeName(parsed.force_disposition_raw_name) === "recon"
            ? "Reconnaissance"
        ...
        if (hit) {
            ...
            diag.warn("disposition-unresolved", "Force Disposition name did not match any 40kdc disposition.", parsed.force_disposition_raw_name);
        }
    }
    // --- Battle size. ---------------------------------------------------------
    ...
    const enhancementById = (id) => ds.enhancements.getAny(id);
    const detachmentById = (id) => faction_id ? ds.detachments.getInFaction(id, faction_id) : ds.detachments.getAny(id);
    if (detachments.length === 0) {
        const inferredDetachmentIds = new Set(units.flatMap((unit) => {
    ...
    if (force_disposition === null &&
        parsed.force_disposition_raw_name === null &&
        detachmentIds.length > 0) {
        const dispositionIds = new Set(detachmentIds.flatMap((id) => detachmentById(id)?.force_dispositions ?? []));
        const everyDetachmentHasOnlyThatDisposition = detachmentIds.every((id) => detachmentById(id)?.force_dispositions?.length === 1);
    ...
    if (format === "gw" && !units.some((unit) => unit.is_warlord)) {
        ...
        if (firstCharacter >= 0)
            units[firstCharacter].is_warlord = true;
    }
 ...
 */
function unitLookupCandidates(raw_name, faction_id, ds) {
    const candidates = [raw_name];
    const delimitedName = raw_name.split(/\s+--?\s+/).at(-1)?.trim();
    if (delimitedName && delimitedName !== raw_name)
        candidates.push(delimitedName);
    const withoutNickname = raw_name.replace(/\s+(?:["“][^"”]+["”]|'[^']+')\s*$/, "");
    if (withoutNickname !== raw_name)
        candidates.push(withoutNickname);
 ...
 */
function findWeaponCandidates(ds, rawName) {
    for (const variant of sourceNameVariants(rawName)) {
 ...
 */
function scopedWeaponId(ds, hit, rawName) {
    const ids = new Set(hit.raw.weapon_ids ?? []);
    for (const opt of ds.wargearOptionsOf(hit.raw)) {
        for (const id of opt.replaces ?? [])
            ids.add(id);
        for (const id of opt.replacement ?? [])
            ids.add(id);
        for (const group of opt.replacement_choice ?? [])
            for (const id of group)
                ids.add(id);
    }
    const directTargets = new Set([rawName, normalizedSourcePunctuation(rawName)].map(normalizeName));
    for (const id of ids) {
        const weapon = ds.weapons.getInFaction(id, hit.raw.faction_id) ?? ds.weapons.getAny(id);
        if (weapon &&
            [weapon.name, normalizedSourcePunctuation(weapon.name)].some((name) => directTargets.has(normalizeName(name)))) {
            return id;
        }
    }
    ...
    for (const id of ids) {
        ...
        if (sourceNameVariants(w.name).some((variant) => singularTargets.has(singularNameKey(variant)))) {
 ...
 */
function resolveWargearItemId(ds, hit, rawName) {
    const targets = lookupNameKeys(rawName);
    const singularTargets = new Set(sourceNameVariants(rawName).map(singularNameKey));
    if (hit) {
        const ids = new Set();
        ...
        for (const id of ids) {
            ...
            if (!item)
                continue;
            if (sourceNameVariants(item.name).some((variant) => targets.has(normalizeName(variant)))) {
                return id;
            }
            if (sourceNameVariants(item.name).some((variant) => singularTargets.has(singularNameKey(variant)))) {
                singularMatches.push(id);
            }
        }
        if (singularMatches.length === 1)
            return singularMatches[0];
    }
    for (const variant of sourceNameVariants(rawName)) {
...
function resolveUnitAbilityId(ds, hit, rawName) {
    if (!hit)
        return null;
    const targets = lookupNameKeys(rawName);
    for (const id of hit.raw.ability_ids ?? []) {
        const ability = ds.abilities.getInFaction(id, hit.raw.faction_id) ?? ds.abilities.getAny(id);
        if (ability &&
...
function resolveUnit(parsed, faction_id, detachmentIds, ds, diag) {
    ...
    const scopedExact = (q) => {
        ...
        return inFaction.find((u) => normalizeName(u.name) === k || (u.raw.aliases ?? []).some((a) => normalizeName(a) === k));
    };
    ...
    if (hit) {
        const unitModelNames = new Set([singularNameKey(hit.name)]);
        for (const alias of hit.raw.aliases ?? []) {
            unitModelNames.add(singularNameKey(alias));
        }
        const composition = ds.unitCompositionOf(hit.raw);
        const compositionModelNames = new Set();
        for (const model of composition?.models ?? []) {
            if (model.name)
                compositionModelNames.add(singularNameKey(model.name));
            if (model.profile_name)
                compositionModelNames.add(singularNameKey(model.profile_name));
        }
        ...
        const modelLineKeys = (raw) => {
            ...
            if (withoutRole !== raw)
                variants.push(withoutRole);
            const withoutNickname = raw.replace(/\s+(?:["“][^"”]+["”]?|'[^']+'?)\s*$/, "");
            if (withoutNickname !== raw)
                variants.push(withoutNickname);
            return new Set(variants.flatMap(sourceNameVariants).map(singularNameKey));
        };
        const matchesModelName = (raw, modelName) => {
            const keys = modelLineKeys(raw);
            return [...keys].some((key) => key === modelName ||
                (!modelName.includes(" with ") && modelName.endsWith(` ${key}`)));
        };
        const isModelLine = (raw) => {
            const keys = modelLineKeys(raw);
            if ([...unitModelNames, ...compositionModelNames].some((modelName) => keys.has(modelName))) {
                return true;
            }
            return [...compositionModelNames].filter((modelName) => matchesModelName(raw, modelName)).length === 1;
        };
        ...
        if (modelSum > 0) {
            ...
            const parsedCountValid = (composition?.tiers ?? []).some((tier) => {
            ...
            const modelSumMatchesPoints = parsed.points !== null &&
                (hit.raw.points ?? []).some((tier) => tier.cost === parsed.points &&
                    tier.models <= modelSum &&
                    modelSum <= (tier.models_max ?? tier.models));
            const covered = rows.length === 0 ||
                rows.every((model) => (model.min ?? 0) <= 0 ||
                    (!model.name && !model.profile_name) ||
                    modelLines.some((line) => (model.name &&
    ...
    const resolveGearRef = (rawName) => {
        ...
        if (scopedId)
            return resolved(scopedId, rawName);
        const hits = findWeaponCandidates(ds, rawName);
        if (hits[0])
            return resolved(hits[0].id, rawName);
        const wargearItemId = resolveWargearItemId(ds, hit ?? null, rawName);
        if (wargearItemId)
            return resolved(wargearItemId, rawName);
    ...
    let wargear = wargearLines.flatMap((w) => {
        ...
        if (parts.length > 1) {
            ...
            if (partRefs.every((part) => part !== null)) {
    ...
    const explicitGroupRefs = (rawName, count) => {
        const direct = wargear.find((item) => normalizeName(item.ref.raw_name) === normalizeName(rawName));
        if (direct)
            return [{ ref: direct.ref, count }];
        ...
        if (parts.length > 1) {
            ...
            if (refs.every((part) => part !== null)) {
                return refs.map((part) => ({ ref: part, count }));
            }
        }
        return [{ ref: unresolved(rawName, []), count }];
    };
    let loadout_groups = parsed.loadout_groups?.map((group) => ({
        model_name: group.model_name,
        count: group.count,
        wargear: group.wargear.flatMap((item) => explicitGroupRefs(item.raw_name, item.count)),
    })) ?? buildLoadoutGroups(hit, model_count, wargear, ds);
    if (parsed.loadout_groups &&
        loadout_groups &&
    ...
    if (!loadout_groups && hit && wargear.every((item) => item.ref.id !== null)) {
        ...
        for (const item of wargear) {
            explicitCounts.set(item.ref.id, (explicitCounts.get(item.ref.id) ?? 0) + item.count);
        }
        const completed = completeLoadout(hit.raw, model_count, ds.wargearOptionsOf(hit.raw), ds.unitCompositionOf(hit.raw)?.models, explicitCounts);
        if (completed) {
            const refForId = (id) => {
                const existing = explicitRefs.get(id);
                if (existing)
                    return existing;
                const name = hit.weapons.find((weapon) => weapon.id === id)?.name ??
                    ds.wargear.get(id)?.name ??
                    ds.abilities.get(id)?.name ??
                    id;
                return resolved(id, name);
            };
            const remaining = new Map([...completed.counts].map(([id, count]) => [
                id,
                { ref: refForId(id), count },
            ]));
            const seen = new Set();
            wargear = wargear.flatMap((item) => {
                const id = item.ref.id;
    ...
    if (hit && wargear.every((w) => w.ref.id !== null)) {
        ...
        const plausibleCount = rows.length === 0 || (model_count >= envMin && model_count <= envMax);
        if (plausibleCount) {
            const counts = new Map();
            for (const w of wargear)
                counts.set(w.ref.id, (counts.get(w.ref.id) ?? 0) + w.count);
            const violations = checkUnitLegality(hit.raw, model_count, ds.wargearOptionsOf(hit.raw), counts, comp?.models, comp?.tiers).filter((v) => v.code !== "invalid-model-count" && v.code !== "below-min");
            if (violations.length > 0) {
                diag.warn("loadout-illegal", `Loadout is not buildable from the datasheet's wargear options: ${violations
                    .map((v) => `${v.code}:${v.id}`)
                    .join(", ")}`, parsed.raw_name);
            }
        }
    }
 ...
 * decompose exactly.
 */
function buildLoadoutGroups(hit, modelCount, wargear, ds) {
    if (!hit)
        return undefined;
    ...
    for (const w of wargear) {
        if (!w.ref.id)
            return undefined; // incomplete aggregate → can't group faithfully
        refById.set(w.ref.id, w.ref);
        counts.set(w.ref.id, (counts.get(w.ref.id) ?? 0) + w.count);
    }
    const groups = groupLoadout(hit.raw, modelCount, ds.wargearOptionsOf(hit.raw), ds.unitCompositionOf(hit.raw)?.models, counts);
    if (!groups)
        return undefined;
    return groups.map((g) => ({
 ...
 */
function applyLeaderAttachments(parsedUnits, units, ds, factionId, diag) {
    ...
    units.forEach((unit, i) => {
        ...
        unit.leader_attachment = {
            bodyguard_ref: bodyguard.ref.id != null
    ...
    units.forEach((unit, i) => {
        if (parsedUnits[i].leader_attachment != null)
            return; // explicit already applied
        ...
        if (resolvedUnit?.raw.attachment_role !== "support")
            return;
        ...
        diag.warn("leader-attachment-inferred", "Support character attached to an eligible bodyguard (it cannot operate alone); provisional.", unit.ref.raw_name);
    });
}
