...
const BULLET = /^[\t ]*•/mu;
const EMBEDDED_APP_BATTLE_SIZE = /^\s*(?:Combat Patrol|Incursion|Strike Force|Onslaught)\s*\(\s*[\d.,]+\s*Points?\s*\)\s*$/im;
const WTC_COMPACT_UNIT = /^(?:Char\d+:\s*)?\d+x\s+.+?\(\s*\d+\s*pts?\s*\)\s*:/im;
...
function isGwText(decoded) {
    ...
    if (WTC_COMPACT_UNIT.test(decoded))
        return null;
    if (SERIALIZED_WTC.test(decoded))
        return null;
    ...
    if (EMBEDDED_APP_BATTLE_SIZE.test(decoded))
        return null;
    return decoded;
}
...
function finishUnit(acc) {
    ...
    for (let i = 0; i < acc.bullets.length; i += 1) {
        ...
        if (b.count === null) {
            ...
            if (attachedAs) {
                attachedRole = attachedAs[1].toLowerCase();
                if (attachedRole !== "bodyguard")
                    is_character = true;
                continue;
            }
...
function parseBody(lines, bodyStart) {
    ...
    let attachedGroupBuffer = [];
    const resolveAttachedGroup = () => {
        if (attachedGroupBuffer.length === 0)
            return;
    ...
    const finalize = () => {
        if (current) {
            ...
            if (attachedRole)
                attachedGroupBuffer.push({ unit, role: attachedRole });
            units.push(unit);
            current = null;
        }
    };
