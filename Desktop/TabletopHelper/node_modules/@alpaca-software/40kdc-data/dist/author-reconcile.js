 ...
 */
const baseName = (name) => kebab(String(name ?? "").replace(/\([^)]*\)/g, " "));
const matchKey = (detachmentId, name) => `${detachmentId ?? ""}::${baseName(name)}`;
const versionKey = (entry) => `${entry.game_version?.edition ?? ""}::${entry.game_version?.dataslate ?? ""}`;
const scopedKey = (entry, key) => `${versionKey(entry)}::${key}`;
...
function detachmentRuleIds(exactRules, fallbackRules) {
...
export function reconcileFaction(faction, core, abilities, force = false) {
    const report = {
        faction,
    ...
    const byKey = new Map();
    const abilityById = new Map(abilities.map((ability) => [ability.ability_id, ability]));
    const usedAbilityIds = new Set(); // abilities matched to a core entity
    const unitToAbilities = new Map(); // unit_id → ability ids across snapshots
    const detachmentRules = new Map(); // snapshot + detachment_id → rule ability ids
    const allDetachmentRules = new Map(); // detachment_id → rule ability ids across snapshots
    ...
    const linkByType = (entities, type, bucket) => {
    ...
    for (const u of core.units ?? []) {
        ...
        if (want.length === 0)
            continue;
        const have = new Set(u.ability_ids ?? []);
        let added = 0;
    ...
    for (const [unitId, ids] of unitToAbilities) {
        if (!coreUnitIds.has(unitId) && core.units)
            report.units.orphanAbilityUnits.push(`${unitId} (${ids.length} abilities)`);
    }
    ...
    for (const d of core.detachments ?? []) {
        const rules = detachmentRuleIds(detachmentRules.get(scopedKey(d, d.id)), allDetachmentRules.get(d.id));
        if (rules.length === 0) {
        ...
        if (JSON.stringify(d.detachment_rule_ids ?? null) !== JSON.stringify(rules)
        ...
        if (rules.length > 1)
            report.detachments.multiRule.push({ detachment_id: d.id, ability_ids: rules });
    }
    // ── orphan abilities (need a core entity seeded) ─────────────────────
    for (const a of abilities) {
        if (!["stratagem", "enhancement", "detachment"].includes(a.ability_type))
            continue;
        if (usedAbilityIds.has(a.ability_id))
            continue;
        if (a.detachment_id
...
function printReport(r) {
    const L = [`\n══ ${r.faction} ══`];
    L.push(`  stratagems:   +${r.stratagems.linked} linked (${r.stratagems.alreadyLinked} already), ` +
        `${r.stratagems.orphanCore.length} core w/o ability, ${r.stratagems.collisions.length} collisions`);
    L.push(`  enhancements: +${r.enhancements.linked} linked (${r.enhancements.alreadyLinked} already), ` +
    ...
    if (r.stratagems.collisions.length)
        L.push(`    strat collisions: ${r.stratagems.collisions.join(" ; ")}`);
    if (r.enhancements.collisions.length)
        L.push(`    enh collisions: ${r.enhancements.collisions.join(" ; ")}`);
    if (r.detachments.multiRule.length)
        L.push(`    multi-rule: ${r.detachments.multiRule.map((m) => `${m.detachment_id}[${m.ability_ids.join(",")}]`).join(" ; ")}`);
    console.log(L.join("\n"));
}
function main() {
    ...
    for (const faction of factions) {
        ...
        if (!asJson)
            printReport(r);
        if (!dryRun) {
            for (const n of ["stratagems", "enhancements", "units", "detachments"]) {
                if (files[n] && paths[n])
                    writeJSON(paths[n], files[n]);
            }
        }
    }
    if (asJson)
...
if (isMain)
    main();
//# sourceMappingURL=author-reconcile.js.map