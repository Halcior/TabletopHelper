...
import { checkUnitLegality } from "./loadout.js";
 ...
 */
export function resolveRosterUnit(rosterUnit, dataset, factionId) {
 ...
 */
export function checkRosterLegality(roster, dataset) {
    const out = [];
    roster.units.forEach((rosterUnit, unitIndex) => {
        ...
        out.push({
            ...
            violations: checkUnitLegality(view.raw, rosterUnit.model_count, options, counts, composition?.models, composition?.tiers),
        });
    });
    return out;
}
 ...
 */
export function validateRosterCore(spec, dataset) {
    const army = [];
    const push = (severity, code, id, message, unitIndex = null) => army.push({ code, id, message, unitIndex, severity });
    const err = (code, id, message, unitIndex = null) => push("error", code, id, message, unitIndex);
    const resolveUnit = (unitId) => {
        if (!unitId)
            return undefined;
        if (spec.factionId) {
    ...
    const armyKeywords = spec.factionId
        ? (dataset.factions.get(spec.factionId)?.raw.keywords ?? [])
        : [];
    const armyKeywordSet = new Set(armyKeywords);
    const keywordSet = (view, unit) => {
        const owned = new Set([
            ...(view.raw.keywords ?? []),
    ...
    spec.units.forEach((su, idx) => {
        ...
        units.push({
            ...
            violations: checkUnitLegality(view.raw, su.modelCount, options, su.counts, composition?.models, composition?.tiers),
        });
    });
    ...
    const detachments = spec.detachmentIds
        .map((id) => (spec.factionId ? dataset.detachments.getInFaction(id, spec.factionId) : undefined) ??
        dataset.detachments.getAny(id))
        .filter((d) => d !== undefined);
    // --- Enhancements: per-unit eligibility + army-wide uniqueness. -----------
    const enhUses = new Map();
    spec.units.forEach((su, idx) => {
        if (!su.enhancementId)
            return;
    ...
    for (const [enhId, uses] of enhUses) {
        ...
        if (uses > max)
            err("enhancement-over-max-targets", enhId, `${enhId} taken ${uses} times, max ${max}`);
    }
    // --- Leader attachment. ----------------------------------------------------
    spec.units.forEach((su, idx) => {
        const view = views[idx];
        ...
        if (su.leaderBodyguardId) {
            ...
            for (const bodyguardId of enhancement?.attachment_bodyguard_ids ?? [])
                eligible.add(bodyguardId);
    ...
    spec.units.forEach((su, idx) => {
        ...
        if (su.enhancementId)
            total += dataset.enhancements.get(su.enhancementId)?.cost ?? 0;
    });
    ...
    for (const [tag, n] of tagCounts)
        if (n > 1)
            err("detachment-tag-conflict", tag, `${n} detachments share the '${tag}' tag`);
    ...
    const factionKeywords = spec.factionId
        ? new Set(dataset.factions.get(spec.factionId)?.raw.keywords ?? [])
        : new Set();
    if (factionKeywords.size) {
        spec.units.forEach((su, idx) => {
            ...
            if (barred.length)
                err("unit-excluded-from-faction", view.id, `${view.id} cannot be taken by ${spec.factionId} (barred by ${barred.join(", ")})`, idx);
        });
    }
    ...
    if (warlords === 0)
        ...
        err("multiple-warlords", "roster", `army has ${warlords} warlords`);
    // --- Unit minimums (e.g. Houndpack: 3+ WAR DOG units). --------------------
    for (const d of detachments) {
        for (const um of d.unit_minimums ?? []) {
            const count = views.filter((view, index) => view !== undefined && keywordSet(view, spec.units[index]).has(um.keyword)).length;
            if (count < um.min)
 ...
 */
export function checkRoster(roster, dataset) {
    const spec = {
