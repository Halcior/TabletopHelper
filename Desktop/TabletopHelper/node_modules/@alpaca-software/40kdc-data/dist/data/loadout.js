 ...
 */
export function optionCap(option, modelCount, models) {
    ...
    if (c.any_number)
        ...
        cap = c.max_count ?? 1;
    if (!c.any_number && c.max_count != null)
    ...
    if (c.model_name && models?.length) {
        ...
        if (eligible != null)
            cap = Math.min(cap, eligible * perModel);
    }
 ...
 */
function eligibleModelCount(models, modelCount, name) {
...
function allReplacedIds(options) {
    const out = new Set();
    for (const o of options)
...
function hasRecordedDefaults(models) {
    return (!!models && models.length > 0 && models.every((m) => (m.default_weapon_ids?.length ?? 0) > 0));
}
/**
 ...
 */
function baseCounts(unit, modelCount, options, models) {
 ...
 */
export function baseLoadout(unit, modelCount, options, models) {
 ...
 */
export function maximalLoadout(unit, modelCount, options, models) {
 ...
 */
function clampFlatBudgets(unit, counts) {
    for (const budget of unit.wargear_budgets ?? []) {
        if (budget.items.length !== 1 || budget.per_models !== 0)
            continue;
        const id = budget.items[0];
        const cur = counts.get(id);
        if (cur != null && cur > budget.count)
            counts.set(id, budget.count);
    }
}
/**
 ...
 */
export function weaponBounds(unit, modelCount, options, models) {
 ...
 */
function assignRowCounts(models, modelCount, counts) {
    const rowDefaults = models.map((m) => toMultiset(m.default_weapon_ids ?? []));
    const rowsWith = new Map();
    for (const def of rowDefaults)
        for (const id of def.keys())
            rowsWith.set(id, (rowsWith.get(id) ?? 0) + 1);
    const minOf = (i) => Math.max(0, models[i].min ?? 0);
    const maxOf = (i) => Math.max(minOf(i), models[i].max ?? minOf(i));
    ...
    const headroom = (i) => maxOf(i) - out[i];
    while (budget > 0) {
        let pick = -1;
 ...
 */
function candidateRowCounts(models, modelCount, counts) {
    ...
    const suffixMin = Array(models.length + 1).fill(0);
    const suffixMax = Array(models.length + 1).fill(0);
    for (let i = models.length - 1; i >= 0; i--) {
        suffixMin[i] = suffixMin[i + 1] + mins[i];
        suffixMax[i] = suffixMax[i + 1] + maxs[i];
    }
    const current = Array(models.length).fill(0);
    const visit = (i, remaining) => {
        if (i === models.length) {
            if (remaining === 0)
                add([...current]);
            return;
...
function candidateCanBeSelected(candidate, upper, optionCaps) {
    for (const [id, per] of candidate.weapons) {
        if (per > 0 && (upper.get(id) ?? 0) < per)
            return false;
    }
...
function enumerateRowCandidates(base, rowName, options) {
    ...
    for (let head = 0; head < queue.length; head++) {
        ...
        for (const oi of applicable) {
            ...
            let uses = 0;
            for (const used of cur.used)
                if (used === oi)
                    uses++;
            const perModelLimit = replaces.length === 0 ? option.model_constraint?.max_count ?? 1 : 1;
 ...
 */
function solveAssignment(rows, lower, upper, optionCaps) {
    ...
    const assignRow = (ri) => {
        if (ri === rows.length) {
            for (const c of remainingLower.values())
                if (c > 0)
                    return false;
            return true;
        }
...
function groupsFromSolution(solution) {
...
function optionsWithPrintedUnitAbilities(unit, options, counts) {
...
export function groupLoadout(unit, modelCount, options, models, counts) {
    ...
    for (const rowN of candidateRowCounts(models, n, bag)) {
        ...
        const optionCaps = effectiveOptions.map((option) => optionCap(option, n, fixedModels));
        const rows = [];
        for (let i = 0; i < models.length; i++) {
            const count = rowN[i];
            if (count <= 0)
            ...
            const candidates = enumerateRowCandidates(base, models[i].name ?? null, effectiveOptions)
 ...
 */
export function completeLoadout(unit, modelCount, options, models, explicitCounts) {
    const n = Math.max(0, Math.floor(modelCount) || 0);
    if (n === 0 || !hasRecordedDefaults(models))
        return null;
    const strictLower = new Map();
    for (const [id, count] of explicitCounts) {
        if (count > 0)
            strictLower.set(id, count);
    }
    ...
    const defaultIds = new Set(models.flatMap((model) => model.default_weapon_ids ?? []));
    const repeatedCoItems = new Set();
    for (const option of options) {
        ...
        for (const [id, count] of occurrences) {
            if (count >= 2 && !defaultIds.has(id))
                repeatedCoItems.add(id);
        }
    }
    const relaxedLower = new Map(strictLower);
    for (const id of repeatedCoItems)
        relaxedLower.delete(id);
    if (relaxedLower.size !== strictLower.size)
        lowerVariants.push(relaxedLower);
    const effectiveOptions = optionsWithPrintedUnitAbilities(unit, options, explicitCounts);
    for (const lower of lowerVariants) {
        for (const rowCounts of candidateRowCounts(models, n, lower)) {
            ...
            for (let index = 0; index < fixedModels.length; index++) {
                ...
                const candidates = enumerateRowCandidates(base, fixedModels[index].name ?? null, effectiveOptions)
                    .filter((candidate) => candidateCanBeSelected(candidate, upper, optionCaps))
                    .sort((a, b) => a.usedOptions.length - b.usedOptions.length ||
            ...
            for (const group of groups) {
                for (const weapon of group.weapons) {
                    counts.set(weapon.id, (counts.get(weapon.id) ?? 0) + weapon.count * group.count);
                }
            }
...
export function validateLoadout(unit, modelCount, options, counts, models) {
 ...
 */
function budgetViolations(unit, modelCount, counts) {
    const out = [];
    for (const budget of unit.wargear_budgets ?? []) {
 ...
 */
export function checkUnitLegality(unit, modelCount, options, counts, models, tiers) {
    if (!tiers?.length) {
    ...
    for (const tier of tiers) {
        ...
        if (modelCount >= min && modelCount <= max)
            candidates.push(tm);
    }
 ...
 */
function swapConflicts(unit, modelCount, options, counts, models) {
    ...
    for (const base of baseIds) {
        const cleanAdds = new Set();
        let messy = false;
        for (const o of options) {
            const replaces = o.replaces ?? [];
            if (!replaces.includes(base))
                continue;
        ...
        if (messy || cleanAdds.size === 0 || (addedBy.get(base) ?? 0) > 0)
            continue;
        ...
        for (const b of cleanAdds)
            total += counts.get(b) ?? 0;
        if (total > cap) {
            out.push({
                id: base,
                code: "swap-conflict",
