...
export class UnitView {
     ...
     */
    get weapons() {
        return resolveAll(this.raw.weapon_ids, (id) => this.ds.weapons.getInFaction(id, this.raw.faction_id) ?? this.ds.weapons.getAny(id));
    }
    /** Abilities referenced by `ability_ids`; unresolved ids are skipped. */
    get abilities() {
        return resolveAll(this.raw.ability_ids, 
...
export class AbilityView {
     ...
     */
    resolveRulesBundles(effect, seen = new Set([this.id])) {
        ...
        if (node.type === "ability-grant" && node.modifier !== null && typeof node.modifier === "object") {
            ...
            if (modifier.rules_bundle === true && typeof abilityId === "string" && !seen.has(abilityId)) {
                ...
                const target = (factionId ? this.ds.abilities.getInFaction(abilityId, factionId) : undefined) ??
                    this.ds.abilities.getAny(abilityId);
                const targetEffect = target?.raw.effect;
                if (targetEffect !== null &&
                    typeof targetEffect === "object" &&
                    targetEffect.type === "rules-bundle") {
    ...
    describeBuffs(source, context, perspective = "attacker") {
        ...
        const translated = effectToBuffs(this.resolveRulesBundles(this.raw.effect), source, ctx, perspective);
        ...
        const range = this.raw.scope?.range_inches;
