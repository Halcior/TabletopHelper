...
const titleCase = (id) => id.split("-").map((w) => (w.length <= 2 ? w : w[0].toUpperCase() + w.slice(1))).join(" ");
function loadManifestFacts(manifestPaths, faction) {
    const byId = new Map();
    for (const f of manifestPaths) {
        const recs = readJSON(f);
        if (!Array.isArray(recs))
            continue;
        for (const r of recs) {
            if (r.faction && r.faction !== faction)
                continue;
 ...
 * nothing.
 */
export function seedCore(faction, missing, facts, existing) {
    const out = { detachments: [], stratagems: [], enhancements: [] };
    const report = { faction, detachmentsCreated: [], stratagemsCreated: [], enhancementsCreated: [], skipped: [] };
    ...
    const ensureDetachment = (detId, f) => {
        if (!detId || detachmentsSeen.has(detId))
            return;
    ...
    for (const m of missing) {
        const f = facts.get(m.ability_id) ?? {};
        ...
        if (m.ability_type === "stratagem") {
            ensureDetachment(m.detachment_id, f);
            if (existing.stratagems.has(m.ability_id))
                continue;
            ...
            const missingFacts = [];
            if (typeof f.cp_cost !== "number")
                missingFacts.push("cp_cost");
            if (!phases)
                missingFacts.push("phases");
            if (!f.timing)
                missingFacts.push("timing");
            if (!playerTurn)
                missingFacts.push("player_turn");
        ...
        if (m.ability_type === "enhancement") {
            ensureDetachment(m.detachment_id, f);
            if (existing.enhancements.has(m.ability_id))
                continue;
            if (!m.detachment_id) {
...
function main() {
        ...
        if (existsSync(p)) {
            if (statSync(p).isDirectory())
                for (const f of readdirSync(p).filter((f) => f.endsWith(".json")))
                    manifestPaths.push(resolve(p, f));
            else
    ...
    for (const faction of factions) {
        ...
        const existing = {
            stratagems: new Set((coreFiles.stratagems ?? []).map((s) => s.id)),
            enhancements: new Set((coreFiles.enhancements ?? []).map((e) => e.id)),
            detachments: new Set((coreFiles.detachments ?? []).map((d) => d.id)),
        };
        ...
        if (seeded.report.detachmentsCreated.length)
            console.log(`    detachments: ${seeded.report.detachmentsCreated.join(", ")}`);
        if (seeded.report.skipped.length) {
            const byReason = {};
            for (const s of seeded.report.skipped)
...
if (isMain)
    main();
//# sourceMappingURL=author-seed-core.js.map