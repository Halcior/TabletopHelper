...
function classifyGrouping(unit, roster, ds) {
    if (unit.loadout_groups)
        return "grouped";
    const view = unit.ref.id ? resolveRosterUnit(unit, ds, roster.faction_id) : undefined;
    if (!view)
        return "unit-unresolved";
    if (unit.wargear.some((w) => w.ref.id === null))
        return "wargear-unresolved";
    if (unit.model_count <= 1)
        return "single-model";
    const models = ds.unitCompositionOf(view.raw)?.models;
    const recorded = !!models && models.length > 0 && models.every((m) => (m.default_weapon_ids?.length ?? 0) > 0);
    if (!recorded)
        return "no-recorded-defaults";
    return "solver-null";
}
function unresolvedRef(kind, ref, unitIndex) {
    if (ref.id !== null)
        return [];
    return [{ kind, ...(unitIndex === undefined ? {} : { unit_index: unitIndex }) }];
}
function collectUnresolved(roster) {
    const out = [];
    if (roster.faction_id === null)
        out.push({ kind: "faction" });
    for (const detachment of roster.detachments)
        out.push(...unresolvedRef("detachment", detachment.ref));
    roster.units.forEach((unit, unitIndex) => {
        out.push(...unresolvedRef("unit", unit.ref, unitIndex));
        if (unit.enhancement)
            out.push(...unresolvedRef("enhancement", unit.enhancement, unitIndex));
        for (const item of unit.wargear)
            out.push(...unresolvedRef("wargear", item.ref, unitIndex));
...
function semanticWargear(items) {
...
function semanticBattleSize(roster) {
    if (roster.battle_size !== null)
        return roster.battle_size;
    const limit = roster.points.declared_limit ??
...
function semanticRoster(roster) {
    return {
        name: roster.name,
        ...
        units: roster.units.map((u) => ({
            ...
            loadout_groups: u.loadout_groups
                ?.map((g) => ({
                ...
                a.count - b.count ||
                JSON.stringify(a.wargear).localeCompare(JSON.stringify(b.wargear))) ?? null,
...
function auditRoundtrips(roster, ds) {
...
function ownerForWarning(warning) {
    if (warning.code === "loadout-illegal")
        return "legality/loadout algorithm";
    return "source-format parsing/resolution";
}
function sanitizeLegality(legality) {
    return {
...
function rowForFile(file, ordinal, ds) {
    ...
    for (const warning of roster.diagnostics.warnings) {
        if (warning.code === "leader-attachment-inferred")
            continue;
        failures.push({
...
export function auditWtcCorpus(corpusRoot) {
    ...
    return {
        ...
        summary: {
            ...
            warning_failures: parsedRows.reduce((sum, row) => sum + row.parse_warnings.filter((warning) => warning.code !== "leader-attachment-inferred").length, 0),
            unresolved_refs: parsedRows.reduce((sum, row) => sum + row.unresolved_refs.length, 0),
            army_legality_errors: parsedRows.reduce((sum, row) => sum + row.army_legality.filter((violation) => violation.severity === "error").length, 0),
            unit_legality_errors: parsedRows.reduce((sum, row) => sum + row.unit_legality.reduce((n, unit) => n + unit.violations.length, 0), 0),
            solver_failures: parsedRows.reduce((sum, row) => sum + row.grouping.filter((group) => group.outcome === "solver-null").length, 0),
            nondeterministic_exports: parsedRows.reduce((sum, row) => sum + row.roundtrips.filter((result) => !result.deterministic).length, 0),
            semantic_roundtrip_differences: parsedRows.reduce((sum, row) => sum + row.roundtrips.reduce((n, result) => n + result.differences.length, 0), 0),
...
if (isMain) {
    ...
    if (!corpusRoot || !outPath || outIndex !== 1 || args.length !== 3)
        usage();
    ...
    if (report.summary.files !== 368 ||
        report.summary.parsed !== 368 ||
        report.summary.failed_lists !== 0) {
        process.exitCode = 1;
    }
}
//# sourceMappingURL=audit-wtc.js.map