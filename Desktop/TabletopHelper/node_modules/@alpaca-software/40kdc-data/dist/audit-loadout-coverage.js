...
function unitIdSet(path, key) {
    const out = new Set();
    if (!existsSync(path))
        return out;
    for (const row of readJSON(path)) {
        const id = row[key];
        if (typeof id === "string")
            out.add(id);
    }
...
export function auditLoadoutCoverage(opts = {}) {
    ...
    for (const faction of listFactionDirs(coreDir)) {
        ...
        for (const u of readJSON(unitsPath)) {
            if (u.is_legend)
                continue;
            if ((u.weapon_ids?.length ?? 0) > 0)
                continue; // has a loadout → not a skeleton
            const missing = ["weapons"];
            if (!wgoUnits.has(u.id))
                missing.push("wargear-options");
            if (!compUnits.has(u.id))
                missing.push("composition");
            if ((u.ability_ids?.length ?? 0) === 0)
...
function renderMarkdown(report) {
    ...
    lines.push("");
    lines.push("Generated by `tools/src/audit-loadout-coverage.ts` (`npm run audit:loadout-coverage`).");
    lines.push("Lists non-Legends core units that carry no `weapon_ids` — MFM `seed-units` skeletons");
    lines.push("the `wargear`/`composition-tiers` reconcile passes never back-filled. Some entries are");
    lines.push("intentionally weaponless (terrain, tokens); the rest want a loadout-authoring follow-up");
...
if (isMain) {
    ...
    if (!dryRun) {
        writeFileSync(join(auditDir, "loadout-coverage.json"), JSON.stringify(report, null, 2) + "\n");
...
//# sourceMappingURL=audit-loadout-coverage.js.map