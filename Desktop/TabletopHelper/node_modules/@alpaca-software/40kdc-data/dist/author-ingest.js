...
function newStub(rec, abilityId) {
    ...
    if (rec.faction_id)
        stub.faction_id = rec.faction_id;
    if (rec.detachment_id)
        stub.detachment_id = rec.detachment_id;
    return stub;
}
/**
 ...
 */
export function ingestFaction(faction, records, existingAbilities, existingInput) {
    const abilities = existingAbilities.map((a) => ({ ...a, unit_ids: [...(a.unit_ids ?? [])] }));
    const byId = new Map(abilities.map((a) => [a.ability_id, a]));
    const inputById = new Map(existingInput.map((e) => [e.ability_id, e]));
    const result = {
    ...
    for (const rec of records) {
        ...
        if (entry) {
            if (rec.detachment_id && entry.detachment_id == null)
                entry.detachment_id = rec.detachment_id;
            for (const u of rec.unit_ids ?? []) {
                if (!entry.unit_ids.includes(u)) {
                    entry.unit_ids.push(u);
                    result.mergedUnits++;
                    if (!hasEmptyModifier(entry.effect))
                        result.mergedIntoAuthored.push({ ability_id: id, unit_id: u });
                }
            }
        }
        else {
...
export function ingestSnapshot(manifest, existingAbilities, existingInput) {
    ...
    const retained = existingAbilities.flatMap((ability) => {
        const unitIds = (ability.unit_ids ?? []).filter((id) => !coveredUnits.has(id));
        const coveredOwner = (ability.unit_ids ?? []).some((id) => coveredUnits.has(id));
    ...
    const result = ingestFaction(scope.faction_id, records, retained, retainedInput);
    ...
    for (const record of records) {
        ...
        if (record.detachment_id)
            ability.detachment_id = record.detachment_id;
        else
            delete ability.detachment_id;
...
function ingestRecordId(record) {
    return record.ability_id ?? kebab(record.name);
}
function hasCanonicalCoveredDetachment(record, coveredDetachments) {
    if (!DETACHMENT_ABILITY_TYPES.has(record.ability_type))
        return false;
    for (const detachmentId of coveredDetachments) {
        if (record.ability_id.endsWith(`-${detachmentId}`))
            return true;
    }
    return false;
}
export function projectPhaseMappings(existing, manifest, deletedAbilityIds, activeAbilityIds) {
    const replacedIds = new Set([
        ...manifest.records.map(ingestRecordId),
        ...deletedAbilityIds,
    ]);
    const projected = existing.filter((mapping) => mapping.source_type !== "ability" ||
...
export function projectRawTextRecords(existing, incoming, manifest) {
    ...
    for (const record of existing) {
        ...
        if (!incomingIds.has(record.ability_id) && (coveredDetachment || canonicalCoveredDetachment || (coveredOwner && remainingUnits.length === 0)))
            continue;
 ...
 */
function ensureStoreRepo() {
    if (existsSync(resolve(RAW_TEXT_STORE, ".jj")) || existsSync(resolve(RAW_TEXT_STORE, ".git")))
        return;
    try {
 ...
 * ownership scope.
 */
export function mergeRawTextRecords(existing, incoming) {
    const merged = new Map(existing.map((e) => [e.ability_id, e]));
    for (const r of incoming)
        merged.set(r.ability_id, r); // updates in place; new ids append
    return Array.from(merged.values());
}
function writeRawTextStore(records, snapshots) {
    mkdirSync(RAW_TEXT_STORE, { recursive: true });
    ensureStoreRepo();
    const readmePath = resolve(RAW_TEXT_STORE, "README.md");
    if (!existsSync(readmePath))
        writeFileSync(readmePath, STORE_README);
    const byFaction = new Map();
    for (const record of records) {
        const factionRecords = byFaction.get(record.faction_id);
        if (factionRecords)
            factionRecords.push(record);
        else
            byFaction.set(record.faction_id, [record]);
    }
    ...
    for (const file of readdirSync(RAW_TEXT_STORE)) {
        if (!file.endsWith(".json") || file.startsWith("bundle-") || file === "index.json")
            continue;
        let entries;
        try {
        ...
        for (const entry of entries) {
            ...
            if (entry.ability_type === "stratagem" && (entry.when || entry.effect)) {
                const indexed = { faction: owner, when: entry.when ?? "", target: entry.target ?? "", effect: entry.effect ?? "" };
                if (entry.restrictions)
                    indexed.restrictions = entry.restrictions;
...
function main() {
    ...
    for (const input of inputs) {
        ...
        if (!existsSync(p)) {
            console.error(`Not found: ${input} — skipping.`);
            continue;
        }
        if (statSync(p).isDirectory()) {
            for (const f of readdirSync(p).filter((f) => f.endsWith(".json")).sort())
                manifestFiles.push(resolve(p, f));
        }
        else {
    ...
    for (const f of manifestFiles) {
        ...
        if (parsed?.records && parsed?.replace_scope && Array.isArray(parsed.records)) {
            ...
            if (snapshots.has(snapshot.replace_scope.faction_id))
                throw new Error(`Multiple snapshots for ${snapshot.replace_scope.faction_id}`);
    ...
    for (const r of records) {
        if (!r.faction) {
            console.error(`Record "${r.name}" has no faction — skipping.`);
            continue;
        }
    ...
    for (const [faction, recs] of byFaction) {
        ...
        const existingAbilities = existsSync(abilitiesPath) ? readJSON(abilitiesPath) : [];
        const inputPath = resolve(INPUT_DIR, `${faction}.json`);
        const existingInput = existsSync(inputPath) ? readJSON(inputPath) : [];
        const snapshot = snapshots.get(faction);
        const r = snapshot
        ...
        console.log(`  ${faction}: ${recs.length} records → +${r.created} stubs, ${r.mergedUnits} unit links merged, ` +
        ...
        if (snapshot) {
            ...
            const existingMappings = existsSync(phasePath) ? readJSON(phasePath) : [];
            const projectedMappings = projectPhaseMappings(existingMappings, snapshot, r.deletedAbilityIds, new Set(r.abilities.map((ability) => ability.ability_id)));
            if (!dryRun)
                writeFileSync(phasePath, JSON.stringify(projectedMappings, null, 2) + "\n");
        }
        if (!dryRun) {
    ...
    if (!dryRun)
        writeRawTextStore(allRawText, snapshots);
...
if (isMain)
    main();
//# sourceMappingURL=author-ingest.js.map