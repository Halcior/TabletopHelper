...
function factionForCat(file, repoDirs) {
    const base = file.replace(/\.(cat|gst)$/i, "");
    const dir = CAT_TO_FACTION[base];
    return dir && repoDirs.has(dir) ? dir : null;
}
function loadCatalogs(repoDirs) {
    const entryById = new Map();
    const groupById = new Map();
    const catById = new Map(); // catalogue id → root node
    const parsed = [];
    ...
    for (const file of files) {
        ...
        for (const e of collectByKey(doc, "selectionEntry"))
            if (e.id)
                entryById.set(String(e.id), e);
        for (const sharedE of collectByKey(doc, "sharedSelectionEntries")) {
            for (const e of asArray(sharedE.selectionEntry))
                if (e.id)
                    entryById.set(String(e.id), e);
        }
        for (const g of collectByKey(doc, "selectionEntryGroup"))
            if (g.id)
        ...
        if (!root)
            continue;
        if (root.id)
            catById.set(String(root.id), root);
    ...
    for (const [faction, roots] of catByFaction) {
        const seen = new Set();
        catByFaction.set(faction, roots.filter((r) => {
...
function entryName(node, cats) {
    if (node.name && String(node.type) !== "selectionEntryGroup")
        return cleanText(String(node.name));
    if (node.targetId) {
        const t = cats.entryById.get(String(node.targetId)) ?? cats.groupById.get(String(node.targetId));
        if (t?.name)
            return cleanText(String(t.name));
    }
    return node.name ? cleanText(String(node.name)) : undefined;
}
/** The "pts" cost on an entry, or 0. */
function ptsCost(node) {
    for (const c of asArray(node.costs?.cost)) {
        if (String(c.name).toLowerCase() === "pts")
            return parseInt(String(c.value), 10) || 0;
    }
    return 0;
}
function constraintVal(node, type) {
...
function nodeConstraint(node, type, cats) {
    ...
    if (node.targetId) {
        ...
        if (t)
            return constraintVal(t, type);
    }
    return undefined;
}
/** An `<association max>` cap (e.g. a 1-per-unit icon/banner), if present. */
function associationMax(node) {
    for (const a of asArray(node.associations?.association)) {
...
function childGroups(node) {
    return asArray(node.selectionEntryGroups?.selectionEntryGroup);
}
function childEntries(node) {
    return asArray(node.selectionEntries?.selectionEntry);
}
function entryLinksOf(node) {
    return asArray(node.entryLinks?.entryLink);
}
/** All `type="model"` selectionEntries beneath a unit (models nest in groups). */
function modelsOf(unit) {
...
function directModelChildren(group) {
...
const NOISE_GROUP_TARGET_IDS = new Set([
...
const NOISE_GROUP_NAME_RE = /crusade|modifications|battle (tallies|honours?|scars?|traits?)|relic|enhancement|warlord|requisition|\bagenda|abilit|abilt|\bpowers?\b|discipline|reanimation system|translocation system|command system|codex /i;
// Names that must never be minted as a wargear item even if profiled — they are
// ability/power selectors, not equipment.
const NOISE_ITEM_NAME_RE = /\bpowers?\b|abilit|abilt|discipline|warlord|requisition|protocol|trademark weapon/i;
/** Display name of a group node (following an entryLink to a shared group). */
function groupName(node, cats) {
    if (node.name)
        return cleanText(String(node.name));
    if (node.targetId) {
...
function isNoiseGroup(node, cats) {
    if (node.targetId && NOISE_GROUP_TARGET_IDS.has(String(node.targetId)))
        return true;
...
function isHidden(node, cats) {
    if (String(node.hidden) === "true")
        return true;
    if (node.targetId) {
        const t = cats.entryById.get(String(node.targetId)) ?? cats.groupById.get(String(node.targetId));
        if (t && String(t.hidden) === "true")
            return true;
    }
    return false;
}
/** A group node directly, or the shared group an entryLink points to. */
function resolveGroup(node, cats) {
...
function hasEquipIdentically(unit) {
    for (const m of collectByKey(unit, "modifier")) {
        if (String(m.field) === "error" && /equipped identically/i.test(String(m.value ?? "")))
            return true;
    }
    return false;
}
/** Distinct model "base" names (stripping a "w/ <weapon>" loadout suffix). */
function distinctModelBases(unit) {
    const bases = new Set();
    for (const m of modelsOf(unit)) {
 ...
 */
function perNFromDecrement(node, unitMax) {
    const max = constraintVal(node, "max");
    if (!max || max < 1 || unitMax <= 1)
        return null;
    for (const mod of asArray(node.modifiers?.modifier)) {
        if (String(mod.type) !== "decrement")
            continue;
        for (const c of asArray(mod.conditions?.condition)) {
...
function profileType(node, cats) {
    const host = node.targetId ? cats.entryById.get(String(node.targetId)) ?? node : node;
    const profiles = asArray(host.profiles?.profile);
    if (profiles.some((p) => /Ranged Weapons|Melee Weapons/i.test(String(p.typeName ?? ""))))
        return "weapon";
    if (profiles.some((p) => /Abilities|Wargear/i.test(String(p.typeName ?? ""))))
        return "wargear";
    return null;
}
export function extractFaction(faction, cats) {
    const res = {
        faction,
        options: [],
        wargear: [],
    ...
    const weaponIds = new Set((existsSync(weaponsPath) ? readJSON(weaponsPath) : []).map((w) => w.id));
    const wargearIds = new Set((existsSync(wargearPath) ? readJSON(wargearPath) : []).map((w) => w.id));
    for (const id of wargearIds)
        weaponIds.add(id);
    const repoUnitIds = new Set((existsSync(unitsPath) ? readJSON(unitsPath) : []).map((u) => u.id));
    ...
    const resolveOption = (node, name) => {
        ...
        for (const link of collectByKey(host, "entryLink")) {
            if (String(link.type) !== "selectionEntry")
                continue;
        ...
        for (const p of asArray(host.profiles?.profile)) {
            if (!/Ranged Weapons|Melee Weapons/i.test(String(p.typeName ?? "")))
                continue;
    ...
    const modelWeaponSet = (model) => {
        const ids = new Set();
        for (const got of resolveOption(model, cleanText(String(model.name ?? ""))) ?? [])
            ids.add(got);
        return ids;
    };
    ...
    const nestedGroups = (model) => {
        const out = [];
        for (const g of childGroups(model)) {
            if (directModelChildren(g).length === 0 && !isNoiseGroup(g, cats))
                out.push(g);
        }
    ...
    const groupChoices = (group) => {
        ...
        for (const e of [...childEntries(group), ...entryLinksOf(group)]) {
            if (String(e.type) === "selectionEntryGroup" || isHidden(e, cats))
                continue;
            const n = entryName(e, cats);
            if (!n || NOISE_ITEM_NAME_RE.test(n))
                continue;
    ...
    const modelFixedSet = (model) => {
        const ids = new Set(resolveOption(model, cleanText(String(model.name ?? ""))) ?? []);
        for (const g of nestedGroups(model)) {
            ...
            for (const e of [...childEntries(g), ...entryLinksOf(g)]) {
                if (String(e.type) === "selectionEntryGroup")
                    continue;
                const n = entryName(e, cats);
                const eids = n ? resolveOption(e, n) : null;
                if (!eids)
                    continue;
                const isDefault = !!defaultId && (String(e.id) === defaultId || String(e.targetId) === defaultId);
                if (!isDefault)
    ...
    const modelChoiceSlots = (model) => {
        const slots = [];
        for (const g of nestedGroups(model)) {
            if (g.defaultSelectionEntryId)
                continue; // defaulted → folded into the fixed set
            if ((constraintVal(g, "min") ?? 0) < 1)
                continue; // optional add-on, not a slot
    ...
    const isDatasheet = (e) => {
        ...
        if (t === "unit")
            return true;
        if (t === "model") {
            return asArray(e.profiles?.profile).some((p) => /^Unit$/i.test(String(p.typeName ?? "")));
        }
        return false;
    };
    ...
    for (const unit of units) {
        ...
        const report = (context, reason) => res.reports.push({ unit: unitName, context, reason });
        ...
        const constraintFor = (node, modelName, groupMax, modelMax) => {
            const mc = {};
            if (nameModels && modelName)
        ...
        for (const g of collectByKey(unit, "selectionEntryGroup")) {
            if (g.defaultSelectionEntryId)
                defaultModelIds.add(String(g.defaultSelectionEntryId));
        }
        const unitBaseSet = new Set();
        for (const m of modelsOf(unit)) {
            if (defaultModelIds.has(String(m.id)))
                for (const id of modelWeaponSet(m))
                    unitBaseSet.add(id);
        }
        ...
        for (const m of ownModels) {
            ...
            if (!defaultModelIds.has(id) && !variantModelIds.has(id))
                maxSquadModels += constraintVal(m, "max") ?? 1;
        }
        if (maxSquadModels === 0)
            maxSquadModels = maxModels;
        for (const group of collectByKey(unit, "selectionEntryGroup")) {
            ...
            if (new Set(names).size < variants.length)
                continue;
            for (const v of variants)
                consumed.add(String(v.id));
            ...
            for (const s of sets) {
                ...
                if (removed.length === 1 && added.length === 1) {
                    const slot = swapSlots.get(removed[0]) ?? { alts: new Set(), max: undefined };
                    slot.alts.add(added[0]);
                    if (s.max !== undefined)
                        slot.max = Math.max(slot.max ?? 0, s.max);
            ...
            const slotOptions = [];
            const knownAdds = new Set(addonAlts);
            for (const slot of swapSlots.values())
                for (const a of slot.alts)
                    knownAdds.add(a);
            for (const s of leftover) {
                ...
                if (added.length === 0)
                    continue; // introduces no new weapon — nothing lost
                if (added.every((id) => knownAdds.has(id)))
                    continue; // redundant cross-product
                report(ctx, `model-variant "${cleanText(String(s.node.name ?? ""))}" introduces weapons not ` +
            ...
            for (const [removed, slot] of swapSlots) {
                ...
                const opt = { replaces: [removed] };
                if (alts.length === 1)
                    opt.replacement = alts;
                else
                    opt.replacement_choice = alts.map((a) => [a]);
                const mc = capFor(slot.max);
                if (mc)
                    opt.model_constraint = mc;
                pushOption(opt, 0);
            }
            if (addonAlts.size > 0) {
                ...
                if (alts.length === 1)
                    ...
                    opt.replacement_choice = alts.map((a) => [a]);
                if (perModel)
                    opt.model_constraint = { any_number: true };
                pushOption(opt, 0);
            }
            ...
            for (const so of slotOptions) {
                const opt = {};
                if (so.replaces)
                    opt.replaces = so.replaces;
                if (so.choices.length === 1)
                    opt.replacement = so.choices[0];
                else
                    opt.replacement_choice = so.choices;
        ...
        for (const model of ownModels) {
            if (consumed.has(String(model.id)))
                continue;
            ...
            const visit = (node) => {
                ...
                for (const e of childEntries(node))
                    if (String(e.type) !== "model")
                        visit(e);
            };
            visit(model);
            for (const group of slotGroups) {
                if (consumed.has(String(group.id)))
                    continue;
                ...
                for (const e of childEntries(group)) {
                    ...
                    const n = entryName(e, cats);
                    if (n && !NOISE_ITEM_NAME_RE.test(n))
                        opts.push({ node: e, name: n });
                }
                for (const link of entryLinksOf(group)) {
                    if (String(link.type) !== "selectionEntry" || isHidden(link, cats))
                        continue;
                    const n = entryName(link, cats);
                    if (n && !NOISE_ITEM_NAME_RE.test(n))
                        opts.push({ node: link, name: n });
                }
                if (opts.length === 0)
                    continue;
                for (const o of opts)
                    consumed.add(String(o.node.id));
                const resolved = [];
                let bad = false;
                for (const o of opts) {
                    ...
                    const isDefault = (defaultId && (String(o.node.id) === defaultId || String(o.node.targetId) === defaultId)) || false;
                    resolved.push({
                        ids,
                        pts: ptsCost(o.node),
                        isDefault,
                ...
                if (defaultId) {
                    ...
                    const opt = { replaces: defaults[0].ids };
                    if (alternatives.length === 1)
                        opt.replacement = alternatives[0].ids;
                    else
                        opt.replacement_choice = alternatives.map((a) => a.ids);
        ...
        for (const host of addonHosts) {
            if (directModelChildren(host).length >= 2)
                continue; // variant group, handled
            const candidates = [...entryLinksOf(host), ...childEntries(host)];
            for (const node of candidates) {
                const id = String(node.id ?? "");
                if (!id || consumed.has(id) || seenAddon.has(id))
                    continue;
                if (String(node.type) === "model" || isHidden(node, cats))
                    continue;
                if (String(node.type) === "selectionEntryGroup")
                    continue;
                ...
                if (min >= 1 || max < 1)
                    continue; // base loadout or not selectable
                const pt = profileType(node, cats);
                if (!pt)
                    continue;
                const name = entryName(node, cats);
                if (!name || NOISE_ITEM_NAME_RE.test(name))
                    continue;
                ...
                const mc = max < maxModels ? { max_count: max } : perModel ? { any_number: true } : undefined;
                const opt = { replacement: ids };
                if (mc)
                    opt.model_constraint = mc;
        ...
        for (const m of ownModels) {
            if (defaultModelIds.has(String(m.id)))
                for (const id of modelFixedSet(m))
                    squadIds.add(id);
        }
        if (squadIds.size === 0)
            for (const id of unitBaseSet)
                squadIds.add(id);
        if (squadIds.size === 0 && ownModels.length === 1) {
            for (const id of modelFixedSet(ownModels[0]))
                squadIds.add(id);
        }
...
// ─────────────────────────── CLI ───────────────────────────
function repoDirSet() {
    return new Set(readdirSync(CORE_DIR, { withFileTypes: true })
        .filter((d) => d.isDirectory() && !d.name.startsWith("_"))
        .map((d) => d.name));
}
/** Mint-merge clean wargear entities into a faction's existing wargear.json. */
function mergeWargear(faction, minted) {
    if (minted.length === 0)
        return;
 ...
 */
function mergeCompDefaults(faction, compDefaults) {
    if (compDefaults.length === 0)
        return;
    const cpath = join(CORE_DIR, faction, "unit-compositions.json");
    if (!existsSync(cpath))
        return;
    const comps = readJSON(cpath);
    ...
    const weaponIdsByUnit = new Map((existsSync(upath) ? readJSON(upath) : []).map((u) => [
        u.id,
        u.weapon_ids ?? [],
    ]));
    const reachableByUnit = new Map();
    for (const o of existsSync(wpath) ? readJSON(wpath) : []) {
        const set = reachableByUnit.get(o.unit_id) ?? new Set();
    ...
    for (const comp of comps) {
        ...
        const idsFor = (m) => cd.byModel[normModelName(String(m.name ?? ""))] ?? cd.squadDefault;
        ...
        for (const m of models)
            for (const id of idsFor(m))
                covered.add(id);
        const reachable = reachableByUnit.get(String(comp.unit_id)) ?? new Set();
        const orphanWeapons = (weaponIdsByUnit.get(String(comp.unit_id)) ?? []).filter((id) => !reachable.has(id));
        if (!orphanWeapons.every((id) => covered.has(id)))
            continue; // would regress → skip
        for (const m of models) {
            const ids = idsFor(m);
            if (!ids.length)
                continue;
    ...
    if (changed)
        writeFileSync(cpath, JSON.stringify(comps, null, 2) + "\n");
}
function writeReport(faction, r) {
    ...
    writeFileSync(path, JSON.stringify({ faction, matchedUnits: r.matchedUnits, skippedUnits: r.skippedUnits, reports: r.reports }, null, 2) + "\n");
}
function main() {
    ...
    for (const faction of factions) {
        ...
        console.log(`${faction}: ${r.options.length} options (+${r.wargear.length} wargear) from ` +
            `${r.matchedUnits} matched units (skipped ${r.skippedUnits.length} unmatched, ` +
            `${r.reports.length} reported for triage)`);
        if (r.matchedUnits > 0 || r.reports.length > 0)
            writeReport(faction, r);
...
main();
//# sourceMappingURL=convert-bsdata-wargear.js.map