...
import { runWargear, buildWargearReport, runWargearBudgets, runWargearCosts, runCompositionNames, runCompositionTiers, } from "./mfm/wargear.js";
...
import { runConditionalKeywords, buildConditionalKeywordReport, } from "./mfm/conditional-keywords.js";
import { applyWrites } from "./mfm/apply.js";
import { writeGolden } from "./mfm/golden.js";
import { mergeMode, modeOfPublication, } from "./mfm/game-mode.js";
const REPORT_DIR = path.join(CORE_DIR, "_reports");
const UNMATCHED_DIR = path.join(REPO_ROOT, "_private", "mfm");
function repoIds(dir, file) {
    return new Set(readJsonArray(path.join(CORE_DIR, dir, file)).map((e) => e.id));
}
/** Bucket dump datasheets / detachments / enhancements by their resolved repo dir. */
function bucketByDir(rows, factionKeywordOf, dump) {
    const out = new Map();
    for (const row of rows) {
        ...
        if (!dir)
            continue; // titans / unmapped — surfaced separately
...
function detIdsByDir(dump) {
    const byDir = bucketByDir(dump.table("detachment"), (d) => dump.factionKeywordOfDetachment(d.id), dump);
    const out = new Map();
    for (const [dir, rows] of byDir) {
        const m = new Map();
...
function enhIdsByDir(dump) {
    const byDir = bucketByDir(dump.table("enhancement"), (e) => dump.factionKeywordOfDetachment(e.detachmentId), dump);
    const out = new Map();
    for (const [dir, rows] of byDir) {
        const m = new Map();
...
export function coverage(dump) {
    ...
    for (const ds of liveDatasheets) {
        ...
        if (id)
            globalUnitIds.add(id);
    }
    const globalDetIds = new Set();
    for (const det of dump.table("detachment")) {
        const id = slug(dump.enName(det));
        if (id)
            globalDetIds.add(id);
    }
    const globalEnhIds = new Set();
    ...
    for (const d of liveDatasheets) {
        ...
        const fkName = fkId ? dump.enName(dump.byId("faction_keyword").get(fkId)) : undefined;
        if (fkName && !repoDirForFactionName(fkName))
            unmapped.add(fkName);
    }
    ...
    for (const dir of [...repoDirs()].sort()) {
        const dumpUnitIds = unitsByDir.get(dir) ?? new Map();
...
export function wholeDatasetCounts(dump) {
    return {
        stratagems: {
            repo: readJsonArray(path.join(CORE_DIR, "stratagems.json")).length,
            dump: dump.table("stratagem").length,
        },
        missions: {
            repo: readJsonArray(path.join(CORE_DIR, "missions.json")).length,
...
function buildReport(dump, cov) {
    const { dirs, unmappedFactions } = cov;
    const sum = (f) => dirs.reduce((a, d) => a + f(d), 0);
    const L = [];
    L.push(`# MFM coverage — dump data_version ${dump.version ?? "?"}`);
    L.push("");
    L.push("Dump-vs-repo coverage by faction dir. **New** = in dump, no repo entity.");
    L.push("**Repo-only** = in repo, absent from the (Legends-free) dump → dropped (Legends/Forge-World; see cull-legends).");
    L.push("");
    L.push("| Faction dir | Units matched | Units new | Units shared-skip | Units repo-only | Det matched | Det new | Det repo-only | Enh matched | Enh new | Enh repo-only |");
    L.push("|---|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|");
    for (const d of dirs) {
        L.push(`| ${d.dir} | ${d.unitsMatched} | ${d.unitsNew.length} | ${d.unitsSharedSkipped} | ${d.unitsRepoOnly.length} | ${d.detMatched} | ${d.detNew.length} | ${d.detRepoOnly.length} | ${d.enhMatched} | ${d.enhNew.length} | ${d.enhRepoOnly.length} |`);
    }
    L.push(`| **TOTAL** | **${sum((d) => d.unitsMatched)}** | **${sum((d) => d.unitsNew.length)}** | **${sum((d) => d.unitsSharedSkipped)}** | **${sum((d) => d.unitsRepoOnly.length)}** | **${sum((d) => d.detMatched)}** | **${sum((d) => d.detNew.length)}** | **${sum((d) => d.detRepoOnly.length)}** | **${sum((d) => d.enhMatched)}** | **${sum((d) => d.enhNew.length)}** | **${sum((d) => d.enhRepoOnly.length)}** |`);
    L.push("");
    ...
    L.push(`| Missions | ${wdc.missions.repo} | ${wdc.missions.dumpPrimary} primary + ${wdc.missions.dumpSecondary} secondary |`);
    L.push(`| Force dispositions | ${wdc.forceDispositions.repo} | ${wdc.forceDispositions.dump} |`);
    L.push(`| Detachment→disposition map | — | ${wdc.detachmentDispositionMap} (1:1) |`);
    L.push("");
    ...
    for (const d of dirs) {
        ...
        const block = (title, items) => {
            ...
            if (items.length > 100)
                L.push(`- …and ${items.length - 100} more`);
        };
...
function runCoverage(dump) {
    ...
    fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-coverage.json"), JSON.stringify({ unmappedFactions: cov.unmappedFactions, dirs: unmatched }, null, 2) + "\n");
    const t = (f) => cov.dirs.reduce((a, d) => a + f(d), 0);
    console.log(`Coverage report → ${path.relative(REPO_ROOT, reportPath)}`);
    console.log(`Units matched ${t((d) => d.unitsMatched)}, new ${t((d) => d.unitsNew.length)}, repo-only ${t((d) => d.unitsRepoOnly.length)} (Legends/FW).`);
    console.log(`Detachments matched ${t((d) => d.detMatched)}, new ${t((d) => d.detNew.length)}. ` +
...
async function runDispositionsCmd(dump, write, includeCombatPatrol = false) {
    ...
    fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
    fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-dispositions.json"), JSON.stringify({
    ...
    const sum = (f) => report.dirs.reduce((a, d) => a + f(d), 0);
    console.log(`Dispositions report → ${path.relative(REPO_ROOT, reportPath)}`);
    console.log(`Matched ${sum((d) => d.matched)}, DP changed ${sum((d) => d.dpChanged.length)}, ` +
...
async function runEnhancementsCmd(dump, write, includeCombatPatrol = false) {
    ...
    fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
    fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-enhancements.json"), JSON.stringify({
        newInDump: report.newInDump,
        combatPatrolExcluded: report.cpExcluded,
    ...
    console.log(`Matched ${sum((d) => d.matched)}, seeded ${report.seeded.length}, ` +
...
async function runNormalizeEnhancementsCmd(dump, write) {
    ...
    const renameLines = Object.entries(report.renames)
        ...
        .map(([o, n]) => `| \`${o}\` | \`${n}\` |`);
    const md = `# MFM enhancement RAW-name normalization\n\n` +
    ...
    fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
    fs.writeFileSync(path.join(UNMATCHED_DIR, "enh-renames.json"), JSON.stringify(report.renames, null, 2) + "\n");
    console.log(`Normalize-enhancements report → ${path.relative(REPO_ROOT, reportPath)}`);
    console.log(`Id renames ${Object.keys(report.renames).length}, name changes ${report.nameChanges.length}, ` +
...
async function runFactionFieldsCmd(dump, write) {
    ...
    fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
    fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-faction-fields.json"), JSON.stringify({
    ...
    const n = (f) => report.dirs.reduce((a, d) => a + (f(d) ? 1 : 0), 0);
    console.log(`Faction fields report → ${path.relative(REPO_ROOT, reportPath)}`);
    console.log(`rule filled ${n((d) => d.ruleFilled)}, parent filled ${n((d) => d.parentFilled)}, ` +
...
async function runDetachmentFieldsCmd(dump, write) {
    ...
    fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
    fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-detachment-fields.json"), JSON.stringify({
    ...
    console.log(`tags changed ${sum((d) => d.tagsChanged.length)} (confirm ${sum((d) => d.tagsConfirmed)}), ` +
 ...
 */
export function mergePointsAndCompositionTierWrites(pointWrites, compositionTierWrites) {
    ...
    for (const tierWrite of compositionTierWrites) {
        ...
        if (!tierWrite.path.endsWith("/units.json") ||
            !Array.isArray(pointWrite.value) ||
            !Array.isArray(tierWrite.value)) {
            throw new Error(`points-and-composition-tiers unexpectedly overlap at ${tierWrite.path}`);
        }
        const tierUnits = new Map(tierWrite.value
        ...
        merged.set(tierWrite.path, {
            ...
            value: pointWrite.value.map((unit) => {
                if (!unit || typeof unit !== "object")
                    return unit;
                const tierUnit = tierUnits.get(unit.id);
...
async function runPointsCmd(dump, write, compositionTiers) {
    ...
    fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
    fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-points.json"), JSON.stringify({
        newInDump: report.newInDump,
    ...
    console.log(`Matched ${sum((d) => d.matched)}, points changed ${sum((d) => d.pointsChanged.length)}, ` +
    ...
    await applyWrites(compositionTiers
        ? mergePointsAndCompositionTierWrites(report.staged, compositionTiers.staged)
        : report.staged, { write, label: compositionTiers ? "points-and-composition-tiers" : "points" });
    if (!write)
        console.log("DRY RUN — no files written. Re-run with --write to apply.");
}
async function runPointsAndCompositionTiersCmd(dump, write) {
    const report = runCompositionTiers(dump);
    const sum = (f) => report.dirs.reduce((a, d) => a + f(d), 0);
    console.log(`Composition tiers — matched ${sum((d) => d.matched)}, units tiered ${sum((d) => d.unitsTiered)}, ` +
    ...
    if (report.skipped.length) {
        fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
        fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-composition-tiers.json"), JSON.stringify(report.skipped, null, 2) + "\n");
    }
    await runPointsCmd(dump, write, report);
}
async function runCullCmd(dump, write) {
    ...
    fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
    fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-cull-legends.json"), JSON.stringify({
    ...
    const sum = (f) => report.dirs.reduce((a, d) => a + f(d), 0);
    console.log(`Dropped ${report.totalDropped} units; pruned wargear-options ${sum((d) => d.wargearOptionsRemoved)}, ` +
    ...
    if (susp)
        console.log(`⚠ ${susp} possible name-match bug(s) flagged for review (see report).`);
    await applyWrites(report.staged, { write, label: "cull-legends" });
    if (!write)
        console.log("DRY RUN — no files written. Re-run with --write to apply.");
}
async function runStratagemsCmd(dump, write) {
    ...
    console.log(`Matched ${sum((d) => d.matched)}, cp applied ${sum((d) => d.cpChanged.length)}, ` +
...
async function runSeedStratagemsCmd(dump, write, includeCombatPatrol = false) {
    ...
    const md = `# MFM stratagem seeding (unseeded competitive set)\n\n` +
    ...
    console.log(`Seeded ${report.seeded.length}, held back ${report.heldBackCombatPatrol.length} Combat-Patrol, ` +
    ...
    if (write && report.seeded.length) {
        console.log(`Follow-through: run \`npx tsx src/mfm-backfill-store.ts --write\` to push the seeded ` +
...
async function runMissionsCmd(dump, write) {
    ...
    if (report.shapeMismatch.length || report.exclusiveReview.length || report.repoOnly.length) {
        fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
        fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-missions.json"), JSON.stringify({
    ...
    console.log(`Matched ${report.matched}, cards changed ${report.cardsChanged}, ` +
    ...
    console.log(`Mission entities: matched ${e.matched}, source filled ${e.sourceFilled.length} ` +
...
async function runMissionMatchupsCmd(dump, write) {
    ...
    if (report.repoOnly.length || report.unresolvedDump.length) {
        fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
        fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-mission-matchups.json"), JSON.stringify({ repoOnly: report.repoOnly, unresolvedDump: report.unresolvedDump }, null, 2) + "\n");
    }
    console.log(`Mission-matchups report → ${path.relative(REPO_ROOT, reportPath)}`);
    console.log(`Matched ${report.matched}, seeded ${report.seeded.length}, corrected ${report.corrected.length}, ` +
...
async function runBaseSizesCmd(dump, write) {
    ...
    if (report.review.length) {
        fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
        fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-base-sizes.json"), JSON.stringify({ review: report.review }, null, 2) + "\n");
    }
    console.log(`Base-sizes report → ${path.relative(REPO_ROOT, reportPath)}`);
    console.log(`Filled ${report.filled.length}, de-drafted ${report.dedrafted.length}, ` +
...
async function runChapterScopeCmd(dump, write) {
    ...
    if (report.repoOnly.length || report.dumpOnly.length) {
        fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
        fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-chapter-scope.json"), JSON.stringify({ repoOnly: report.repoOnly, dumpOnly: report.dumpOnly }, null, 2) + "\n");
    }
    console.log(`Chapter-scope report → ${path.relative(REPO_ROOT, reportPath)}`);
    console.log(`Matched ${report.matched}, faction_keywords collapsed ${report.factionKeywordsChanged.length}, ` +
...
async function runWargearCmd(dump, write, onlyDir) {
    ...
    console.log(`Matched ${sum((d) => d.matched)}, options ${sum((d) => d.optionsChanged)}, ` +
...
async function runWargearBudgetsCmd(dump, write, onlyDir) {
    const report = runWargearBudgets(dump, onlyDir);
    const sum = (f) => report.dirs.reduce((a, d) => a + f(d), 0);
    console.log(`Wargear budgets — matched ${sum((d) => d.matched)}, ` +
...
async function runWargearCostsCmd(dump, write, onlyDir) {
    const report = runWargearCosts(dump, onlyDir);
    const sum = (f) => report.dirs.reduce((a, d) => a + f(d), 0);
    console.log(`Wargear costs — matched ${sum((d) => d.matched)}, ` +
...
async function runCompositionNamesCmd(dump, write, onlyDir) {
    const report = runCompositionNames(dump, onlyDir);
    const sum = (f) => report.dirs.reduce((a, d) => a + f(d), 0);
    console.log(`Composition names — matched ${sum((d) => d.matched)}, rows renamed ${sum((d) => d.rowsRenamed)}, ` +
    ...
    if (report.skipped.length) {
        fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
        fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-composition-names.json"), JSON.stringify(report.skipped, null, 2) + "\n");
    }
    await applyWrites(report.staged, { write, label: "composition-names" });
    if (!write)
        console.log("DRY RUN — no files written. Re-run with --write to apply.");
}
async function runCompositionTiersCmd(dump, write, onlyDir) {
...
async function runAttachmentRoleCmd(dump, write, onlyDir) {
    ...
    console.log(`Attachment roles — matched ${sum((d) => d.matched)}, roles Δ ${sum((d) => d.rolesChanged)}, ` +
...
async function runSeedUnitsCmd(dump, write, onlyDir, includeCombatPatrol = false) {
    ...
    if (skipped || cpExcluded) {
        fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
        fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-seed-units.json"), JSON.stringify({
            skipped: report.dirs.flatMap((d) => d.skipped.map((s) => ({ dir: d.dir, ...s }))),
            combatPatrolExcluded: report.dirs.flatMap((d) => d.cpExcluded.map((c) => ({ dir: d.routedTo ?? d.dir, ...c }))),
        }, null, 2) + "\n");
    }
    console.log(`Seed-units report → ${path.relative(REPO_ROOT, reportPath)}`);
    console.log(`Seed-units — created ${created} skeleton unit(s), ` +
...
async function runSeedDetachmentsCmd(dump, write, onlyDir, includeCombatPatrol = false) {
...
async function runAlliesCmd(dump, write) {
    ...
    console.log(`Pools emitted ${report.rules.length}, skipped ${report.skipped.length}` +
...
async function runWeaponVariantsCmd(dump, write, onlyDir) {
    ...
    console.log(`Conflicting ids ${sum((d) => d.conflictingIds)}, variants +${sum((d) => d.variantsAdded)}, ` +
...
async function runConditionalKeywordsCmd(dump, write, onlyDir) {
    ...
    fs.mkdirSync(UNMATCHED_DIR, { recursive: true });
    fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-conditional-keywords.json"), JSON.stringify({ unsupported: report.unsupported, unresolved: report.unresolved }, null, 2) + "\n");
    console.log(`Conditional-keyword report → ${path.relative(REPO_ROOT, reportPath)}`);
    console.log(`Changed ${report.dirs.reduce((sum, entry) => sum + entry.changed, 0)} units; ` +
...
export async function runIngestMfmCommand(command, options) {
    ...
    if (command === "coverage")
        ...
        await runPointsCmd(dump, options.write);
    else if (command === "cull-legends")
        await runCullCmd(dump, options.write);
        ...
        await runMissionsCmd(dump, options.write);
    else if (command === "mission-matchups")
        await runMissionMatchupsCmd(dump, options.write);
    else if (command === "base-sizes")
        await runBaseSizesCmd(dump, options.write);
        ...
        await runSeedUnitsCmd(dump, options.write, options.onlyDir, options.includeCombatPatrol);
    else if (command === "seed-detachments")
        await runSeedDetachmentsCmd(dump, options.write, options.onlyDir, options.includeCombatPatrol);
    else if (command === "allies")
        await runAlliesCmd(dump, options.write);
...
async function main() {
    ...
    if (!INGEST_MFM_COMMANDS.includes(command)) {
        console.error(`Usage: ingest-mfm <${INGEST_MFM_COMMANDS.join("|")}> [--write] [--dump <path>] [--dir <faction>] [--include-combat-patrol]`);
        process.exit(2);
    }
    await runIngestMfmCommand(command, {
...
//# sourceMappingURL=ingest-mfm.js.map