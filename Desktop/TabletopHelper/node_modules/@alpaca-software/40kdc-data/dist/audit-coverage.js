 ...
 */
function coverageDigest(r) {
    const rows = [...r.factions].sort((a, b) => b.stubStructural - a.stubStructural ||
        b.stub - a.stub ||
    ...
    for (const f of rows) {
        lines.push(`| ${f.faction} | ${f.stubStructural} | ${f.stub} | ${f.gwTextLeak} | ${f.defensiveSkipped} |`);
    }
    const t = r.totals;
    lines.push(`| **TOTAL** | **${t.stubStructural}** | **${t.stub}** | **${t.gwTextLeak}** | **${t.defensiveSkipped}** |`, "", "`stub*` = empty-modifier placeholder nodes (highest need first) · `notes-stub` = flagged in " +
        ...
        "Full total+repair breakdown: [`data/_audit/summary.md`](data/_audit/summary.md).", COVERAGE_END);
    return lines.join("\n");
}
/**
 ...
 */
function injectCoverageIntoContributing(report) {
    let md;
    try {
    ...
    if (next !== md)
        writeFileSync(CONTRIBUTING_PATH, next);
}
