...
function refCoordinate(rp, k, where) {
    const axis = axisOfEdge(k.edge);
    if (k.ref.kind === "vertex") {
        const v = rp.vertices[k.ref.index];
        if (!v) {
            throw new TerrainKeystoneError(`${where}: keystone vertex index ${k.ref.index} out of range (${rp.vertices.length} vertices)`);
        }
        return v[axis];
    }
    const sideAxis = k.ref.side === "min-x" || k.ref.side === "max-x" ? "x" : "y";
    if (sideAxis !== axis) {
        throw new TerrainKeystoneError(`${where}: face "${k.ref.side}" cannot be measured from the ${k.edge} edge (axis mismatch)`);
    }
 ...
 */
export function keystoneMeasurements(layout, templates, board = BOARD_INCHES) {
...
//# sourceMappingURL=keystones.js.map