...
import { resolveLayout, polygonCentroid, } from "./terrain/resolve.js";
...
const r4 = (v) => ({ x: round4(v.x), y: round4(v.y) });
...
const rotAboutCenter = (v, deg) => {
    const d = rotateCw({ x: v.x - CENTER.x, y: v.y - CENTER.y }, deg);
    return { x: d.x + CENTER.x, y: d.y + CENTER.y };
};
const reflectX = (v) => ({ x: 2 * CENTER.x - v.x, y: v.y }); // about x=18
const reflectY = (v) => ({ x: v.x, y: 2 * CENTER.y - v.y }); // about y=18
/** Build a piece authored directly in board coordinates: footprint = board − centroid, position = centroid. */
function boardPolygonPiece(base, boardVerts) {
    const c = polygonCentroid(boardVerts);
    const footprint = { type: "polygon", points: boardVerts.map((v) => r4({ x: v.x - c.x, y: v.y - c.y })) };
...
function arenaArc(a0deg) {
    const a0 = a0deg * DEG;
    const half = ARC_THETA / 2;
    const pt = (r, a) => ({ x: CENTER.x + r * Math.cos(a), y: CENTER.y + r * Math.sin(a) });
    const verts = [];
    for (let i = 0; i <= ARC_STEPS; i++)
        verts.push(pt(ARC_RO, a0 - half + (ARC_THETA * i) / ARC_STEPS));
    for (let i = ARC_STEPS; i >= 0; i--)
        verts.push(pt(ARC_RI, a0 - half + (ARC_THETA * i) / ARC_STEPS));
    return verts;
}
/**
 ...
 */
function lWall(vertex, legA, legB, dx, dy) {
...
function objectivePiece(id, name, pos, role) {
    return {
        id,
        name,
        piece_type: "area",
...
function buildPieces() {
    ...
    const innerBaseCenter = { x: INNER_NEAR + INNER_LEG / 2, y: INNER_OFF + INNER_LEG / 2 };
    const innerBaseVertex = { x: innerBaseCenter.x + INNER_LEG / 2, y: innerBaseCenter.y + INNER_LEG / 2 };
    const innerBaseWall = lWall(innerBaseVertex, INNER_LEG, INNER_LEG, 1, 1);
    const ks = (edge, index) => ({ edge, ref: { kind: "vertex", index } });
    ...
    const deployBaseVertex = { x: DEPLOY_X + DEPLOY_LONG / 2, y: DEPLOY_BASE_OFF };
    const deployBaseWall = lWall(deployBaseVertex, DEPLOY_LONG, DEPLOY_SHORT, 1, 1);
    const deployBaseCenter = { x: DEPLOY_X, y: DEPLOY_BASE_OFF - DEPLOY_SHORT / 2 };
...
function main() {
    ...
    if (!layout)
        throw new Error(`layout "${LAYOUT_ID}" not found`);
    for (const id of ["kotc-ruin-inner", "kotc-ruin-deployment", "impassable-wall"]) {
        if (!catalog.some((t) => t.id === id))
            throw new Error(`catalog missing template "${id}"`);
    }
    ...
    const probe = { id: layout.id, name: layout.name, pieces };
    ...
    for (const p of pieces) {
        if (!p.footprint || p.footprint.type !== "polygon" || p.parent_area_id)
            continue;
    ...
    check(`arena outer radius ≈ ${ARC_RO}`, outerRadii.every((r) => Math.abs(r - ARC_RO) < 1e-3));
    const innerArea = byId.get("ruin-inner-nw");
    check("inner NW (west) ruin near corner 8″ off the left edge & 13″ off the top", innerArea.vertices.some((v) => Math.abs(v.x - 8) < 1e-3 && Math.abs(v.y - 13) < 1e-3));
    const deployWall = byId.get("ruin-deploy-nw-walls");
    check("deployment NW baseline wall at y=5.5", deployWall.vertices.some((v) => Math.abs(v.y - 5.5) < 1e-3));
...
main();
//# sourceMappingURL=build-kotc-colosseum.js.map