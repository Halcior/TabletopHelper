...
const EXPORT_ONLY_TEXT_FORMATS = [
...
const EXPORT_ONLY_DATASET_FORMATS = [{ format: "yellowscribe", goldenName: "expected.yellowscribe.ros" }];
function genRosters() {
...
function runLinkedQuery(ds, q) {
    switch (q.query) {
        ...
        case "logo_url_of_faction": {
            const f = ds.factions.get(q.args.factionId);
            if (!f)
                throw new Error(`logo_url_of_faction: unknown faction ${q.args.factionId}`);
        ...
        case "base_loadout": {
            ...
            const u = ds.units.getAny(q.args.unitId);
            if (!u)
                throw new Error(`base_loadout: unknown unit ${q.args.unitId}`);
            const comp = ds.unitCompositions.find((c) => c.unit_id === q.args.unitId);
        ...
        case "triggers_for_event":
            return (ds.triggerIndex().get(q.args.event) ?? []).map((rt) => rt.abilityId);
    }
}
/**
                ...
                ops: [
                    {
                        ...
                        round: 2,
                        asserted: awardsOf(ds.missionCards.get("battlefield-dominance")).map((aw, index) => aw.vp_per != null ? { index, count: aw.per_max ?? 3 } : { index }),
                        roundCap: 15,
                        gameCap: 45,
                    },
...
function genEffectTranslation() {
    ...
    for (const a of abilities) {
        ...
        if (a.raw.usage != null)
            entry.usage = a.raw.usage;
        if (a.raw.trigger != null)
            entry.trigger = a.raw.trigger;
        entry.expected = {
    ...
    // "Before the first battle round, …" phrasing pinned cross-impl.
    {
        ...
        cases.push({
            ...
            scope: fc.scope,
            expected: {
                text: describeAbility({ effect: fc.effect, scope: fc.scope }),
            },
        });
    }
