...
const GAME_EVENTS = (() => {
    ...
    return new Set(common.$defs["game-event"].enum);
})();
...
const TIMING_ALIAS = { "before-this-model-removed": "before-bearer-removed" };
const toEvent = (timing) => {
    const ev = TIMING_ALIAS[timing] ?? timing;
    if (!GAME_EVENTS.has(ev))
        throw new Error(`timing '${timing}' -> '${ev}' is not a game-event enum member`);
    return ev;
};
const abilities = readJSON(ABILITIES);
const byId = new Map();
for (const a of abilities)
    if (typeof a.ability_id === "string")
        byId.set(a.ability_id, a);
...
function resolveTargets(sourceId) {
...
const storeIds = (() => {
    ...
    return new Set(store.map((e) => e.ability_id).filter(Boolean));
})();
function runWorklist() {
    const tf = readJSON(TIMING_FLAGS);
    const worklist = [];
    const dangling = [];
    const fanouts = [];
    let resolvedRows = 0;
    for (const r of tf) {
        ...
        if (targets.length === 0) {
            dangling.push(`${r.source_id} (${r.source_type}, ${r.timing})`);
            continue;
        }
        if (targets.length > 1)
            fanouts.push(`${r.source_id} -> ${targets.length}: ${targets.join(", ")}`);
        resolvedRows++;
    ...
    for (const f of fanouts.slice(0, 40))
        console.log(`     - ${f}`);
    console.log(`  with raw-text for depth authoring: ${worklist.filter((w) => w.raw_text_present).length}/${worklist.length}`);
    if (resolvedRows + dangling.length !== tf.length)
        throw new Error("row accounting mismatch — resolved + dangling != total");
...
function validateTrigger(t, id) {
    if (typeof t !== "object" || t === null)
        throw new Error(`${id}: trigger not an object`);
    const keys = Object.keys(t);
    const allowed = new Set(["event", "subject", "proximity", "condition", "optional", "cost", "window"]);
    for (const k of keys)
        if (!allowed.has(k))
            throw new Error(`${id}: trigger has unknown key '${k}'`);
    if (typeof t.event !== "string" || !GAME_EVENTS.has(t.event))
        throw new Error(`${id}: trigger.event invalid: ${t.event}`);
    if (t.subject != null && !SUBJECTS.has(t.subject))
        throw new Error(`${id}: trigger.subject invalid: ${t.subject}`);
    if (t.optional != null && typeof t.optional !== "boolean")
        throw new Error(`${id}: trigger.optional not boolean`);
    if (t.proximity != null) {
        const p = t.proximity;
        if (typeof p !== "object" || typeof p.range !== "number" || p.range <= 0)
            throw new Error(`${id}: trigger.proximity.range invalid`);
        if (p.of != null && !PROX_OF.has(p.of))
            throw new Error(`${id}: trigger.proximity.of invalid: ${p.of}`);
        for (const k of Object.keys(p))
            if (k !== "of" && k !== "range")
                throw new Error(`${id}: trigger.proximity unknown key '${k}'`);
    }
}
function runApply() {
    const authoredPath = join(OUT, "authored-triggers.json");
    if (!existsSync(authoredPath))
        throw new Error(`missing ${authoredPath} — run the authoring workflow first`);
    ...
    const tf = readJSON(TIMING_FLAGS);
    for (const r of tf)
        for (const id of resolveTargets(r.source_id))
            expected.add(id);
    const missing = [...expected].filter((id) => !(id in authored));
    if (missing.length)
        throw new Error(`authored-triggers.json missing ${missing.length} ids: ${missing.slice(0, 10).join(", ")}`);
    let attached = 0;
    const collisions = [];
    for (const [id, trigger] of Object.entries(authored)) {
        const a = byId.get(id);
        if (!a)
            throw new Error(`authored id has no ability: ${id}`);
    ...
    if (collisions.length)
        throw new Error(`refusing to overwrite existing triggers on: ${collisions.join(", ")}`);
...
if (APPLY)
    ...
    runWorklist();
//# sourceMappingURL=migrate-timing-flags.js.map