...
import { footprintVertices, resolveLayout, } from "./terrain/resolve.js";
...
function battlemasterPlateCorners(templateId) {
...
function nearestVertexIndices(vertices, targets) {
    const indices = [];
 ...
 * corners.
 */
export function cardinalCornerIndices(footprint, templateId) {
    const vertices = footprintVertices(footprint);
    if (vertices.length <= 4)
        return vertices.map((_, index) => index);
    const plateCorners = templateId ? battlemasterPlateCorners(templateId) : null;
    if (plateCorners)
        return nearestVertexIndices(vertices, plateCorners);
    let minX = Infinity;
    let maxX = -Infinity;
    let minY = Infinity;
    let maxY = -Infinity;
...
function pieceFootprint(piece, templates) {
    if (piece.footprint)
        return piece.footprint;
    const template = piece.template ? templates.get(piece.template) : undefined;
    if (!template) {
        throw new Error(`${piece.id ?? "piece"}: no footprint for keystone derivation`);
    }
    return template.footprint;
}
function templatesById(templates) {
...
function keystonesForVertex(v, index) {
    return [
        {
...
export function isAxisAligned(piece) {
...
function deriveForPiece(rp, axisAligned, cornerIndices) {
    const c = centroid(rp);
    const corner = {
    ...
    if (axisAligned)
        return keystonesForVertex(a, aIndex);
    let bIndex = aIndex;
    ...
    const bKeystone = Math.abs(b.x - a.x) >= Math.abs(b.y - a.y)
        ? {
...
function explicitResolved(layout, templates) {
    ...
    for (const piece of pieces) {
        ...
        if (!rp)
            throw new Error(`${layout.id}: resolved emission shorter than layout.pieces`);
        out.push(rp);
        cursor += 1;
...
function mirrorOntoTwin(k, primary, twin) {
 ...
 */
export function clearNonCardinalKeystones(layouts, templates) {
    const byTemplate = templatesById(templates);
    let piecesCleared = 0;
    for (const layout of layouts) {
        if (!layout.id.startsWith("bm-"))
            continue;
        for (const piece of layout.pieces ?? []) {
            const corners = new Set(cardinalCornerIndices(pieceFootprint(piece, byTemplate), piece.template));
            const hasNonCardinalAnchor = (piece.keystones ?? []).some((keystone) => keystone.ref.kind === "vertex" && !corners.has(keystone.ref.index));
            if (!hasNonCardinalAnchor)
                continue;
...
export function authorKeystones(layouts, templates) {
    const byTemplate = templatesById(templates);
    ...
    for (const layout of layouts) {
        ...
        for (let i = 0; i < pieces.length; i++) {
            ...
            for (let j = 0; j < pieces.length; j++) {
                ...
                if (Math.abs(cj.x - r.x) <= TWIN_CENTROID_TOLERANCE_IN &&
        ...
        for (let i = 0; i < pieces.length; i++) {
            ...
            if (piece.keystones && piece.keystones.length > 0)
                continue;
            const corners = cardinalCornerIndices(pieceFootprint(piece, byTemplate), piece.template);
            const derived = deriveForPiece(resolved[i], isAxisAligned(piece), corners);
            ...
            if (twinPiece.keystones && twinPiece.keystones.length > 0)
                continue;
            const mirrored = derived.map((k) => mirrorOntoTwin(k, resolved[i], resolved[j]));
            if (mirrored.every((k) => k !== null)) {
...
export function keystonePairingViolations(layouts, templates) {
    const violations = [];
    for (const layout of layouts) {
        if (!layout.id.startsWith("bm-"))
            continue;
        ...
        for (const i of terrainIdx) {
            ...
            const twin = terrainIdx.find((j) => {
                const cj = centroid(resolved[j]);
                return (Math.abs(cj.x - reflected.x) <= TWIN_CENTROID_TOLERANCE_IN &&
            ...
            if (a.length !== expected || b.length !== expected) {
                violations.push(`${layout.id}: piece ${pieces[i].id ?? i} expected ${expected} keystones`);
                continue;
            }
            for (let k = 0; k < expected; k++) {
                if (Math.abs(a[k] - b[k]) > PAIR_TOLERANCE_IN) {
                    violations.push(`${layout.id}: pieces ${pieces[i].id ?? i}/${pieces[twin].id ?? twin} measure apart ` +
...
//# sourceMappingURL=derive-keystones.js.map