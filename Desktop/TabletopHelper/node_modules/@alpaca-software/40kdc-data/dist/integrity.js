...
import { baseLoadout, checkUnitLegality } from "./data/loadout.js";
 ...
 */
export const KNOWN_REPLICATED_DRIFT = new Set([]);
/**
 ...
 */
export const KNOWN_LOADOUT_ORPHANS = new Set([]);
/**
...
function collectDiceTableErrors(node, out) {
    if (Array.isArray(node)) {
        for (const value of node)
            collectDiceTableErrors(value, out);
        return;
    }
    ...
    if (effect.type === "dice-table" && (effect.dice === "D3" || effect.dice === "D6") && Array.isArray(effect.outcomes)) {
        ...
        for (const outcome of effect.outcomes) {
            if (outcome === null || typeof outcome !== "object" || !("results" in outcome) || !Array.isArray(outcome.results))
                continue;
        ...
        if (missing.length > 0)
            out.push(`dice-table ${effect.dice} omits result${missing.length === 1 ? "" : "s"} ${missing.join(", ")}`);
        if (overlaps.length > 0)
            out.push(`dice-table ${effect.dice} repeats result${overlaps.length === 1 ? "" : "s"} ${overlaps.join(", ")}`);
    }
    for (const value of Object.values(effect))
        collectDiceTableErrors(value, out);
}
export function diceTableInvariantErrors(effect) {
    const errors = [];
...
export async function checkReferentialIntegrity(dataRoot) {
    ...
    const coreAbilityById = new Map();
    try {
        for (const ability of readArray(resolve(root, "enrichment/_core/abilities.json"))) {
            const id = ability.ability_id ?? ability.id;
            if (id)
                coreAbilityById.set(id, ability);
        }
    }
    catch {
    ...
    for (const file of unitFiles) {
        ...
        for (const u of units)
            if (u.id)
                idCounts.set(u.id, (idCounts.get(u.id) ?? 0) + 1);
    ...
    const allAbilityIds = new Set(coreAbilities);
    const abilityIdsByFaction = new Map();
    const abilityRecordsByFaction = new Map();
    ...
    for (const f of abilityFiles) {
        ...
        try {
            ...
            const abilityRecords = new Map();
            for (const a of readArray(f)) {
                if (a.ability_id) {
    ...
    for (const f of await glob("core/*/factions.json", { cwd: root, absolute: true })) {
        try {
            for (const fac of readArray(f)) {
                if (fac.faction_rule_id)
                    factionRuleIds.add(fac.faction_rule_id);
            }
        }
        catch {
    ...
    const collectRuleStateRefs = (node, out) => {
        if (Array.isArray(node)) {
            for (const v of node)
                collectRuleStateRefs(v, out);
        }
    ...
    const collectAbilityGrantRefs = (node, out) => {
        if (Array.isArray(node)) {
            for (const value of node)
                collectAbilityGrantRefs(value, out);
        }
        else if (node !== null && typeof node === "object") {
            const effect = node;
            if (effect.type === "ability-grant" && effect.modifier !== null && typeof effect.modifier === "object") {
                ...
                if (modifier.rules_bundle === true && typeof abilityId === "string")
                    out.push(abilityId);
            }
            for (const value of Object.values(effect))
                collectAbilityGrantRefs(value, out);
    ...
    for (const file of abilityFiles) {
        ...
        for (let i = 0; i < abilities.length; i++) {
            ...
            if (refs.length === 0 && abilityGrantRefs.length === 0 && diceTableErrors.length === 0)
                continue;
            result.totalItems++;
            const errs = [];
            ...
            for (const abilityId of abilityGrantRefs) {
                ...
                else if (grantedAbility.effect?.type !== "rules-bundle") {
                    errs.push({
                        path: `/${i}/effect`,
    ...
    for (const file of optionFiles) {
        ...
        const optIdCounts = new Map();
        for (const o of options)
            if (o.id)
                optIdCounts.set(o.id, (optIdCounts.get(o.id) ?? 0) + 1);
        ...
        try {
            weaponIds = new Set(readArray(resolve(dirname(file), "weapons.json")).map((w) => w.id ?? ""));
        }
        catch {
    ...
    for (const file of compFiles) {
        ...
        for (const c of comps)
            if (c.unit_id)
                compCounts.set(c.unit_id, (compCounts.get(c.unit_id) ?? 0) + 1);
        ...
        let units;
        try {
            units = readArray(resolve(dir, "units.json"));
        }
        catch {
            continue;
        }
        const weaponIdsByUnit = new Map(units.map((u) => [u.id ?? "", u.weapon_ids ?? []]));
        const unitRecById = new Map(units.map((u) => [u.id ?? "", u]));
        const reachableByUnit = new Map();
        const optionsByUnit = new Map();
        try {
            for (const o of readArray(resolve(dir, "wargear-options.json"))) {
        ...
        for (let i = 0; i < comps.length; i++) {
            ...
            for (const m of models)
                for (const id of m.default_weapon_ids ?? [])
                    defaults.add(id);
            const reachable = reachableByUnit.get(c.unit_id ?? "") ?? new Set();
            const errs = [];
            for (const wid of weaponIdsByUnit.get(c.unit_id ?? "") ?? []) {
                if (defaults.has(wid) || reachable.has(wid))
                    continue;
            ...
            if (unitRec) {
                ...
                const options = (optionsByUnit.get(c.unit_id ?? "") ?? []);
                const asLoadoutModels = (ms) => ms.map((m) => ({ ...m, min: m.min ?? 0, max: m.max ?? m.min ?? 0 }));
                const seen = new Set();
                for (const { size, rows } of shapes.sort((a, b) => a.size - b.size)) {
                    if (size <= 0 || seen.has(size))
                        continue;
                    seen.add(size);
                    const stock = baseLoadout(unitRec, size, options, asLoadoutModels(rows));
                    const violations = checkUnitLegality(unitRec, size, options, stock.counts, asLoadoutModels(models), tiers);
        ...
        for (const c of comps) {
            ...
            const tiers = c.tiers;
            if (!u || !points?.length || !tiers?.length)
            ...
            if (!ranges.length)
                continue;
            const overlap = ranges.some((a, i) => ranges.some((b, j) => i < j && a.min <= b.max && b.min <= a.max));
            if (overlap)
                continue; // choice-based pricing — not a flat size→cost map
            ...
            const covers = (n) => active.some((p) => (p.models ?? 0) <= n && n <= (p.models_max ?? p.models ?? 0));
            const shown = points
    ...
    const stale = [...KNOWN_LOADOUT_ORPHANS].filter((k) => scannedFactions.has(k.split("/")[0]) && !seenAllowed.has(k));
    if (stale.length > 0) {
        result.failed++;
        result.errors.push({
    ...
    factionFiles.sort();
    const canonical = (v) => JSON.stringify(v, (_key, value) => value !== null && typeof value === "object" && !Array.isArray(value)
    ...
    for (const file of factionFiles) {
        ...
        if (faction.startsWith("_") && faction !== "_core")
            continue;
        ...
        if (!policy) {
            ...
            result.errors.push({
                ...
                errors: [
                    {
                        path: "/",
                        message: `data file type "${name}.json" has no declared collision policy — add "${name}" to ` +
    ...
    for (const [name, group] of byBasename) {
        const policy = COLLISION_POLICIES[name];
        if (!policy || policy.policy === "merged" || policy.policy === "faction-scoped")
            continue;
        result.totalFiles++;
    ...
    const staleDrift = [...KNOWN_REPLICATED_DRIFT].filter((k) => byBasename.has(k.split("/")[0]) && !seenDrift.has(k));
