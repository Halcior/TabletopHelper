...
export function lintCanonical(effect) {
    ...
    const visit = (node) => {
        ...
        if (allow && node.modifier && typeof node.modifier === "object") {
            ...
            if (m.result_scope != null && !CANONICAL_RESULT_SCOPES.has(String(m.result_scope)))
                issues.push(`${type}: unknown result_scope "${String(m.result_scope)}"`);
 ...
 */
export function buildRepairedEntry(original, effect, scope, behavior, fields = {}) {
    const entry = { ...original, effect, scope, community_notes: authoringNote(original) };
    if (behavior && BEHAVIOR_VALUES.has(behavior))
        entry.behavior = behavior;
    for (const key of ["trigger", "usage", "applies_to"]) {
 ...
 */
function isFaithfulVerdict(verdict) {
    if (!verdict || verdict.faithful !== true)
        return false;
    return verdict.severity === "ok";
}
/** Whether a proposal is safe to apply automatically. */
export function passesGate(p, opts) {
    if (!p.schema_valid || !p.final_faithful || !isFaithfulVerdict(p.verdict) || p.unencodable)
        return false;
    if (p.confidence === "low")
        return false;
...
export function canReplaceEffect(proposal, effect, opts) {
...
export const repairUserPrompt = (items) => `Repair each ability's DSL. Emit the full effect tree + scope + behavior and any trigger, usage, or applies_to fields needed to fix the stated gap. Return results[] (one per ability, echo its ability_id):\n\n` +
...
async function proposeFaction(faction, opts, validate) {
    ...
    const input = readJSON(inputPath)
        .filter((e) => e.resolved)
        .filter((e) => !opts.stubsOnly || hasEmptyModifier(original.get(e.ability_id)?.effect));
    if (input.length === 0)
        return { faction, skipped: opts.stubsOnly ? "no resolved stubs" : "no resolved source rules" };
    ...
    const prior = new Map();
    if (!opts.fresh && existsSync(outPath))
        for (const p of readJSON(outPath))
            prior.set(p.ability_id, p);
    const proposals = [];
    const pending = [];
    ...
    for (const it of input) {
        ...
        if (prev && !prev.error && prev.src_hash === h)
            proposals.push(prev);
        else
            pending.push(it);
    }
    ...
    if (resumed > 0)
        process.stderr.write(`  ${faction}: resuming — ${resumed} kept, ${pending.length} to (re)propose\n`);
    const checkpoint = () => writeJSON(outPath, proposals);
    await mapLimit(chunk(pending, opts.batch), opts.concurrency ?? 1, async (batch) => {
        ...
        for (const p of proposals)
            if (p.src_hash == null)
                p.src_hash = hashById.get(p.ability_id);
...
async function repairFaction(faction, opts, validate) {
    ...
    await mapLimit(chunk(targets, opts.batch), opts.concurrency ?? 1, async (batch) => {
        let results;
        try {
            ({ results } = await callClaude(REPAIR_SYSTEM, repairUserPrompt(batch.map(({ p }) => {
        ...
        if (toVerify.length > 0) {
            try {
                const { results: vs } = await callClaude(VERIFY_SYSTEM, verifyUserPrompt(toVerify.map((b) => ({
...
function applyFaction(faction, opts) {
    ...
    for (const p of proposals) {
        ...
        for (const [proposalKey, entryKey] of [
            ...
            if (!Object.prototype.hasOwnProperty.call(p, proposalKey))
                continue;
            if (p[proposalKey] == null)
                delete entry[entryKey];
            else
                entry[entryKey] = p[proposalKey];
        }
...
async function main() {
    ...
    if (mode === "propose") {
        ...
        const opts = { batch: Number(flag(argv, "--batch")) || 15, model: flag(argv, "--model") ?? "claude-haiku-4-5", fresh: argv.includes("--fresh"), concurrency: Number(flag(argv, "--concurrency")) || 1, stubsOnly: argv.includes("--stubs-only") };
        const summary = [];
