...
const BATTLE_SIZES = ["incursion", "strike-force"];
...
function requireIndex(idx, kind, id) {
    const i = idx.index(kind, id);
    if (i === undefined) {
        throw new Error(`share registry has no ${kind} id "${id}" — run \`npm run registry:build\` and commit the result`);
    }
    return i;
}
/** Encode a {@link ShareList} into a URL-safe `share-v1` token. */
export function encodeShareList(list, registry) {
    ...
    w.varint(list.disposition === null ? 0 : requireIndex(registry, "disposition", list.disposition) + 1);
    w.varint(list.detachmentIds.length);
    for (const id of list.detachmentIds)
        w.varint(requireIndex(registry, "detachment", id));
    ...
    for (const u of list.units) {
        ...
        const flags = (u.isWarlord ? FLAG_WARLORD : 0) |
        ...
        w.varint(flags);
        if (u.enhancementId)
            w.varint(requireIndex(registry, "enhancement", u.enhancementId));
        if (u.attachedToOrdinal !== null)
            w.varint(u.attachedToOrdinal);
        if (flags & FLAG_ALLY) {
...
function requireId(idx, kind, slot) {
...
export function decodeShareList(token, registry) {
    try {
        ...
        const disposition = dispositionRef === 0 ? null : requireId(registry, "disposition", dispositionRef - 1);
        ...
        for (let i = 0; i < unitCount; i++) {
            ...
            const enhancementId = flags & FLAG_ENH ? requireId(registry, "enhancement", r.varint()) : null;
            const attachedToOrdinal = flags & FLAG_ATTACH ? r.varint() : null;
            let allyFactionId = null;
            let allyRuleId = null;
            if (flags & FLAG_ALLY) {
                const fRef = r.varint();
        ...
        if (e instanceof StaleRegistryError)
