 ...
 */
export async function applyWrites(staged, opts) {
    if (staged.length === 0) {
    ...
    try {
        ...
        if (failed > 0) {
            const detail = formatReport(mergeSchema(core, enrich), "pretty") +
                "\n" +
                formatReport(integrity, "pretty", "40kdc Referential Integrity Report");
            throw new Error(`[${opts.label}] projected dataset FAILS validation (${failed} error(s)). ` +
                (opts.write
                    ? "Nothing was written."
                    : "This is exactly what --write would have produced.") +
                `\n${detail}`);
        }
        console.log(`[${opts.label}] projected dataset valid — ${staged.length} file(s), ` +
        ...
        if (opts.write) {
            ...
            try {
                ...
                throw new Error(`[${opts.label}] I/O FAULT after ${written.length}/${staged.length} files written ` +
