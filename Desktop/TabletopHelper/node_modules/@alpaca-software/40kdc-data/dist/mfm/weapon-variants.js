...
import { mintWeapon, } from "./gear-projection.js";
...
function weaponFp(profiles) {
    return JSON.stringify([...profiles].map(profileFp).sort());
}
// ── dump loadout walk: the weapon wargear_items a datasheet actually fields ──
// ── reference rewiring ──
const remap = (ids, m) => ids?.map((id) => m.get(id) ?? id);
function rewireUnit(u, m) {
    const out = { ...u };
    if (u.weapon_ids)
        out.weapon_ids = remap(u.weapon_ids, m);
    if (u.wargear_budgets)
        out.wargear_budgets = u.wargear_budgets.map((b) => ({ ...b, items: remap(b.items, m) ?? b.items }));
    return out;
}
function rewireComp(c, m) {
    if (!c.models)
        return c;
    return {
        ...c,
        models: c.models.map((mdl) => mdl.default_weapon_ids
            ? { ...mdl, default_weapon_ids: remap(mdl.default_weapon_ids, m) }
            : mdl),
    };
}
function rewireOption(o, m) {
    const out = { ...o };
    if (o.replaces)
        out.replaces = remap(o.replaces, m);
    if (o.replacement)
        out.replacement = remap(o.replacement, m);
...
export function runWeaponVariants(dump, onlyDir) {
    ...
    for (const dir of dirs) {
        ...
        const weapons = readJsonArray(wpnPath);
        if (weapons.length === 0)
            continue;
        const units = readJsonArray(unitsPath);
        const comps = readJsonArray(compsPath);
        const options = readJsonArray(optsPath);
        ...
        const bareFp = new Map();
        const repoEntryByFp = new Map();
        for (const [id, es] of entriesById) {
            ...
            if (byFp.size < 2)
                continue; // identical-stat duplicates — leave untouched
            conflicting.add(id);
        ...
        const optsByUnit = new Map();
        for (const o of options)
            (optsByUnit.get(o.unit_id) ?? optsByUnit.set(o.unit_id, []).get(o.unit_id)).push(o);
        const refsOf = (u) => {
            const refs = new Set();
        ...
        const warnings = [];
        const assignments = new Map(); // unitId -> (baseId -> variantId)
        const variantEntries = new Map(); // variantId -> weapon
        for (const unit of [...units].sort((a, b) => a.id.localeCompare(b.id))) {
            ...
            for (const wargearItem of wargearItemsForDatasheet(dump, ds.id)) {
                if (wargearItem.wargearType !== "weapon")
                    continue;
                ...
                try {
                    ...
                    warnings.push(`${unit.id}: could not mint "${name}": ${e.message}`);
                    continue;
                }
            ...
            for (const baseId of [...refs].sort()) {
                ...
                if (fp === bareFp.get(baseId))
                    continue; // matches the bare entry — keep it
                ...
                variantEntries.set(variantId, {
                    ...
                    game_version: src.game_version ?? gv,
                });
        ...
        for (const v of [...variantEntries.values()].sort((a, b) => a.id.localeCompare(b.id)))
            newWeapons.push(v);
...
export function buildWeaponVariantsReport(report, write) {
    const { dirs } = report;
    const sum = (f) => dirs.reduce((a, d) => a + f(d), 0);
    const L = [];
    L.push(`# Weapon-variant split ${write ? "(applied)" : "(dry run)"}`);
    L.push("");
    L.push(`Totals: ${sum((d) => d.conflictingIds)} conflicting id(s), ` +
    ...
    L.push("| --- | --: | --: | --: | --: | --: |");
    for (const d of dirs) {
        if (d.conflictingIds === 0 && d.variantsAdded === 0)
            continue;
...
//# sourceMappingURL=weapon-variants.js.map