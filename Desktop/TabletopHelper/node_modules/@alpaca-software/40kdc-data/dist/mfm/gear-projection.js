...
function mapWeaponKeyword(raw) {
    ...
    if (valued) {
        ...
        const value = /^\d+$/.test(rawValue) ? Number(rawValue) : rawValue.toUpperCase();
        const byBase = {
            melta: "melta",
    ...
    const id = flat[lower];
    if (!id || ["anti", "melta", "cleave", "rapid-fire", "sustained-hits"].includes(id))
        return null;
...
function parseArmourPenetration(value) {
    const numeric = Number.parseInt((value ?? "0").trim(), 10);
    return Number.isFinite(numeric) ? numeric : 0;
}
function parseRange(value, type) {
    if (type === "melee")
        return "Melee";
    const normalized = (value ?? "").trim();
...
export function mintWeapon(ctx, item, id, name) {
    const { dump } = ctx;
    ...
    const built = profiles.map((profile) => {
        ...
        if (ranged)
            stats.BS = parseSkill(profile.ballisticSkill);
        else
            stats.WS = parseSkill(profile.weaponSkill);
        const keywords = [];
...
export function mintWargear(ctx, item, id, name) {
    return { id, name, game_version: ctx.gv };
}
//# sourceMappingURL=gear-projection.js.map