...
function applyStep(base, step, extra) {
    if (!step)
...
export function deriveDatasheet(dump, datasheetId) {
    ...
    for (const c of comps) {
        if (c.referenceGroupingKeywordId || c.points == null)
            continue;
        const { min, max } = rangeOf(c.id);
        if (max <= 0)
            continue; // no model rows (data-only / attachment-at-0) — no size
        const cost = c.points;
    ...
    const ambiguous = nativeBase.some((a, i) => nativeBase.some((b, j) => i < j && a.cost !== b.cost && a.models <= b.models_max && b.models <= a.models_max));
    // Allied (grouped) tiers, keyed by host faction keyword.
    const alliedBaseByHost = new Map();
    for (const c of comps) {
        if (!c.referenceGroupingKeywordId || c.points == null)
            continue;
 ...
 */
export function routeChapterTwin(rec, host, twinNative) {
    const nativeRanges = new Set((rec.points ?? []).map((t) => `${t.models}:${t.models_max ?? t.models}`));
...
export function runPoints(dump, write) {
    ...
    const results = [];
    const staged = [];
    const matchedDatasheets = new Set(); // ds.id that found a repo unit somewhere
    for (const dir of [...dirs].sort()) {
        const p = path.join(CORE_DIR, dir, "units.json");
        if (!fs.existsSync(p))
            continue;
        const units = readJsonArray(p);
        ...
        for (const ds of dsList) {
            ...
            if (!rec)
                continue; // not in this dir — may still match in another candidate dir
            if (matchedRepoIds.has(id)) {
                ...
                if (twin.ambiguous || !twin.native.length)
                    continue;
                ...
                if (next.length)
                    rec.allied_points = next;
                else
                    delete rec.allied_points;
                continue;
            }
            ...
            if (!native.length)
                continue; // no dump price (e.g. data-only datasheet)
            if (ambiguous) {
            ...
            if (alliedClean.length)
                rec.allied_points = alliedClean;
    ...
    for (const ds of dump.table("datasheet")) {
        if (ds.isLegends || matchedDatasheets.has(ds.id) || seenNew.has(ds.id))
            continue;
...
export function buildPointsReport(report, write) {
    const { dirs, newInDump } = report;
    const sum = (f) => dirs.reduce((a, d) => a + f(d), 0);
    const size = (x) => x.models_max != null && x.models_max !== x.models ? `${x.models}-${x.models_max}` : `${x.models}`;
    const fmt = (t) => t.length
        ? t
            .map((x) => x.unit_count_min === undefined
            ? `${size(x)}m=${x.cost}`
    ...
    L.push("");
    L.push("| Dir | Matched | Points changed | Allied added | Ambiguous (kept) | Repo-only (Legends/FW) |");
    L.push("|---|--:|--:|--:|--:|--:|");
    for (const d of dirs.filter((d) => d.matched || d.repoOnly.length)) {
        L.push(`| ${d.dir} | ${d.matched} | ${d.pointsChanged.length} | ${d.alliedAdded.length} | ${d.ambiguousSkipped.length} | ${d.repoOnly.length} |`);
    }
    L.push(`| **TOTAL** | **${sum((d) => d.matched)}** | **${sum((d) => d.pointsChanged.length)}** | **${sum((d) => d.alliedAdded.length)}** | **${sum((d) => d.ambiguousSkipped.length)}** | **${sum((d) => d.repoOnly.length)}** |`);
    L.push("");
    for (const d of dirs) {
        ...
        if (d.alliedAdded.length) {
            ...
            d.alliedAdded.forEach((c) => L.push(`- ${c.id}: ${c.allied.map((a) => `${a.host_faction}:${size(a)}m=${a.cost}`).join(", ")}`));
        }
