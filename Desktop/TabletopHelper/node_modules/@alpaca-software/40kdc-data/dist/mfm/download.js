...
export async function downloadMfmDump(outputPath = DEFAULT_DUMP_PATH, fetchImpl = fetch) {
    const response = await fetchImpl(MFM_DUMP_URL);
    if (!response.ok)
        throw new Error(`MFM dump download failed: HTTP ${response.status}.`);
