...
import { deriveDatasheet, cleanTier } from "./points.js";
import { isCombatPatrolPublication } from "./game-mode.js";
...
const COMBAT_PATROL_ONLY = ["combat-patrol"];
 ...
 */
function statInt(raw, field, name) {
 ...
 */
function buildProfiles(dump, datasheetId, unitName) {
...
function buildKeywords(dump, datasheetId) {
    const minis = (dump.groupBy("miniature", "datasheetId").get(datasheetId) ?? [])
        .slice()
        .sort((a, b) => a.displayOrder - b.displayOrder);
    ...
    for (const m of minis) {
        ...
        for (const mk of mks) {
            ...
            if (!nm || seen.has(nm) || NON_GAME_KEYWORDS.has(nm))
                continue;
            seen.add(nm);
            out.push(nm);
        }
 ...
 */
function buildFactionKeywords(dump, datasheetId, dir) {
    const home = FACTION_HOME_KEYWORD[dir];
    if (home)
        return [home];
    const rows = (dump.groupBy("datasheet_faction_keyword", "datasheetId").get(datasheetId) ?? [])
        .slice()
        .sort((a, b) => a.displayOrder - b.displayOrder);
...
function deriveRole(keywords) {
    ...
    if (kw.has("Battleline"))
        return "battleline";
    if (kw.has("Dedicated Transport"))
        return "dedicated-transport";
    if (kw.has("Fortification"))
 ...
 */
function buildModelCount(dump, datasheetId) {
...
export function buildSeedUnit(dump, ds, dir) {
    const name = dump.enName(ds);
    if (!name)
        throw new SeedSkip(`<${ds.id}>: datasheet has no English name`);
    let id;
    try {
        id = nameToId(name);
    }
 ...
 */
export function effectiveDir(dir) {
    if (fs.existsSync(unitsPath(dir)))
        return dir;
    for (const parent of SHARED_ROSTERS[dir] ?? []) {
        if (fs.existsSync(unitsPath(parent)))
            return parent;
    }
    return null;
}
export function runSeedUnits(dump, opts = {}) {
    ...
    const filesByEffDir = new Map();
    const loadEff = (dir) => {
        let arr = filesByEffDir.get(dir);
        if (!arr) {
            arr = JSON.parse(fs.readFileSync(unitsPath(dir), "utf8"));
    ...
    for (const dir of [...byDir.keys()].sort()) {
        ...
        if (!effDir)
            continue; // dir has no units home (parent-less, unit-less)
        ...
        for (const ds of byDir.get(dir) ?? []) {
            ...
            if (effIds.has(id) || sharedIds.has(id))
                continue; // matched, or a shared-roster dup
        ...
        const res = { dir, created: [], cpExcluded: [], skipped: [] };
        if (effDir !== dir)
            res.routedTo = effDir;
    ...
    const staged = [...filesByEffDir.entries()]
...
export function buildSeedUnitsReport(report, write) {
    const { dirs } = report;
    ...
    L.push("Combat-Patrol-only units (from `Combat Patrol: X` publications) are held back " +
    ...
    L.push("|---|---|--:|--:|--:|");
    for (const d of dirs) {
        L.push(`| ${d.dir} | ${d.routedTo ?? d.dir} | ${d.created.length} | ${d.cpExcluded.length} | ${d.skipped.length} |`);
    }
...
//# sourceMappingURL=seed-units.js.map