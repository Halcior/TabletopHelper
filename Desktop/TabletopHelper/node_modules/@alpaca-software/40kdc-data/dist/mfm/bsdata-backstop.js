...
const ALLOWED_PROFILE_TYPES = new Map([
    ["unit", "unit"],
...
function parseJsonRoot(file, content) {
    let parsed;
    try {
    ...
    if (!isRecord(parsed))
        throw new Error(`Malformed BSData root in ${file}`);
    return parsed;
}
function readDocuments(tree, commit) {
    const documents = [];
    for (const file of [...tree.listFiles(commit)].filter((item) => item.endsWith(".json")).sort()) {
        ...
        const root = (isRecord(gameSystem) ? gameSystem : catalogue);
        const id = text(root.id);
        if (!id)
            throw new Error(`Missing BSData root id in ${file}`);
        const imports = kind === "catalogue"
...
function profileContract(gameSystem) {
    ...
    const pointsCostTypeIds = new Set();
...
function extractProfiles(node, pointer, contract, warnings, sourceFile) {
    const result = [];
...
function extractPoints(node, contract) {
...
function walkDocument(source, contract, parserWarnings, heuristicWarnings) {
    const facts = [];
    const nodes = [];
    const links = [];
    const rootPointer = source.kind === "game-system" ? "/gameSystem" : "/catalogue";
    const entryKeys = ["selectionEntries", "sharedSelectionEntries"];
    const groupKeys = ["selectionEntryGroups", "sharedSelectionEntryGroups"];
    const recordFact = (node, pointer, id) => {
        const categoryHints = [];
        for (const [categoryIndex, category] of records(node.categoryLinks).entries()) {
            const label = safeLabel(category.name);
            if (label)
                categoryHints.push(label);
    ...
    const visitContainer = (container, pointer) => {
...
function parseRevision(tree, commit) {
    ...
    if (gameSystems.length !== 1)
        throw new Error(`Expected one BSData game system, found ${gameSystems.length}`);
    const gameSystem = gameSystems[0];
    for (const document of documents) {
        if (document.kind !== "catalogue")
            continue;
...
function importPath(source, target, documentsById, gameSystem) {
    ...
    const queue = [{ document: source, chain: [] }];
...
function resolveLinks(parsed) {
    ...
    for (const link of parsed.links) {
        const candidates = (nodesById.get(link.targetId) ?? [])
            ...
            .filter((item) => item.chain !== undefined);
        if (candidates.length !== 1) {
            parsed.parserWarnings.push({
...
function associateLinkFacts(facts, links) {
    const byLocation = new Map(facts.map((fact) => [`${fact.source_file}\0${fact.pointer}`, fact]));
    const resolvedByLink = new Map(links.map((link) => [`${link.source_file}\0${link.pointer}`, byLocation.get(`${link.target_file}\0${link.target_pointer}`)]));
    return facts.map((fact) => {
        ...
        return {
            ...
            category_hints: [...new Set([...fact.category_hints, ...target.category_hints])].sort((left, right) => left.localeCompare(right)),
        };
    });
}
function normalizedNames(facts, kind) {
...
function mfmComparisonFacts(dump) {
    const units = new Map();
    for (const row of dump.table("datasheet")) {
        const name = dump.enName(row);
        if (name)
            units.set(normalizeLabel(name), name);
    }
...
function compareToMfm(facts, dump) {
    ...
    const bsPoints = new Map();
    const bsProfiles = new Map();
    const bsHints = new Set();
    for (const fact of facts) {
        ...
        for (const hint of fact.category_hints)
            bsHints.add(normalizeLabel(hint));
    }
...
function heuristicClassificationWarnings(facts) {
    const result = [];
    for (const fact of facts) {
        const labels = [fact.name, ...fact.category_hints].map(normalizeLabel);
        for (const kind of ["detachment", "enhancement", "stratagem"]) {
...
function sourceComparisonWarnings(facts, dump) {
    const result = [{ kind: "snapshot-skew-review", id: dump ? String(dump.version ?? "unknown") : "unknown" }];
    if (!dump)
        return result;
    const mfm = mfmComparisonFacts(dump);
...
export function analyzeBsdataRevision(tree, sourceRef, dump) {
    const commit = tree.resolveRevision(sourceRef);
    if (!/^[0-9a-f]{40}$/.test(commit))
        throw new Error(`Revision did not resolve to one full commit: ${sourceRef}`);
...
export class JjRevisionTree {
    checkout;
    commandRunner;
    constructor(checkout, commandRunner = runCommand) {
...
export function resolvePrivateOutputPath(output) {
...
export function parseBsdataCli(argv) {
...
export function runBsdataCli(argv) {
    ...
    console.log(`BSData backstop: ${report.summary.mechanical_differences} mechanical differences, ` +
...
//# sourceMappingURL=bsdata-backstop.js.map