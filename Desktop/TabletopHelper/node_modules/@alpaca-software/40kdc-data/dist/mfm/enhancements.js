 ...
 */
export function cleanEnhName(name) {
 ...
 *  with null. */
export function buildEnhCanon(dump) {
...
function safeGroupBy(dump, name, key) {
    return dump.tables[name] ? dump.groupBy(name, key) : new Map();
}
...
function eligibilityShape(fields) {
...
export function reconcileEnhancementEligibility(record, fields) {
    const result = {};
    ...
    if (!sameLabels(from.keyword_restrictions, to.keyword_restrictions) ||
        ...
        if (to.keyword_restrictions === null)
            delete record.keyword_restrictions;
        else
            record.keyword_restrictions = to.keyword_restrictions;
        if (to.keyword_restriction_groups === null)
            delete record.keyword_restriction_groups;
        else
            record.keyword_restriction_groups = to.keyword_restriction_groups;
    }
    ...
    if (!sameLabels(exclusionFrom, fields.exclusion_keywords) ||
        ...
        if (fields.exclusion_keywords === null)
            delete record.exclusion_keywords;
        else
 ...
 */
export function buildEnhFieldCanon(dump) {
    const detName = dump.byId("detachment");
    ...
    const attachmentDatasheetsByGroup = safeGroupBy(dump, "enhancement_bodyguard_group_datasheet", "enhancementBodyguardGroupId");
    const out = new Map();
    for (const e of dump.table("enhancement")) {
        const en = dump.enName(e);
        const dn = dump.enName(detName.get(e.detachmentId));
        ...
        const unresolved = [];
        const exclusion_keywords = keywordLabels(dump, (excludedByEnh.get(e.id) ?? []).map((r) => r.keywordId), unresolved);
        ...
        for (const g of groupsByEnh.get(e.id) ?? []) {
            ...
            for (const r of fkwByGroup.get(g.id) ?? []) {
                ...
                if (label)
                    ...
                    unresolved.push(r.factionKeywordId);
            }
            ...
            if (g.datasheetId) {
                ...
                if (dsName)
                    ...
                    unresolved.push(g.datasheetId);
            }
            if (members.size)
                groupSets.push([...members].sort((a, b) => a.localeCompare(b)));
        }
        ...
        const attachmentBodyguardIds = new Set();
        for (const group of attachmentGroupsByEnh.get(e.id) ?? []) {
            for (const relation of attachmentDatasheetsByGroup.get(group.id) ?? []) {
                ...
                if (dsName)
                    attachmentBodyguardIds.add(nameToId(dsName));
                else
                    unresolved.push(relation.datasheetId);
            }
        }
        out.set(id, {
 ...
 */
export function combatPatrolEnhIds(dump) {
    const detName = dump.byId("detachment");
    const ids = new Set();
    for (const e of dump.table("enhancement")) {
        if (!e.isCombatPatrol)
 ...
 */
export function normalizeEnhancementNames(dump) {
    const detName = dump.byId("detachment");
    ...
    for (const e of dump.table("enhancement")) {
        ...
        if (prev && prev.newId !== rawId)
            collided.add(base);
        else
            baseMap.set(base, { newId: rawId, newName: en });
    }
    ...
    for (const dir of [...repoDirs()].sort()) {
        ...
        if (!fs.existsSync(p))
            continue;
        const enhs = readJsonArray(p);
        let touched = false;
        for (const e of enhs) {
            const det = e.detachment_id ?? "";
            if (!det)
                continue;
    ...
    for (const dir of [...repoDirs()].sort()) {
        ...
        if (!fs.existsSync(p))
            continue;
        const dets = readJsonArray(p);
        let touched = false;
        for (const det of dets) {
            if (!Array.isArray(det.enhancement_ids))
                continue;
...
export function runEnhancements(dump, write, opts = {}) {
    ...
    const newInDump = [];
    const cpExcluded = [];
    const enhancementsByDir = new Map();
    const detachmentLocations = new Map();
    const detachmentsByDir = new Map();
    for (const dir of [...repoDirs()].sort()) {
        const detPath = path.join(CORE_DIR, dir, "detachments.json");
        if (!fs.existsSync(detPath))
            continue;
        const records = readJsonArray(detPath);
    ...
    const touchedDetachmentPaths = new Set();
    for (const dir of [...repoDirs()].sort()) {
        const p = path.join(CORE_DIR, dir, "enhancements.json");
        if (!fs.existsSync(p))
            continue;
        ...
        for (const e of enhs) {
            ...
            if (cost !== null) {
                if (e.cost !== cost)
                    res.costChanged.push({ id: e.id, from: e.cost, to: cost });
            }
            const needsConfirm = e.points_provisional !== false ||
            ...
            if (f) {
                ...
                if (!sameLabels(attachmentAuthored, f.attachment_bodyguard_ids)) {
                    ...
                    if (f.attachment_bodyguard_ids === null)
                        delete e.attachment_bodyguard_ids;
                    else
                        e.attachment_bodyguard_ids = f.attachment_bodyguard_ids;
                }
    ...
    for (const source of dump.table("enhancement")) {
        ...
        if (dump.tables.publication && dump.tables.faction_keyword) {
            ...
            if (ownDir)
                sourceDirs.add(ownDir);
        }
        if (sourceDetachment?.id &&
...
export function buildEnhReport(report, write) {
    ...
    for (const d of dirs.filter((d) => d.matched || d.unmatchedRepo.length)) {
        L.push(`| ${d.dir} | ${d.matched} | ${d.costChanged.length} | ${d.upgradeChanged.length} | ${d.maxTargetsChanged.length} | ${d.eligibilityChanged.length} | ${d.exclusionChanged.length} | ${d.unmatchedRepo.length} |`);
    }
    L.push(`| **TOTAL** | **${sum((d) => d.matched)}** | **${sum((d) => d.costChanged.length)}** | **${sum((d) => d.upgradeChanged.length)}** | **${sum((d) => d.maxTargetsChanged.length)}** | **${sum((d) => d.eligibilityChanged.length)}** | **${sum((d) => d.exclusionChanged.length)}** | **${sum((d) => d.unmatchedRepo.length)}** |`);
    L.push("");
    for (const d of dirs) {
        const hasDetail = d.costChanged.length ||
        ...
        if (d.eligibilityChanged.length) {
            ...
            d.eligibilityChanged.forEach((c) => L.push(`- ${c.id}: ${JSON.stringify(c.from)} → ${JSON.stringify(c.to)}`));
        }
        if (d.exclusionChanged.length) {
            L.push("", "**exclusion_keywords changes:**");
            d.exclusionChanged.forEach((c) => L.push(`- ${c.id}: ${JSON.stringify(c.from)} → ${JSON.stringify(c.to)}`));
        }
    ...
    if (seeded.length) {
        ...
        seeded.forEach((entry) => L.push(`- ${entry.dir}/${entry.id} (${entry.name}) → ${entry.detachment_id}`));
        L.push("");
    }
    ...
    if (newInDump.length) {
        ...
        if (newInDump.length > 200)
            L.push(`- …and ${newInDump.length - 200} more`);
        L.push("");
    }
    if (cpExcluded.length) {
        L.push(`## Combat-Patrol enhancements held back (${cpExcluded.length} — pass --include-combat-patrol to author)`);
        L.push("");
        cpExcluded.slice(0, 200).forEach((s) => L.push(`- ${s}`));
        if (cpExcluded.length > 200)
            L.push(`- …and ${cpExcluded.length - 200} more`);
        L.push("");
    }
    return L.join("\n") + "\n";
}
