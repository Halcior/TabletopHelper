...
import { modeOfPublication, mergeMode } from "./game-mode.js";
import { repoDirForFactionName, repoDirs, FACTION_ALIASES, SHARED_ROSTERS } from "./faction-map.js";
import { mintWargear, mintWeapon, } from "./gear-projection.js";
 ...
 */
export const WEAPON_ALIASES_BY_UNIT = {
    "imperial-knights": {
 ...
 */
export function makeResolver(validIds, audit, aliases = {}, priorityAliases = {}) {
 ...
 */
export function unitScopedResolver(validIds, autoResolved, dir, unitId, factionResolve) {
 ...
 */
export function wargearItemsForDatasheet(dump, datasheetId) {
...
export function dirValidIds(dir, units, wopts) {
    const validIds = new Set();
    for (const filename of ["weapons.json", "wargear.json"]) {
        for (const item of readJsonArray(path.join(CORE_DIR, dir, filename))) {
            if (item.id)
...
function defaultUnitComposition(dump, datasheetId) {
...
function modelCountByMiniId(dump, datasheetId) {
    const out = new Map();
    ...
    for (const m of minis ?? [])
        out.set(m.miniatureId, (out.get(m.miniatureId) ?? 0) + m.min);
    return out;
}
/**
 ...
 */
function baseFromMiniatureLoadout(dump, datasheetId, resolve, unresolved, skip) {
    const out = new Map();
    ...
    const miniName = (id) => dump.enName(miniById.get(id)) ?? id;
    ...
    for (const b of dump.groupBy("base_miniature_loadout", "datasheetId").get(datasheetId) ?? []) {
        ...
        for (const x of bmlOpts.get(b.id) ?? []) {
            ...
            for (let i = 0; i < Math.max(1, x.count); i++)
                ids.push(id);
        }
        if (ids.length && !unresolvedWeapon)
            out.set(b.miniatureId, ids);
    }
    return out;
}
function deriveDefaults(dump, datasheetId, resolve, unresolved, notes) {
    const miniById = dump.byId("miniature");
    const miniName = (id) => dump.enName(miniById.get(id)) ?? id;
    const wiName = dump.byId("wargear_item");
    ...
    const baseGroupsByMini = new Map();
    ...
    for (const [miniId, ids] of baseFromMiniatureLoadout(dump, datasheetId, resolve, unresolved, new Set(byMiniId.keys()))) {
        if (!byMiniId.has(miniId))
            byMiniId.set(miniId, ids);
    }
    const byName = new Map();
    for (const [miniId, ids] of byMiniId)
        byName.set(miniName(miniId), ids);
 ...
 */
function miniScopedSingleCaps(dump, datasheetId, resolve) {
    const sets = dump.groupBy("limited_wargear_choice_set", "datasheetId").get(datasheetId) ?? [];
    ...
    for (const s of sets) {
        if (!s.miniatureId)
            continue; // datasheet-wide → a budget, not a per-option cap
        ...
        if (itemIds.size !== 1)
            continue; // shared (≥2) sets are budgets
        let minRatio = null;
 ...
 */
export function limitedSetBudgets(dump, datasheetId, resolve, unitOptions = []) {
    const sets = dump.groupBy("limited_wargear_choice_set", "datasheetId").get(datasheetId) ?? [];
    if (sets.length === 0)
        return [];
    const limitsBySet = dump.groupBy("wargear_limit", "limitedWargearChoiceSetId");
    ...
    const addedByModelName = new Map();
    const uncappedBranches = [];
    for (const o of unitOptions) {
        ...
        for (const branch of branches) {
            if (uncapped)
                uncappedBranches.push(new Set(branch));
            const name = c?.model_name;
            if (name) {
                for (const id of branch) {
                    const names = addedByModelName.get(id) ?? new Set();
                    names.add(name);
    ...
    const choicesOfSet = new Map();
    const itemsOfSet = new Map();
    for (const s of sets) {
        const choices = [];
        for (const c of choicesBySet.get(s.id) ?? []) {
            const line = [];
            for (const it of itemsByChoice.get(c.id) ?? []) {
                const id = resolve(dump.enName(wiName.get(it.wargearItemId)) ?? "");
                if (id)
                    line.push({ id, copies: Math.max(1, it.count ?? 1) });
            }
            choices.push(line);
        }
        choicesOfSet.set(s.id, choices);
        itemsOfSet.set(s.id, new Set(choices.flat().map((x) => x.id)));
    }
    ...
    const grantingRecords = (id) => {
        let n = 0;
        for (const o of unitOptions) {
            const branches = o.replacement ? [o.replacement] : (o.replacement_choice ?? []);
            if (branches.some((b) => b.includes(id)))
                n++;
        }
        return n;
    };
    ...
    for (const s of [...sets].sort((a, b) => a.id.localeCompare(b.id))) {
        ...
        const setItems = itemsOfSet.get(s.id) ?? new Set();
        ...
        const excluded = (id) => {
            ...
            if (leaked.has(id))
                return true;
            for (const o of sets) {
                if (o.id !== s.id && itemsOfSet.get(o.id)?.has(id))
                    return true;
            }
            if (s.miniatureId && setMini) {
                const names = addedByModelName.get(id);
                if (names && [...names].some((n) => n !== setMini))
                    return true;
            }
            return false;
        };
        ...
        const splitAllowance = items.size === 1 && s.miniatureId != null && grantingRecords(sorted[0]) >= 2;
        if (ratioCount != null && ratioPer != null) {
            if (items.size >= 2 || s.miniatureId == null || splitAllowance) {
                ...
                const hasOffsetBaseline = flat != null &&
                    defaultModelCount > 0 &&
 ...
 */
function factorCrossProduct(branches, baseSet) {
    const keys = new Set();
    const uniq = [];
    for (const b of branches) {
        if (b.length === 0 || new Set(b).size !== b.length)
            return null;
        const k = [...b].sort().join("|");
        if (!keys.has(k)) {
            keys.add(k);
    ...
    if (uniq.length < 4)
        return null;
    if (baseSet.length === 0 || new Set(baseSet).size !== baseSet.length)
        return null;
    if (!keys.has([...baseSet].sort().join("|")))
        return null;
    const width = uniq[0].length;
    if (width < 2 || uniq.some((b) => b.length !== width))
        return null;
    // Slots = connected components of the "never co-occur" graph.
    const items = [...new Set(uniq.flat())];
    const co = new Map(items.map((i) => [i, new Set()]));
    for (const b of uniq) {
        for (const x of b)
            for (const y of b)
                if (x !== y)
    ...
    for (const seed of items) {
        ...
        while (queue.length > 0) {
            ...
            for (const cand of items) {
                if (slotOf.has(cand) || co.get(cur).has(cand))
                    continue;
    ...
    for (const s of slots)
        product *= s.length;
    if (product !== uniq.length)
        return null;
    const out = new Map();
    for (const slot of slots) {
        const baseItems = slot.filter((x) => baseSet.includes(x));
        if (baseItems.length !== 1)
            return null; // implied by base ∈ branches; defensive
        const [base] = baseItems;
        const others = slot.filter((x) => x !== base).sort();
        if (others.length === 0)
            continue; // fixed slot — no option to offer
 ...
 */
export function deriveWargear(dump, datasheetId, resolve) {
    const unresolved = [];
    const notes = [];
    const { byName: defaultsByModel, byMiniId: baseByMiniId } = deriveDefaults(dump, datasheetId, resolve, unresolved, notes);
    const miniName = (id) => dump.enName(dump.byId("miniature").get(id)) ?? id;
    const wiName = dump.byId("wargear_item");
    ...
    const flatBudgetCap = new Map();
    limitedSetBudgets(dump, datasheetId, resolve).forEach((b, i) => {
        if (b.per_models !== 0)
            return;
        for (const id of b.items)
            flatBudgetCap.set(id, { key: `b${i}`, count: b.count });
    });
    const options = [];
    for (const set of (sets ?? []).slice().sort((a, b) => a.id.localeCompare(b.id))) {
        ...
        if (set.alternate)
            notes.push(`alternate loadout_choice_set ${set.id.slice(0, 8)} (${mini ?? "all"}) — review`);
        ...
        for (const ch of (choicesBySet.get(set.id) ?? []).slice().sort((a, b) => a.id.localeCompare(b.id))) {
            ...
            for (const it of itemsByChoice.get(ch.id) ?? []) {
                ...
                for (let i = 0; i < Math.max(1, it.count); i++)
                    ids.push(id);
            }
            if (dropped && ids.length === 0)
                continue;
            branches.push(ids);
        }
        ...
        const scope = new Set(branches.flat());
        ...
        if (allowDup && setLimit > 1 && baseSet.length > 0) {
            ...
            for (const [id, n] of counts) {
                for (let i = 0; i < Math.floor(n / setLimit); i++)
                    perPick.push(id);
            }
            baseSet = perPick;
        }
        ...
        const factored = setLimit === 1 && !allowDup && set.miniatureId
            ? factorCrossProduct(branches, baseSet)
            : null;
        if (factored) {
            notes.push(`cross-product loadout set ${set.id.slice(0, 8)} factored into ${factored.size} independent slot swap${factored.size === 1 ? "" : "s"} (${mini ?? "all"})`);
        }
        const groups = factored ?? new Map();
        ...
        if (!factored)
            for (const b of branches) {
                ...
                if (seenAdded.has(aKey))
                    continue; // duplicate delta
                seenAdded.add(aKey);
        ...
        for (const { removed, added } of groups.values()) {
            ...
            const pushOption = (mc, branches_) => {
                const opt = {
                    model_constraint: Object.keys(mc).length ? mc : null,
                };
                if (removed.length > 0)
                    opt.replaces = removed;
                if (branches_.length === 1)
                    opt.replacement = branches_[0];
                else
                    opt.replacement_choice = branches_;
                options.push(opt);
            };
            ...
            const branchCap = (branch) => {
                ...
                return branch
                    ...
                    .sort((a, b) => a.perN - b.perN ||
                    a.setId.localeCompare(b.setId))[0] ?? null;
            };
            ...
            for (const { cap, branches: partBranches } of partitions.values()) {
                ...
                const flatCovered = removed.length > 0 &&
                    grantedIds.length > 0 &&
 ...
 */
export function dumpComposition(dump, datasheetId) {
    const miniById = dump.byId("miniature");
    const miniName = (id) => dump.enName(miniById.get(id)) ?? id;
    const order = (id) => miniById.get(id)?.displayOrder ?? 0;
    const uc = defaultUnitComposition(dump, datasheetId);
    if (!uc)
        return [];
    const minis = (dump.groupBy("unit_composition_miniature", "unitCompositionId").get(uc.id) ?? [])
        .slice()
 ...
 */
export function aggregateComposition(dump, datasheetId) {
    const miniById = dump.byId("miniature");
    const miniName = (id) => dump.enName(miniById.get(id)) ?? id;
    const order = (id) => miniById.get(id)?.displayOrder ?? 0;
    ...
    for (const c of comps) {
        const minis = (dump.groupBy("unit_composition_miniature", "unitCompositionId").get(c.id) ?? [])
            .slice()
            .sort((a, b) => order(a.miniatureId) - order(b.miniatureId));
        const rows = [];
        const seen = new Set();
        for (const m of minis) {
            if (m.max <= 0)
                continue; // optional attachment listed at 0/0 — not a model row
            const name = miniName(m.miniatureId);
            if (seen.has(name))
 ...
 */
export function reconcileModels(models, dumpMinis, defaultsByModel, unit) {
    if (dumpMinis.length === 0)
        return null;
    const byName = new Map(models.map((m) => [m.name, m]));
    const missing = dumpMinis.filter((d) => !byName.has(d.name));
    if (missing.length === 0)
        return null; // defaults-patch path already covers this
    ...
    const sibBases = models
        .map((m) => m.base_size_mm)
        .filter((b) => !!b);
    const uniform = sibBases.length && sibBases.every((b) => JSON.stringify(b) === JSON.stringify(sibBases[0]))
        ? sibBases[0]
        : unit.base_size_mm;
    const notes = [];
    const synthesized = [];
    ...
    for (const d of dumpMinis) {
        ...
        const row = { name: d.name, min: d.min, max: d.max };
        row.is_leader_model = d.max === 1 && dumpMinis.some((x) => x.name !== d.name && x.max > 1);
        if (uniform)
            row.base_size_mm = uniform;
        else
            notes.push(`synthesized row "${d.name}": no uniform base_size_mm to inherit — left unset`);
        const def = defaultsByModel.get(d.name);
        if (def?.length)
            row.default_weapon_ids = def;
        else
            notes.push(`synthesized row "${d.name}": dump has no resolvable base loadout — default_weapon_ids left empty`);
        notes.push(`synthesized model row "${d.name}" (min ${d.min}/max ${d.max}, leader=${row.is_leader_model}) from dump composition`);
 ...
 */
export function candidateDirs(dump, ds) {
...
export function runWargear(dump, write, onlyDir) {
    const dirs = repoDirs();
    ...
    for (const dir of [...dirs].sort()) {
        ...
        const comps = readJsonArray(cpath);
        ...
        const gear = readJsonArray(gearPath);
        ...
        for (const ds of dsList) {
            ...
            if (!rec) {
                if (!res.newInDump.includes(id))
                    res.newInDump.push(id);
                continue;
            }
            if (matchedRepoIds.has(id))
                continue; // first candidate dir wins
                ...
                if (item.wargearType === "weapon") {
                    ...
                    if (!existingId) {
                        ...
                        try {
                            ...
                            res.notes.push({ id, note: `could not mint "${itemName}": ${error.message}` });
                        }
                        for (const warning of warnings)
                            res.notes.push({ id, note: warning });
                    }
                    continue;
                }
                if (item.wargearType !== "wargear" || existingId)
                    continue;
            ...
            for (const u of derived.unresolved)
                res.unresolvedNames.push({ id, name: u.name, context: u.context });
            for (const n of derived.notes)
                res.notes.push({ id, note: n });
            ...
            const dumpMinis = composition.skip === "duplicate-names"
                ? []
            ...
            const built = derived.options.map((o, i) => {
                const rec = {
                    id: `${id}-wgo-mfm-${i + 1}`,
                    unit_id: id,
                ...
                if (o.replaces)
                    rec.replaces = o.replaces;
                if (o.replacement)
                    rec.replacement = o.replacement;
                if (o.replacement_choice)
                    rec.replacement_choice = o.replacement_choice;
                if (o.model_constraint)
                    rec.model_constraint = o.model_constraint;
                return rec;
            });
            ...
            for (const comp of compsByUnit.get(unitId) ?? []) {
                for (const model of comp.models) {
                    for (const itemId of model.default_weapon_ids ?? []) {
                        if (weaponEntityIds.has(itemId))
                            reachable.add(itemId);
                    }
                }
            }
            for (const option of optionsByUnit.get(unitId) ?? []) {
                for (const itemId of option.replaces ?? [])
                    if (weaponEntityIds.has(itemId))
                        reachable.add(itemId);
                for (const itemId of option.replacement ?? [])
                    if (weaponEntityIds.has(itemId))
                        reachable.add(itemId);
        ...
        if (compsChanged)
            staged.push({ path: cpath, value: comps });
        if (unitsChanged)
            staged.push({ path: upath, value: units });
        if (res.weaponsAdded > 0 || res.weaponNamesChanged > 0)
            staged.push({ path: weaponsPath, value: weapons });
        if (res.wargearAdded > 0)
            staged.push({ path: gearPath, value: gear });
        ...
        for (const u of units) {
            if (!matchedRepoIds.has(u.id))
                res.repoOnlyFallback.push(u.id);
        }
    ...
    if (write && results.some((r) => r.unresolvedNames.length)) {
        ...
        fs.writeFileSync(path.join(UNMATCHED_DIR, "unmatched-wargear.json"), JSON.stringify(results.flatMap((r) => r.unresolvedNames.map((u) => ({ dir: r.dir, ...u }))), null, 2) + "\n");
    }
 ...
 */
export function forEachDirDatasheet(dump, cb) {
    const dirs = repoDirs();
    const byDir = new Map();
    for (const ds of dump.table("datasheet")) {
        if (ds.isLegends)
    ...
    for (const dir of [...dirs].sort()) {
        ...
        if (!fs.existsSync(upath))
            continue;
        const units = readJsonArray(upath);
        const wopts = readJsonArray(path.join(CORE_DIR, dir, "wargear-options.json"));
        const validIds = dirValidIds(dir, units, wopts);
        const autoResolved = [];
        const resolve = makeResolver(validIds, autoResolved, WEAPON_ALIASES[dir] ?? {});
        const byId = new Map(units.map((u) => [u.id, u]));
        ...
        for (const ds of dsList) {
            ...
            if (!byId.has(id))
                continue; // dump datasheet with no repo unit → caught by unitInventory
            if (matchedRepoIds.has(id))
                continue; // first candidate dir wins
            matchedRepoIds.add(id);
            const unitResolve = unitScopedResolver(validIds, autoResolved, dir, id, resolve);
...
export function wargearOptionInventory(dump) {
    const out = new Map();
    forEachDirDatasheet(dump, ({ dir, ds, unitId, resolve }) => {
        if (deriveWargear(dump, ds.id, resolve).options.length === 0)
            return;
        const m = out.get(dir) ?? out.set(dir, new Map()).get(dir);
...
export function compositionInventory(dump) {
    const out = new Map();
...
export function weaponInventory(dump) {
    const out = new Map();
    forEachDirDatasheet(dump, ({ dir, ds, resolve }) => {
        ...
        for (const ids of dw.defaultsByModel.values())
            for (const id of ids)
                add(id);
 ...
 */
export function runWargearBudgets(dump, onlyDir) {
    const dirs = repoDirs();
    const byDir = new Map();
    for (const ds of dump.table("datasheet")) {
        if (ds.isLegends)
    ...
    for (const dir of [...dirs].sort()) {
        ...
        const byId = new Map(units.map((u) => [u.id, u]));
        const wopts = readJsonArray(path.join(CORE_DIR, dir, "wargear-options.json"));
        const validIds = new Set(readJsonArray(path.join(CORE_DIR, dir, "weapons.json")).map((w) => w.id ?? ""));
        for (const u of units)
            for (const id of u.weapon_ids ?? [])
                validIds.add(id);
        for (const o of wopts) {
        ...
        for (const ds of dsList) {
            ...
            const rec = byId.get(id);
            if (!rec || matchedRepoIds.has(id))
                continue;
            matchedRepoIds.add(id);
            matched++;
            const unitResolve = unitScopedResolver(validIds, autoResolved, dir, id, resolve);
            const budgets = limitedSetBudgets(dump, ds.id, unitResolve, wopts.filter((o) => o.unit_id === id));
            ...
            if (budgets.length) {
                ...
                delete rec.wargear_budgets;
            }
            if (JSON.stringify(rec.wargear_budgets ?? null) !== before)
                changed = true;
        }
        if (changed)
            staged.push({ path: upath, value: units });
 ...
 */
export function pricedWargearItems(dump, datasheetId, resolve) {
    const wiName = dump.byId("wargear_item");
    const woByGroup = dump.groupBy("wargear_option", "wargearOptionGroupId");
    const groups = dump.groupBy("wargear_option_group", "datasheetId").get(datasheetId) ?? [];
 ...
 */
export function runWargearCosts(dump, onlyDir) {
    const dirs = repoDirs();
    const byDir = new Map();
    for (const ds of dump.table("datasheet")) {
        if (ds.isLegends)
    ...
    for (const dir of [...dirs].sort()) {
        ...
        const validIds = dirValidIds(dir, units, wopts);
        for (const w of readJsonArray(path.join(CORE_DIR, dir, "wargear.json"))) {
            if (w.id)
                validIds.add(w.id);
        }
        for (const c of readJsonArray(path.join(CORE_DIR, dir, "unit-compositions.json"))) {
        ...
        for (const ds of dsList) {
            ...
            if (costs.length) {
                ...
                delete rec.wargear_costs;
            }
            if (JSON.stringify(rec.wargear_costs ?? null) !== before)
                unitsChanged = true;
        }
        ...
        if (unitsChanged)
            staged.push({ path: upath, value: units });
        if (optsChanged)
            staged.push({ path: wpath, value: wopts });
 ...
 */
export function runCompositionNames(dump, onlyDir) {
    const dirs = repoDirs();
    const byDir = new Map();
    for (const ds of dump.table("datasheet")) {
        if (ds.isLegends)
    ...
    for (const dir of [...dirs].sort()) {
        ...
        const upath = path.join(CORE_DIR, dir, "units.json");
        const units = fs.existsSync(upath) ? readJsonArray(upath) : [];
        const wopts = readJsonArray(path.join(CORE_DIR, dir, "wargear-options.json"));
        const validIds = dirValidIds(dir, units, wopts);
        const autoResolved = [];
        const resolve = makeResolver(validIds, autoResolved, WEAPON_ALIASES[dir] ?? {});
        ...
        for (const ds of dsList) {
            ...
            if (matchedRepoIds.has(id) || !compsByUnit.has(id))
                continue;
            matchedRepoIds.add(id);
            matched++;
            ...
            const withManual = (rowName, ids) => {
                const add = MANUAL_DEFAULTS[dir]?.[id]?.[rowName] ?? [];
                if (!ids?.length)
                    return add.length ? [...add] : ids;
            ...
            for (const comp of compsByUnit.get(id) ?? []) {
                ...
                const mapped = matchNamesNormalized(comp.models.map((m) => m.name), view.map((d) => d.name));
                if (mapped) {
                    ...
                    const proposed = comp.models.map((m) => withManual(m.name, defaultsByModel.get(m.name)));
                    const reachable = new Set();
                    ...
                    for (const o of wopts) {
                        if (o.unit_id !== id)
                            continue;
                        for (const wid of o.replaces ?? [])
                            reachable.add(wid);
                        for (const wid of o.replacement ?? [])
                            reachable.add(wid);
                        for (const g of o.replacement_choice ?? [])
                            for (const wid of g)
                                reachable.add(wid);
                    ...
                    for (let i = 0; i < comp.models.length; i++) {
                        ...
                        if (!def?.length || sameMultiset(comp.models[i].default_weapon_ids ?? [], def))
                            continue;
                        const strands = (comp.models[i].default_weapon_ids ?? []).some((wid) => unitWeapons.has(wid) && !reachable.has(wid));
                        if (strands) {
                            notes.push({
                                dir,
                                id,
                ...
                for (const d of view) {
                    for (const wid of defaultsByModel.get(d.name) ?? [])
                        keptCoverage.add(wid);
                    const old = oldByNorm.get(normModelName(d.name));
                    for (const wid of old?.default_weapon_ids ?? [])
                        keptCoverage.add(wid);
                }
                for (const o of wopts) {
                    if (o.unit_id !== id)
                ...
                const sibBases = comp.models
                    .map((m) => m.base_size_mm)
                    .filter((b) => !!b);
                const uniform = sibBases.length &&
                    sibBases.every((b) => JSON.stringify(b) === JSON.stringify(sibBases[0]))
                    ? sibBases[0]
                    : unitById.get(id)?.base_size_mm;
                const rebuilt = view.map((d) => {
                    const env = agg.envelope.get(d.name);
                    const min = env?.min ?? d.min;
                    const max = env?.max ?? d.max;
                    const old = oldByNorm.get(normModelName(d.name));
                    const row = old ? { ...old, name: d.name, min, max } : { name: d.name, min, max };
                    if (!old) {
                ...
                const droppedRows = comp.models.filter((m) => !view.some((d) => normModelName(d.name) === normModelName(m.name)));
 ...
 */
function matchNamesNormalized(repoNames, dumpNames) {
    if (repoNames.length !== dumpNames.length)
        return null;
    const byNorm = new Map();
    for (const e of dumpNames) {
        const k = normModelName(e);
        if (byNorm.has(k))
            return null; // ambiguous dump side
        byNorm.set(k, e);
    }
    const used = new Set();
    const out = [];
 ...
 */
export function runCompositionTiers(dump, onlyDir) {
    const dirs = repoDirs();
    const byDir = new Map();
    for (const ds of dump.table("datasheet")) {
        if (ds.isLegends)
    ...
    for (const dir of [...dirs].sort()) {
        ...
        for (const ds of dsList) {
            ...
            for (const comp of compsByUnit.get(id) ?? []) {
                const matches = (ns) => ns.length === envNames.size && ns.every((n) => envNames.has(n));
                ...
                if (!matches(names)) {
                    ...
                    if (recovered && matches(recovered))
                        names = recovered;
                }
                if (!matches(names)) {
...
export function buildWargearReport(report, write) {
    const { dirs } = report;
    ...
    for (const d of dirs.filter((d) => d.matched || d.repoOnlyFallback.length)) {
        L.push(`| ${d.dir} | ${d.matched} | ${d.optionsChanged} | ${d.defaultsChanged} | ${d.weaponIdsChanged} | ${d.weaponNamesChanged} | ${d.weaponsAdded} | ${d.wargearAdded} | ${d.synthesizedRows} | ${d.unresolvedNames.length} | ${d.autoResolved.length} | ${d.notes.length} | ${d.newInDump.length} | ${d.repoOnlyFallback.length} |`);
    }
    L.push(`| **TOTAL** | **${sum((d) => d.matched)}** | **${sum((d) => d.optionsChanged)}** | **${sum((d) => d.defaultsChanged)}** | **${sum((d) => d.weaponIdsChanged)}** | **${sum((d) => d.weaponNamesChanged)}** | **${sum((d) => d.weaponsAdded)}** | **${sum((d) => d.wargearAdded)}** | **${sum((d) => d.synthesizedRows)}** | **${sum((d) => d.unresolvedNames.length)}** | **${sum((d) => d.autoResolved.length)}** | **${sum((d) => d.notes.length)}** | **${sum((d) => d.newInDump.length)}** | **${sum((d) => d.repoOnlyFallback.length)}** |`);
    L.push("");
    for (const d of dirs) {
        ...
        if (d.defaultChanges.length) {
            ...
            d.defaultChanges.forEach((change) => L.push(`- ${change.id} / ${change.model}: ${JSON.stringify(change.from)} → ${JSON.stringify(change.to)}`));
        }
        ...
        if (d.unresolvedNames.length) {
            ...
            const byName = new Map();
            for (const u of d.unresolvedNames)
                (byName.get(u.name) ?? byName.set(u.name, new Set()).get(u.name)).add(u.id);
            for (const [name, units] of [...byName].sort())
                L.push(`- \`${name}\` — ${[...units].sort().join(", ")}`);
        }
        if (d.notes.length) {
...
//# sourceMappingURL=wargear.js.map