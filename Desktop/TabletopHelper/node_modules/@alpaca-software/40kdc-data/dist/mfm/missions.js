...
function modeFromScoringType(st) {
    if (st === "fixed")
        return "fixed";
    if (st === "tactical")
        return "tactical";
...
const modeKey = (m) => m ?? "none";
function assembleRows(out, dump, kind, missions, objByMission, scoreByObj) {
    for (const mission of missions) {
        // Detachment-scoped primaries are crusade/narrative reskins the repo omits.
        if (kind === "primary" && "detachmentId" in mission && mission.detachmentId)
            continue;
...
function assembleInto(out, dump, kind) {
    if (kind === "secondary") {
        assembleRows(out, dump, kind, dump.table("secondary_mission"), dump.groupBy("secondary_mission_objective", "secondaryMissionId"), dump.groupBy("secondary_mission_objective_scoring", "secondaryMissionObjectiveId"));
        return;
    }
    assembleRows(out, dump, kind, dump.table("primary_mission"), dump.groupBy("primary_mission_objective", "primaryMissionId"), dump.groupBy("primary_mission_objective_scoring", "primaryMissionObjectiveId"));
}
/** repo card-id → ordered dump scoring rows, for both secondary and generic primary missions. */
export function buildMissionScoringCanon(dump) {
    const out = new Map();
...
export function missionInventory(dump) {
...
function exclusiveKey(cardId, mode, objKey) {
...
export function reconcileCard(card, rows) {
    ...
    for (const k of new Set([...awByMode.keys(), ...rowByMode.keys()])) {
        ...
        if (repo !== dumpN)
            shapeMismatch.push({ mode: k, repo, dump: dumpN });
    }
    if (shapeMismatch.length)
        return { changes: [], shapeMismatch, exclusiveReview: [] };
    const changes = [];
    const exclusiveReview = [];
...
export function missionEntityCanon(dump) {
    const packById = dump.byId("mission_pack");
    const out = new Map();
    for (const m of dump.table("primary_mission")) {
        if (m.detachmentId)
            continue; // detachment-scoped reskins the repo omits
 ...
 */
export function reconcileMissionEntities(dump) {
    const canon = missionEntityCanon(dump);
    const missions = readJsonArray(MISSIONS_PATH);
    const report = {
        matched: 0,
        sourceFilled: [],
        sourceReview: [],
    ...
    for (const m of missions) {
        ...
        for (const [field, dumpVal] of [
            ...
            const authored = m[field];
            if (authored === dumpVal)
                report.capConfirmed++;
            else if (authored != null)
                report.capReview.push({ id: m.id, field, authored, dump: dumpVal });
        }
    }
    if (dirty)
        report.staged.push({ path: MISSIONS_PATH, value: missions, text: formatCompact(missions) });
    return report;
}
export function runMissions(dump, _write) {
    const canon = buildMissionScoringCanon(dump);
    const cards = readJsonArray(CARDS_PATH);
    const report = {
        matched: 0,
        cardsChanged: 0,
        vpChanged: [],
    ...
    for (const card of cards) {
        ...
        for (const c of res.changes) {
            if (c.field === "vp" || c.field === "vp_per")
                report.vpChanged.push(c);
            else if (c.field === "vp_max")
                report.vpMaxChanged.push(c);
            else if (c.field === "cumulative")
                report.cumulativeChanged.push(c);
            else if (c.field === "exclusive_group")
                report.exclusiveAdded.push(c);
        }
    ...
    if (dirty)
        report.staged.push({ path: CARDS_PATH, value: cards, text: formatCompact(cards) });
    // The mission-entity reconcile (missions.json) stages independently.
    report.staged.push(...report.entities.staged);
    return report;
}
export function buildMissionsReport(report, write) {
    const L = [];
    ...
    if (report.entities.sourceReview.length || report.entities.capReview.length) {
        ...
        report.entities.sourceReview.forEach((r) => L.push(`- ${r.id} source: authored "${r.authored}" vs dump "${r.dump}"`));
        report.entities.capReview.forEach((r) => L.push(`- ${r.id} ${r.field}: authored ${r.authored} vs dump ${r.dump}`));
        L.push("");
    }
    const list = (title, items) => {
        ...
        for (const c of items)
            L.push(`- ${c.cardId}[${c.index}] ${c.field}: ${fmt(c.from)} → ${fmt(c.to)}`);
        L.push("");
    };
    ...
    if (report.exclusiveReview.length) {
        ...
        for (const r of report.exclusiveReview)
            L.push(`- ${r.cardId}[${r.index}] keeps "${r.key}"`);
        L.push("");
    }
