...
function readDetachmentRosters(dir) {
    const detachmentPath = path.join(CORE_DIR, dir, "detachments.json");
    const rosters = new Map();
    if (!fs.existsSync(detachmentPath))
        return rosters;
    for (const detachment of readJsonArray(detachmentPath)) {
...
function rosterFor(rostersByDirectory, dir, detachmentId) {
...
export function seedStratagems(dump, opts = {}) {
    const rostersByDirectory = new Map();
    const acceptedStratagemsByDirectory = new Map();
    ...
    const load = (p) => {
        if (!arrays.has(p))
            arrays.set(p, fs.existsSync(p) ? readJsonArray(p) : []);
        return arrays.get(p);
    };
...
//# sourceMappingURL=seed-stratagems.js.map