...
function detachmentsPath(dir) {
    return path.join(CORE_DIR, dir, "detachments.json");
}
function enhancementsPath(dir) {
 ...
 */
export function collectSeedDetachments(dump) {
    ...
    for (const det of dump.table("detachment")) {
        ...
        if (!name)
            throw new Error(`detachment <${det.id}> has no English name`);
        ...
        if (applicableDirs.size === 0) {
            ...
            if (ownedDir && knownDirs.has(ownedDir))
                applicableDirs.add(ownedDir);
        }
        if (applicableDirs.size === 0) {
        ...
        if (!disposition)
            throw new Error(`detachment "${name}" has no force disposition in the dump`);
        const enhancements = [];
        for (const e of enhByDet.get(det.id) ?? []) {
            const en = dump.enName(e);
            if (!en)
                throw new Error(`CP enhancement <${e.id}> of "${name}" has no English name`);
...
export function runSeedDetachments(dump, opts = {}) {
    ...
    const detsByDir = new Map();
    const enhsByDir = new Map();
    const loadDets = (dir) => {
        let a = detsByDir.get(dir);
        if (!a)
            detsByDir.set(dir, (a = readJsonArray(detachmentsPath(dir))));
        return a;
    };
    const loadEnhs = (dir) => {
        let a = enhsByDir.get(dir);
        if (!a)
            enhsByDir.set(dir, (a = readJsonArray(enhancementsPath(dir))));
        return a;
    };
    const resultByDir = new Map();
    const result = (dir) => {
        let r = resultByDir.get(dir);
        if (!r) {
            resultByDir.set(dir, (r = { dir, createdDetachments: [], createdEnhancements: [], cpExcluded: [], skipped: [] }));
        }
        return r;
    };
    const touchedDets = new Set();
    const touchedEnhs = new Set();
    const acceptedDetachmentsByDir = new Map();
    for (const c of candidates.sort((a, b) => a.dir.localeCompare(b.dir) || a.id.localeCompare(b.id))) {
        if (onlyDir && c.dir !== onlyDir)
            continue;
        ...
        if (c.combatPatrol)
            detachment.game_modes = [...COMBAT_PATROL_ONLY];
        dets.push(detachment);
...
export function buildSeedDetachmentsReport(report, write) {
    const { dirs } = report;
    const L = [];
    ...
    L.push("| --- | --- | --- | --- | --- |");
    for (const d of dirs) {
        L.push(`| ${d.dir} | ${d.createdDetachments.length} | ${d.createdEnhancements.length} | ${d.cpExcluded.length} | ${d.skipped.length} |`);
    }
    const sum = (f) => dirs.reduce((a, d) => a + f(d), 0);
    L.push("");
    L.push(`Total: ${sum((d) => d.createdDetachments.length)} detachment(s), ` +
    ...
    L.push("");
    for (const d of dirs) {
        if (!d.createdDetachments.length && !d.cpExcluded.length)
            continue;
        L.push(`## ${d.dir}`);
        ...
        for (const c of d.cpExcluded)
            L.push(`- held back \`${c.id}\` (${c.name})`);
        L.push("");
    }
    return L.join("\n") + "\n";
}
//# sourceMappingURL=seed-detachments.js.map