...
import { deriveWargear, aggregateComposition, makeResolver, WEAPON_ALIASES, WEAPON_ALIASES_BY_UNIT, } from "./wargear.js";
import { repoDirForFactionName } from "./faction-map.js";
import { nameToId } from "../converters/id-generator.js";
import { applyWrites } from "./apply.js";
import { mintWeapon, mintWargear, parseSkill, } from "./gear-projection.js";
 ...
 */
export function projectInvulnerableSave(rows) {
    const warnings = [];
    let invulnSv = null;
    let rangedSv = null;
    let meleeSv = null;
    let found = false;
    const combine = (current, incoming, label) => {
    ...
    for (const raw of rows) {
        const row = raw;
        if (row.miniatureId != null)
            continue; // model-level exception: not representable here
        found = true;
        ...
        if (rangedVal != null)
            rangedSv = combine(rangedSv, rangedVal, "ranged");
        if (meleeVal != null)
            meleeSv = combine(meleeSv, meleeVal, "melee");
        if (structuredScope) {
...
function invulnSummary(projection) {
    const parts = [];
    if (projection.invuln_sv != null)
        parts.push(`${projection.invuln_sv}+`);
    if (projection.invuln_sv_ranged != null)
        parts.push(`${projection.invuln_sv_ranged}+ ranged`);
    if (projection.invuln_sv_melee != null)
        parts.push(`${projection.invuln_sv_melee}+ melee`);
 ...
 */
function syncInvulnProfile(profile, projection) {
    profile.invuln_sv = projection.invuln_sv;
    if (projection.invuln_sv_ranged != null)
        profile.invuln_sv_ranged = projection.invuln_sv_ranged;
    else
        delete profile.invuln_sv_ranged;
    if (projection.invuln_sv_melee != null)
        profile.invuln_sv_melee = projection.invuln_sv_melee;
    else
        delete profile.invuln_sv_melee;
}
/**
...
async function runInvulnsOnly(opts) {
    ...
    for (const unitId of targets) {
        ...
        for (const w of projection.warnings)
            console.log(`   ⚠ ${w}`);
        if (Array.isArray(unit.profiles))
            for (const prof of unit.profiles)
                syncInvulnProfile(prof, projection);
    }
...
// ───────────────────────── datasheet lookup ─────────────────────────
export function findDatasheet(dump, unitId, dir) {
    const matches = [];
...
function projectUnit(dump, dir, unitId, files) {
    const warnings = [];
    const unit = files.units.find((u) => u.id === unitId);
    if (!unit)
        throw new Error(`unit "${unitId}" not found in ${dir}/units.json`);
    const ds = findDatasheet(dump, unitId, dir);
    if (!ds)
        throw new Error(`no unambiguous dump datasheet for "${unitId}" in ${dir}`);
    const gv = unit.game_version ?? { edition: "11th", dataslate: "launch" };
    // Valid repo weapon ids (for resolve-before-mint): existing faction weapons +
    // anything already on units / options in this faction.
    const validIds = new Set(files.weapons.map((w) => w.id));
    ...
    for (const it of dump.table("wargear_item")) {
        ...
        if (n)
            itemByName.set(n.trim().toLowerCase(), it);
    }
    ...
    const audit = [];
    const baseResolve = makeResolver(validIds, audit, WEAPON_ALIASES[dir] ?? {}, WEAPON_ALIASES_BY_UNIT[dir]?.[unitId]);
    const ctx = { dump, gv, warnings };
    const minted = new Map();
    const mintedWargear = new Map();
    const resolve = (name) => {
        ...
        if (item.wargearType === "weapon") {
            if (!minted.has(id)) {
                try {
                    ...
                    warnings.push(`could not mint "${name}": ${e.message}`);
                    return null;
                }
            }
        ...
        if (!mintedWargear.has(id))
            mintedWargear.set(id, mintWargear(ctx, item, id, name.trim()));
        validIds.add(id);
        return id;
    };
    const dsId = ds.id;
    ...
    for (const u of derived.unresolved)
        warnings.push(`unresolved: "${u.name}" (${u.context})`);
    for (const n of derived.notes)
        warnings.push(`note: ${n}`);
    // Composition: tiers + envelope from the dump's unit_composition rows.
    const agg = aggregateComposition(dump, dsId);
    if (agg.skip)
        warnings.push(`composition skipped: ${agg.skip} (kill-team shape — author by hand)`);
    const models = [];
    ...
    for (const name of orderedNames) {
        ...
        for (const w of def)
            if (!mintedWargear.has(w))
                weaponIdSet.add(w);
        const m = { name, min: env.min, max: env.max };
        if (def.length)
            m.default_weapon_ids = def;
        models.push(m);
    }
    if (models.length === 0) {
    ...
    derived.options.forEach((o, i) => {
        ...
        if (o.model_constraint && Object.keys(o.model_constraint).length)
    ...
    const invuln = projectInvulnerableSave(dump.groupBy("invulnerable_save", "datasheetId").get(dsId) ?? []);
    for (const w of invuln.warnings)
        warnings.push(w);
    const composition = models.length > 0
        ? {
            unit_id: unitId,
            faction_id: unit.faction_id,
            models,
...
async function main() {
    ...
    for (let i = 0; i < argv.length; i++)
        if (argv[i] === "--unit")
            unitIds.push(argv[i + 1]);
    if (!dir) {
        console.error("Usage: project-loadout --dir <faction> --unit <id> [--unit <id> ...] [--invulns-only] [--dump <path>] [--write]");
        process.exit(2);
    }
    ...
    const files = {
        units: readJsonArray(unitsPath),
        weapons: readJsonArray(weaponsPath),
        wargear: readJsonArray(wargearPath),
        comps: readJsonArray(compsPath),
        options: readJsonArray(optionsPath),
    };
    let targets = unitIds;
    if (allSkeletons) {
    ...
    for (const unitId of targets) {
        try {
            ...
            console.error(`[${dir}] ${unitId}: ${e.message}`);
            projections.push({
                unitId,
                mintedWeapons: [],
                mintedWargear: [],
    ...
    for (const p of projections) {
        console.log(`\n## ${p.unitId}`);
        console.log(`   minted ${p.mintedWeapons.length} weapon(s): ${p.mintedWeapons.map((w) => w.id).join(", ") || "—"}`);
        ...
        for (const w of p.warnings)
            console.log(`   ⚠ ${w}`);
        console.log(`   minted ${p.mintedWargear.length} wargear: ${p.mintedWargear.map((w) => w.id).join(", ") || "—"}`);
        ...
        const toVariant = (id) => refresh && weaponIdsPresent.has(`${id}-${p.unitId}`) ? `${id}-${p.unitId}` : id;
        const weaponIds = p.weaponIds.map(toVariant);
        if (p.composition?.models) {
            for (const m of p.composition.models) {
                if (Array.isArray(m.default_weapon_ids))
                    m.default_weapon_ids = m.default_weapon_ids.map(toVariant);
            }
        }
        ...
        if (u) {
            if (((u.weapon_ids?.length ?? 0) === 0 || refresh) && weaponIds.length)
                u.weapon_ids = weaponIds;
        ...
        if (p.composition && !compsOut.some((c) => c.unit_id === p.unitId))
            compsOut.push(p.composition);
        ...
        if (p.options.length && !optionsOut.some((o) => o.unit_id === p.unitId))
            optionsOut.push(...p.options);
    }
    const staged = [
...
//# sourceMappingURL=project-loadout.js.map