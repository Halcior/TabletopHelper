 ...
 */
export function requiredKeywordsForDetachment(dump, detId, unresolved) {
    ...
    for (const e of edges) {
        ...
        if (label)
            ...
            unresolved.push(e.factionKeywordId);
    }
    if (labels.length === 0)
        return null;
    // Ownership keyword present ⇒ roster-wide enumeration, not a restriction.
    if (ownName && labels.includes(ownName))
        return null;
    ...
    const narrowing = [...new Set(labels)].filter((l) => l !== ownName && !(dir && nameToId(l) === dir));
 ...
 */
export function ruleIdsForDetachment(dump, detId) {
    const ids = new Set();
...
function authoredRuleLinks(det) {
    const s = new Set();
    if (det.detachment_rule_id)
        s.add(det.detachment_rule_id);
    for (const id of det.detachment_rule_ids ?? [])
        s.add(id);
...
function makeAbilityIdLoader() {
    ...
    return (dir) => {
        ...
        const set = new Set();
        if (fs.existsSync(p)) {
            for (const a of readJsonArray(p)) {
 ...
 */
function dumpDetIdByRepoId(dump) {
    const byDir = new Map();
    const register = (dir, slug, detId) => {
    ...
    for (const det of dump.table("detachment")) {
        ...
        for (const kw of requiredKeywordsForDetachment(dump, det.id) ?? []) {
            ...
            if (chapterDir && chapterDir !== ownDir)
                register(chapterDir, slug, det.id);
        }
    }
    return byDir;
}
...
export function applyAuthoritativeDetachmentFields(detachment, tags, requiredKeywords) {
    const tagsChanged = !same(detachment.tags, tags);
    if (tagsChanged)
        detachment.tags = [...tags];
    const requiredKeywordsChanged = !same(detachment.restrictions?.required_keywords, requiredKeywords);
...
export function runDetachmentFields(dump) {
    ...
    for (const dir of [...repoDirs()].sort()) {
        ...
        for (const det of dets) {
            ...
            if (derivedRules.length > 0) {
                ...
                if (authoredRules.length === 0) {
                    ...
                    if (unauthored.length > 0)
                        res.ruleUnauthored.push({ id: det.id, derived: unauthored });
                }
            ...
            if (unresolved.length)
                res.unresolvedKeywords.push({ id: det.id, ids: [...new Set(unresolved)] });
        }
        if (changed)
...
export function buildDetFieldsReport(report, write) {
    const { dirs } = report;
    ...
    for (const d of dirs.filter((d) => d.matched)) {
        if (!d.tagsChanged.length && !d.tagsConfirmed &&
            !d.reqChanged.length && !d.reqConfirmed &&
            !d.ruleFilled.length && !d.ruleConfirmed && !d.ruleReview.length && !d.ruleUnauthored.length)
            continue;
        L.push(`| ${d.dir} | ${d.matched} | ${d.tagsChanged.length} | ${d.tagsConfirmed} | ${d.reqChanged.length} | ${d.reqConfirmed} | ${d.ruleFilled.length} | ${d.ruleConfirmed} | ${d.ruleReview.length} | ${d.ruleUnauthored.length} |`);
    }
    L.push(`| **TOTAL** | **${sum((d) => d.matched)}** | **${sum((d) => d.tagsChanged.length)}** | **${sum((d) => d.tagsConfirmed)}** | **${sum((d) => d.reqChanged.length)}** | **${sum((d) => d.reqConfirmed)}** | **${sum((d) => d.ruleFilled.length)}** | **${sum((d) => d.ruleConfirmed)}** | **${sum((d) => d.ruleReview.length)}** | **${sum((d) => d.ruleUnauthored.length)}** |`);
    L.push("");
    for (const d of dirs) {
        const details = [];
...
//# sourceMappingURL=detachment-fields.js.map