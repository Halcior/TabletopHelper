 ...
 */
export function buildCanon(dump) {
    ...
    const overrideBySlugDir = new Map();
    for (const row of dump.table("detachment_faction_detachment_points_cost")) {
        ...
        if (slug && dir)
            overrideBySlugDir.set(`${slug}@@${dir}`, row.detachmentPointsCost);
    }
 ...
 */
export function combatPatrolDetSlugs(dump) {
    const slugs = new Set();
    for (const det of dump.table("detachment")) {
...
function dispEqual(a = [], b = []) {
    return a.length === b.length && [...a].sort().join("|") === [...b].sort().join("|");
}
export function runDispositions(dump, write, opts = {}) {
    ...
    for (const dir of [...repoDirs()].sort()) {
        ...
        for (const det of dets) {
            ...
            if (cpSlugs.has(det.id))
                det.game_modes = ["combat-patrol"];
        }
    ...
    for (const s of unmatched) {
        if (!opts.includeCombatPatrol && cp.has(s))
            cpExcluded.push(s);
        else
            newInDump.push(s);
    }
...
export function buildDispReport(report, write) {
    ...
    for (const d of dirs.filter((d) => d.matched || d.unmatchedRepo.length)) {
        L.push(`| ${d.dir} | ${d.matched} | ${d.dpChanged.length} | ${d.dispChanged.length} | ${d.unmatchedRepo.length} |`);
    }
    L.push(`| **TOTAL** | **${sum((d) => d.matched)}** | **${sum((d) => d.dpChanged.length)}** | **${sum((d) => d.dispChanged.length)}** | **${sum((d) => d.unmatchedRepo.length)}** |`);
    L.push("");
    for (const d of dirs) {
        if (!d.dpChanged.length && !d.dispChanged.length && !d.unmatchedRepo.length)
            continue;
        L.push(`## ${d.dir}`);
        ...
        if (d.dispChanged.length) {
            ...
            d.dispChanged.forEach((c) => L.push(`- ${c.id}: [${c.from.join(", ")}] → [${c.to.join(", ")}]`));
        }
    ...
    if (cpExcluded.length) {
        L.push(`## Combat-Patrol detachments held back (${cpExcluded.length} — pass --include-combat-patrol to author)`);
        L.push("");
