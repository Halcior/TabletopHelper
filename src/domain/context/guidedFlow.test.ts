...
describe('Guided Mode progression guard', () => {
  ...
  it('clears the blocker after CP is recorded and keeps Fast Mode non-blocking', () => {
    ...
    expect(guided.sections.flatMap((section) => section.items).find((item) => item.type === 'COMMAND_POINT')?.status).toBe('DONE')
    const fastSession = {
      ...testCauldronGame(),
    ...
    expect(fast.sections.flatMap((section) => section.items).find((item) => item.type === 'COMMAND_POINT')?.status).toBe('AVAILABLE')
  })
})
